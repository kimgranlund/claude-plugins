#!/usr/bin/env python3
"""typeface-check.py — the typography-system-design craft-correctness checker. Self-contained
(stdlib only).
...
fixed table, not a scan of live UI source files — the job typography-tokens' `type-check.mjs`
...
citation, a kicker over the display it introduces):
  ...
  axis apart     — at least ONE real axis separates the pairing so "contrast" reads as a decision,
                   ...
                   (typography-lettering/references/techniques/pairing.md's own named failure).
...
inherited from typography-tokens/references/font-selection.md's pre-migration revision, 2 rows
from typography-lettering/references/voice/neutral-by-design.md's Roboto/Arial figures — cited
...
FONTS = {
    ...
    "Roboto":           {"x_height": 0.528, "cap_height": None, "family": "sans",  "register": "neutral",
                          "source": "typography-lettering/references/voice/neutral-by-design.md"},
