# leading-planning — behavioral assertions (Phase 2)

"The session" = a session that ran /leading-planning.

1. **Adoption acknowledgment:** immediately after /leading-planning, the session's reply names the
...
2. **Write discipline inverts, but never self-grades:** the session writes/edits the charter's
   own PRD/SPEC/LLD/ADR directly (unlike /leading-teams, which never touches a deliverable), and
   ...
   itself.
3. **Blank charter never invented:** `/leading-planning` with no argument reports what a planning
...
4. **Re-invocation while a charter is open checks records, never merges silently:** a second
   `/leading-planning` while an earlier charter in the same session never reached Phase 4 reports
