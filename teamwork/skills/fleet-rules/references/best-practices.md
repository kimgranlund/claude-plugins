...
- **A fleet-registered seat's dispatch states `model` explicitly, read from `seats.<role>.tier`'s model half — never left to frontmatter alone (issue #919).** A correctly-named `Agent`-tool dispatch against a real agent file already inherits that file's own frontmatter tier by default (the platform's own per-dispatch `model` param overrides frontmatter, frontmatter overrides session-inherit) — but frontmatter is a hardcoded estate default, not this REPO's own manifest, so a `fleet.json` seat carrying a recorded per-repo MODEL deviation (`seats.<role>.justification_date`) is silently ignored unless the dispatcher reads that value and passes it explicitly. Two dispatch shapes carry NO frontmatter to fall back on at all, and silently ride the dispatching session's own model instead (`agent-writing-rules` §Model tiering, issue #313): an ad-hoc `Agent` call (`subagent_type: general-purpose`, or any target naming no real agent file — a stale/misspelled `subagent_type` string degrades to this same silently-unpinned shape, not a load error) and a `Workflow` `agent()` call (frontmatter concept doesn't apply there at all — every stage states its own `model`/`effort` opts or inherits the workflow-runner's). A genuine `claude -p` subprocess spawn (`fleet-bootstrap`'s `reviewer` path) is a third unpinned-by-default shape again: its own CLI default, set independently of any dispatching session, applies unless `--model`/`--effort` are passed on the spawn command line. **`effort` is a narrower surface than `model` — a plain `Agent` dispatch cannot vary it per-call at all; the target's frontmatter value is what it always gets** (`agent-writing-rules` §Model tiering), so a recorded `seats.<role>.tier` effort deviation can only be realized via a `Workflow` `agent()` call (which does take both opts) or an edit to the seat's own frontmatter — never claimed as an `Agent`-tool dispatch param, which would silently no-op. **`context: fork` dispatches are the one shape that cannot be pinned on EITHER axis** (issue #313, already noted for forks) — the fork rides the dispatching session's own model structurally, with no override channel; name that caveat rather than treating it as a gap to close.
