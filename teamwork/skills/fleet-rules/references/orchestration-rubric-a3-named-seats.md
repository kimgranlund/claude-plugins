...
| A3-R2 | **(G1)** Durable-channel evidence reader: charters carry the cloud-can't-message-back caveat; anything a peer must see goes to `fleet.json`/roster/GitHub, never solely a nudge — the READER locates and parses these channels as the primary conduct-evidence source | 2026-08-18 fold-in ruling on #666; `fleet-rules` Part A §3 | **mechanizable — built** — `orchestration-audit`'s `scripts/audit.py` `check_durable_channel` check: confirms `.claude/ops/fleet.json` parses as JSON and `.claude/ops/fleet-roster.md` exists with ≥1 parseable data row, then reconciles roster rows against `fleet.json`'s own `live_state.joined` entries, naming any mismatch either direction. This is `fleet.json`↔roster reconciliation ONLY — it never reads a GitHub Issue/PR comment trail (that half of A3-R2's own evidence source stays **mechanizable — not built**, a real future check, not judgment: the comment-trail API calls are a straightforward addition, just not yet written) |
