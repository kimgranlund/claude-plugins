...
| `harness:check-state --fleet` | The report-side realization of fleet-wide state visibility (open work, plugin-cache version drift, cross-repo consolidating-record citations) — cited here, never restated; collector logic and report shape live in `check-state` alone (lld-0011, #620) |
