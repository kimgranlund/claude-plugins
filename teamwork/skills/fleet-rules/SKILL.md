...
| `references/best-practices.md` / `references/delegation-plan-schema.md` | Part B design guidance / explaining a finding — the latter is Design step 5's citable task-graph shape (`id`/`assignee`/`depends_on[]`/`outputs[]`/`mode`, propose→merge→delegate→execute, read-only-by-tool-allowlist) for a fan-out with real cross-task dependencies |
