...
description: >-
  ...
  PRD/SPEC/LLD/ADR the change actually earns. Unlike lead-team, the host writes every design doc
...
# lead-planning — the host runs the design seat, not a dispatched copy of it

`planner` (`teamwork/agents/planner.md`) is the dispatched form of the standing design seat. This
command is the other half of the pair — the `/lead-team` ↔ `team-lead` pattern, teamwork's fifth
`/lead-*` member: it makes **this session** — the one the human is typing into — hold that
...
- **Write discipline INVERTS relative to lead-team.** `/lead-team`'s one rule is the host never
