...
Shared by every `/bind-*` skill-as-command that makes the host session run under a dispatched agent's own
contract instead of spawning a copy of it (`bind-planning` ↔ `planner`, `bind-team` ↔ `fleet-marshal`,
`bind-build` ↔ `build-leader`). `bind-review` is deliberately NOT a party to this ritual — it adopts
...
`bind-planning`/`bind-team`, session-scoped for `bind-build`) — those are genuine per-seat
...
4. **Re-acknowledge, never stack, on re-invocation — the session-scoped variant only
   (`bind-build`).** For a session-scoped adoption, re-invocation always lands inside the still-
   ...
   charter-scoped adoption (`bind-planning`, `bind-team`) instead follows the second failure
