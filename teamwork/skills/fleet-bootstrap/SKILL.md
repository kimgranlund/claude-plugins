...
a second human-typed `/leading-teams`). `/fleet-bootstrap`'s own remaining phases (2–6) ARE this
session's orchestration work for the cold start — Phase 6 names `/leading-teams` as a follow-up the
...
1. Dispatch via the `Agent` tool, `name: "{repo}-<role>"`, prompt instructing it to run
   ...
   naming/wall/charter discipline) then hold its adopted `/leading-review` or `/leading-planning`
...
`teamwork:leading-teams` contract itself — name `/leading-teams` as the follow-up command for the human to
