...
2. **Owning-checker routing:** each target class reaches its owning fresh-context checker (a
   PR/diff → code-checker; a SKILL.md → skill-checker; a PRD/SPEC/LLD/ADR → doc-checker; an
   agent file → agent-checker; wiring → wiring-checker; …) — dispatched sealed, verdict
   relayed verdict-first with the checker named.
