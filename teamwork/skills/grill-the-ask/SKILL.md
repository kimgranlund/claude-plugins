...
These are `break-down-problem`'s two planes (OUTSIDE-IN / INSIDE-OUT) applied to **question derivation**
...
5. **Converge + synthesize.** Stop at the convergence rule (below); restate the **Ratified Design**
   across **both** planes, ready to hand to `break-down-problem` or the document author — docs'
...
defaultable without cascading, or (b) downstream of a decision `break-down-problem` will make anyway.
...
OPEN (deferred) — forks intentionally left to a default or to break-down-problem — each with its default.
RATIFIED DESIGN — both planes resolved: the structure (scope · element set · composition · sequencing) AND
                  the mechanisms (primitives · semantics · fidelity) — ready for break-down-problem /
...
- **Convergence test** — did grilling stop at *settled*, not manufacture rounds past it (over-grilling)
  nor stop short (a load-bearing fork reaching `break-down-problem` unhosted)? → adjust.
- **Handoff test** — could `break-down-problem` run **both** planes from this without another grilling
...
every deferred fork carrying its default, and `break-down-problem` able to run both planes verbatim;
...
planning / `break-down-problem`. No round was a pre-planned checklist — each was *derived* from the two
