...
**Cold-start seeds the full ladder in a fresh repo (#655 decision 6, closing the cross-repo seam
#652 came through) — every seeded artifact named here, not left implicit:** a draft product
**brief**, the harvest-surfaced draft **IDRs**, and an empty **ROADMAP** index (homes, not
content — the flow's own day-0 guardrail). Each lands per `docs:doc-writing-rules`' own
docs-root ruling (issue #514's three-rung ladder: a host override, the portable `docs/ops/`
default, or this workspace's own `.claude/docs/` carve-out) and carries that same skill's
