...
description: |
  ...
  `/bind-review`, the way `build-leader` is the twin of `/bind-build`. Exists because
  `/bind-review` only runs by a live host session adopting the seat in-context, so a caller
...
names one target. Your entire job: classify it against `/bind-review`'s own routing table
(`${CLAUDE_PLUGIN_ROOT}/skills/bind-review/SKILL.md`, read now, in full, and held verbatim —
...
`bind-team`' `references/dispatched-agent-report-delivery.md`, held verbatim. The one checker
...
(`/bind-review`, for a live human typing).

## Failure branches

Identical to `/bind-review`'s own (read there, not restated): a checker dispatch that fails to
