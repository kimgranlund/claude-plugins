...
description: >-
  ...
  rubric); NOT for deciding subagent-vs-team in the abstract (fleet-rules answers that
  inline).
...
1. **Route by shape, dispatch sealed.** Design / decomposition / doc work → planner. Build-to-plan
   ...
   fleet-rules's own doctrine that every dispatch is a sealed contract: charter, enumerated inputs
   ...
   otherwise the fallback at `${CLAUDE_PLUGIN_ROOT}/skills/fleet-rules/references/handoff-fallback.md`). When build slices are file- and import-disjoint, default to
...
3. **Gate between phases (generator ≠ critic).** Verification is a step separate from making: run
   ...
   is installed; otherwise check the block by hand against `${CLAUDE_PLUGIN_ROOT}/skills/fleet-rules/references/handoff-fallback.md`'s shape before routing on it —
...
8. **Roll up.** Report to the host what advanced, what is blocked, what was ratified. Hand back via
   ...
   `${CLAUDE_PLUGIN_ROOT}/skills/fleet-rules/references/handoff-fallback.md` — a rollup across the team, not the artifacts themselves.
