...
description: >-
  ...
  wires them — scored against the fleet-rules rubric. Use PROACTIVELY after an agent
...
- **Score against the fleet-rules rubric** (preloaded; it declares its own dimensions and
...
Return the gap-map in fleet-rules's Review output contract
(`${CLAUDE_PLUGIN_ROOT}/skills/fleet-rules/SKILL.md`) via harness's `write-handoff` block
where harness is installed; otherwise the fallback at
`${CLAUDE_PLUGIN_ROOT}/skills/fleet-rules/references/handoff-fallback.md`. Either shape: Files changed = (none,
