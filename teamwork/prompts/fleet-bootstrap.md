---
description: "Full one-terminal fleet cold-start (#410 addendum 3, level 2 — composes team-scaffolding + the roster): adopts the orchestrator seat, dispatches the product seat to draft the intent layer, then HARD-GATES on human ratification of that draft (AskUserQuestion where reachable; no live user stops and names the pending gate). Seeds or updates the fleet manifest (roster, tier deviations with dated justification, manned-vs-background, live seat state). Run /fleet-bootstrap [spawn-list], a comma-separated subset of reviewer,planner (default empty — those two are manually operated per #410 addendum 3), always confirmed via one AskUserQuestion before spawning — the argument only pre-selects, it never skips that confirm. NOT a single seat's own bootstrap (team-scaffolding — reproduced inline here for the orchestrator seat, called directly for reviewer/planner spawns; alone correct for a human joining one seat by hand); NOT for redoing the gate after ratification (fires once per cold start)."
...
`mode: "manual"` for every seat, empty `live_state.joined`, `expected_branch` per below) — no
interview, matching `team-scaffolding`'s own rule that an explicit command invocation with no role
question asked is itself the human's act of declining one; running `/fleet-bootstrap` at all is
that same explicit act. **Seed `expected_branch`** (`references/fleet-manifest-schema.md` §"Branch
reconcile on every bind", canonical there, issue #932): `git rev-parse --abbrev-ref HEAD` right
now, at seed time. Seeding on `main` (the ordinary case, the primary checkout) → omit the field,
letting it default to `"main"` per the schema's own absent-reads-as-main rule — never write the
literal string redundantly. Seeding from a non-`main` branch — this cold start is running inside
an already-checked-out campaign worktree — → write that branch's name explicitly as
`expected_branch`, the worktree-isolated case `fleet-rules` already licenses ("a campaign gets its
own branch + worktree"). **Name the resolved scope root (its basename) and which branch (existing-read vs.
...
4. Print the comms charter (`team-scaffolding` Phase 4): the `agent` seat tier (sonnet+high,
   ...
   (issue #919). **Also run the branch reconcile** (`references/fleet-manifest-schema.md`
   §"Branch reconcile on every bind", canonical there, issue #932): `git rev-parse --abbrev-ref
   HEAD` (and, where this session's cwd is a linked worktree — `git rev-parse --git-common-dir`
   vs. `--git-dir`, same test Phase 5's reviewer precondition already runs — that worktree's
   absolute path), diffed against `fleet.json`'s `expected_branch` (absent reads `"main"`),
   naming any mismatch plainly — never a hard stop, flag only.
5. **Resolve this session's real SendMessage address before writing anything**: read it from
   ...
   **Fresh join** (no live entry found in step 1): append `live_state.joined` (`role: agent`,
   `mode: manual`, `action: "joined"`, today's date, `agent_name`: the resolved address, `branch`
   and, when applicable, `worktree_path`/`branch_mismatch` from step 4's branch reconcile) in
   ...
   `branch` and, when applicable, `worktree_path`/`branch_mismatch` from step 4's branch reconcile)
...
**Return-address split (issue #993).** The two mechanisms above also split how `planner` reaches
this session back, and rule `reviewer` out of the split entirely: `planner`'s `Agent`-tool
dispatch is in-process — its own `SendMessage` finds this session only at the in-process address
`main`, never at this session's own harness/fleet name — so quote `main` into its dispatch prompt
as the literal return address, exactly as `mobilize-chores`'s own step 5 does. `reviewer`'s
`claude -p` subprocess carries **no `SendMessage` identity in either direction** — not `main`,
not a resolvable `agent_name` either (`references/reviewer-subprocess-spawn.md`'s own field note:
"there is no fleet messaging identity for a subprocess"; `fleet.json`'s `agent_name` for this row
holds a log-path/PID pointer, never a live address). It reports back only through its captured
stdout at spawn (the I2 probe, step 4 below) and through GitHub issue/PR comments during ongoing
review work — never through this split at all.
...
issue #852 found never actually enforces — an `Agent`-tool dispatch inherits the parent's
permission mode rather than re-deriving one from its own cwd, so it was never a genuinely new OS
process. Issue #853 live-tested the fix instead: a `claude -p` child spawned with cwd already
inside an ALREADY-walled worktree IS correctly denied, even under this machine's global
`bypassPermissions` default. Five steps realize that tested shape — worktree precondition (stop on
a shared checkout, never wall or spawn), write-and-verify the wall BEFORE any spawn, spawn exactly
one `claude -p` child with the model/effort flags passed explicitly on its command line and a
mandatory I2 three-probe self-check on stdout, the orchestrator polls and collects that verdict
into `live_state.joined`'s `wall_applied`/`wall_verified_via` fields, and ongoing review work rides
the wall already on disk via `teamwork/scripts/reviewer_scheduler.py` rather than any long-lived
process. **Full mechanics — every step's exact command, the I2 probe format, the four
`wall_applied` outcome values, and the `fleet.json` append contract**: `references/
reviewer-subprocess-spawn.md` (F6 split); read it before this dispatch's first live firing.
...
**Close with the address roster (Kim's ruling, 2026-08-22).** Every seat, one line each, naming
its resolved `agent_name` (never the `@{scope}-<role>` label, which is a display sigil only — no
session is ever registered under that printed string) and classified into one of four
addressability states off `fleet.json`'s LATEST row for that role. Exact line shape, the
four-class breakdown, and the label-vs-`agent_name` authority split: `references/
address-roster.md` (F6 split) — read it before this phase's first live firing.
