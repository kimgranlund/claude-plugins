---
description: "Makes this session a dedicated planning seat, running the planner agent's own contract for one named charter: decompose across both planes, then author only the PRD/SPEC/LLD/ADR the charter earns, each doc riding to docs:doc-checker fresh-context before it counts gated. Holds until the charter closes on a named loop-rules decision. Run /bind-planning [charter] — blank binds a default charter: adopt against the current repo and hold for the first design/decomposition ask fed into the session. NOT implementing an approved LLD (builder, Agent tool); NOT a bugfix or single-file change (host states Components/Risks inline, no doc); NOT a generic multi-seat charter (/bind-team); NOT reviewing an existing design doc standalone (docs:check-doc); NOT an unattended dispatch (planning-leader, Agent tool)."
