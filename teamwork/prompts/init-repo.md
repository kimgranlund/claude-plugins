...
The /bind-* family's composer: everything it does is a shipped part reached by reference — the
...
3. **Adopt team-lead — this session becomes the apex, no spawn.** `/bind-team` itself is
...
6. **The armed report.** One closing block: each step's outcome; how to feed each seat (raw
   ...
   its owning checker, or `/bind-review` in a dedicated session); the per-session lifetime
