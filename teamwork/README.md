...
v2.12.0 · 2026-08-14 · quick-build auto-merge (#244, ADR-0012 proposed, lld-0002 v0.2.0): `dispatch-ticket` Phase 5 gains stage 2b — on an explicit `auto-merge: authorized` grant plus an all-green fail-closed QB0–QB7 predicate (QB4 an ALLOW-list, so an unlisted file class is ineligible by construction), the seat watches CI under `timeout 900`, squash-merges, SHA-verifies MERGED, runs `campaign_close.py`, posts a dated QB snapshot; any miss falls back to PR-opened. Critic, gates, CI and the PR are never skipped. build-lead/build-feature/mobilize-chores + the workspace CLAUDE.md amended in step
