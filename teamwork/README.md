...
| `scripts/merge_queue_watch.py` | Plugin script | `${CLAUDE_PLUGIN_ROOT}/scripts/merge_queue_watch.py [--repo <owner/repo>]` — run by the marshal seat (`fleet-rules`) once per polling wave in place of per-PR watch tasks | Batched merge-queue watcher (#713): polls every open PR's head SHA via the `check-runs` REST endpoint (never `gh pr checks --watch`, whose exit code lies on non-terminal/failed states) and reports one consolidated `green: #A #B / pending: #C / failed: #D` line, replacing two feed events per PR per wave with one line per wave. A failed check-run always wins over a pending one on a mixed PR; an unrecognized future conclusion value fails closed. `selftest` (zero network) proves the classification/report logic incl. the mixed and fail-closed negative controls |
| `scripts/gate_guard.py` | Plugin script | `${CLAUDE_PLUGIN_ROOT}/scripts/gate_guard.py record\|run <gate.py> [<clone-dir>]` — called by `dispatch-ticket`'s gate-first mode round loop (Phase 5, `references/gate-first-mode.md`) | Hash-record/hash-check/bounded-run/tri-state wrapper (LLD-0026, gh#939) for a gate-first validator-authored gate script: `record` writes the gate's SHA-256 immediately after authoring or repair; `run` refuses to execute a gate whose current hash no longer matches the recorded one (`gate-tampered`, exit 2), otherwise runs it bounded (`--timeout`, default 300s) and relays the gate's own tri-state exit/stdout verbatim. `selftest` proves record/run, FAIL/PASS relay, real tamper detection, post-repair re-record, and the timeout bound — no network |
...
v2.29.13 · 2026-08-31 · closes #1008 (harness slice): new plugin bytes — harness's `harness_emit.py`/`release_gate.py` cross-harness fidelity upgrade regenerates this plugin's own overlays: Codex `agents/openai.yaml` now carries `policy.allow_implicit_invocation: false` for a command-only skill (previously silently model-routable on Codex); `HARNESS-NOTES.md` gains a `## Cross-harness token degradations` section naming every skill whose SKILL.md body carries a Claude-only `${CLAUDE_PLUGIN_ROOT}`/`$ARGUMENTS` token; new gate G15b (WARN-only) surfaces the same inventory. A cached install must refresh to receive the overlay. PATCH: overlay bytes only, no behavior change to this plugin's own skills.
v2.29.12 · 2026-08-29 · closes #994: overdue build handbacks were undetectable from the record — two agent-ui build-leader dispatches (90/60-minute budgets) ran 5-6 hours with no chase, alive but blocked on long test runs, emitting only content-free idle notifications. `agents/build-leader.md` and `dispatch-ticket`'s Phase 4/5 now require a dated progress comment on the ticket at budget/2 and again before any single wait longer than 5 minutes, naming what it is waiting on. `agents/fleet-marshal.md` Priority 5 now arms a wake-up at dispatch for the seat's stated budget (a `CronCreate` wake or a `Monitor` until-loop — the implementer's call), chases on expiry by re-reading the ticket for the owed progress note plus a direct nudge, and re-dispatches on a second silence; `bind-team`'s host-adopted Priority 5 mirror states the identical timer/chase/re-dispatch sequence. The 5-minute wait threshold and budget/2 checkpoint are the ticket's own seed numbers, to be tuned only against evidence from a second incident. Reviewed by fresh-context `agent-checker` passes on both agent files and a `skill-checker` pass on both skill edits per the plugin-authoring semantic-edit gate. PATCH: comms-discipline requirement across four files, no behavior change to dispatch mechanics themselves.
v2.29.11 · 2026-08-29 · closes #993: `mobilize-chores` step 5, `fleet-bootstrap` Phase 5, and `fleet-manifest-schema.md`'s `agent_name` note now state the return-address split explicitly — an in-process `Agent`-tool dispatch (no separate session, e.g. a `build-<ticket-id>` build seat or `planner`'s own `fleet-bootstrap` Phase 5 dispatch) reaches its own dispatcher only via `SendMessage` to `main`, the in-process address; a genuinely separate harness session with a real messaging identity (a `mode: "background"` Agent-tool seat, or a live human terminal) is addressed via that session's `fleet.json` `agent_name` field instead — `main` is not a synonym for the coordinator's own harness/fleet name, and reading the wrong one finds no such peer reachable. `fleet-bootstrap`'s new Phase 5 paragraph also corrects an adjacent error the first draft introduced and this same review caught: `reviewer`'s `claude -p` subprocess carries no `SendMessage` identity in either direction (its `agent_name` row holds a log-path/PID pointer, not a live address) and sits outside this split entirely, reporting back only via its spawn-time stdout capture and GitHub comments. `mobilize-chores` step 5 also carries a literal dispatch-prompt example line (`Return address: SendMessage to "main" (this dispatching session).`). `fleet-bootstrap`'s Phase 6 address-roster block (unchanged content) moved to `references/address-roster.md` to hold the body under the F6 line cap after the new Phase 5 paragraph. Reviewed by a fresh-context `skill-checker` pass per the plugin-authoring semantic-edit gate (caught and fixed one MAJOR — the reviewer-addressing error above — plus a parenthetical tightening in harness's `agent-writing-rules` and a line-wrap artifact, before landing). PATCH: doctrine clarification across three files, no behavior change to dispatch mechanics themselves.
v2.29.10 · 2026-08-28 · closes #980: reconciles a same-day (2026-08-23) internal contradiction between `fleet-bootstrap`'s Phase 6 address-roster block, which falsely claimed the printed `@{scope}-<role>` label IS itself a reachable `SendMessage` target, and `fleet-rules` Section 7 / `fleet-manifest-schema.md`, which correctly document the resolved `agent_name` field as the only real address (#902). The roster block now prints each seat's resolved `SendMessage:` address alongside its label, or `none` when unresolved, and states plainly that the label alone never routes. `team-scaffolding`'s Phase 2 `.claude/ops/fleet-roster.md` row-shape contract gains an `agent-name` column (role · session-name · agent-name · date · repo) — resolved before the roster line is appended, not after — so the file peers read for discovery now carries the routable address, closing the gap that left a caller needing to open `fleet.json` by hand. The retire-sync row (Phase 5) matches the new 5-column shape. PATCH: doctrine reconciliation + a discovery-surface field addition, no behavior change to `SendMessage` resolution itself (that resolver is harness-level, outside this plugin).
v2.29.9 · 2026-08-28 · closes #979: `dispatch-ticket`'s stage-2b QB3 auto-merge predicate widened — R now also exempts the G15-mandated harness-overlay riders (`.codex-plugin/plugin.json`, `HARNESS-NOTES.md`, `plugin.yaml`, per-skill `agents/openai.yaml`, plugin-root `prompts/*.md`) that `harness_emit.py` regenerates alongside an ordinary skill edit, so a normal single-skill semantic-edit PR with QB1/QB2/QB4 green can actually clear QB3 instead of failing on 4+ changed files every time. `__init__.py`, `hermes-mcp.yaml`, and `package.json` deliberately stay OUT of R — same G15 output, narrower fix scope, per the ticket's own Acceptance. PATCH: predicate-staleness fix, no new capability.
v2.29.8 · 2026-08-28 · closes #977: `fleet-bootstrap` Phase 2 no longer dispatches `product-leader` unconditionally — three consecutive no-material-change cold starts (2026-08-23/26/28) motivated a throttle. Phase 3's `live_state.gate` write is now an object (`outcome`/`date`/`since_commit`, the last-ratification git SHA), and Phase 2 diffs `since_commit..HEAD` for `adr-*.md`/`idr-*.md`/`prd-*.md` filename edits plus a queued `falsified` row in `.claude/ops/revalidation-queue.json` before dispatching; neither signal skips the dispatch and records it as a `live_state.joined` row (`mode: "skipped"`). A legacy bare-string `gate` value or an unreachable `since_commit` (rebase/force-push) falls back to unconditional dispatch, never a guessed skip. `references/fleet-manifest-schema.md` documents both the new `gate` object shape and the `"skipped"` mode value.
...
section, and Phase 5's existing `/goal` try-cap threads gate FAIL lines back verbatim as each
round's work instruction, capped at 3 rounds with `loop-rules`' triage-diagnostician composing on
top rather than a second budget. New `teamwork/scripts/gate_guard.py` hash-records/hash-checks the
gate before every bounded run (tri-state exit, `selftest` with tampered and failing fixtures). New
`references/gate-first-mode.md` carries the full mechanics; `SKILL.md` Phase 4/5/Failure branches
and `agents/build-leader.md` gain short pointer/relay additions. Default path (no grant line) is
byte-for-byte unchanged. · v2.28.54 · 2026-08-25 · closes #936: `chores-run`'s step 2 (the one skill body in this estate with
...
`VALIDATOR_TOOLS`→`READONLY_TOOLS` flip). The existing escalation clause ("the same check failing
twice → stop and report, not retry", Design step 2) now routes through one structured diagnosis
first: a fresh-context, read-only checker-class seat inspects the real project state (never the
builder's claims), compares it against what the gate demands, and returns a bounded
Diagnosis/Do-exactly-this/Do-NOT brief as advisory context — the gate's own output stays the
source of truth. Only when the diagnosis finds the gate itself defective (impossible to satisfy,
or demanding something never asked) may the diagnostician repair it, exactly once per run:
byte-diff the gate file to detect the repair, keep the pre-repair version as an audit copy, drop
the seat to read-only tooling the moment the repair lands, and re-run the repaired gate without
charging the builder a try. The never-weaken rule holds throughout — every check mapping to a real
requirement stays full strength; this fixes the gate's own bug, it never moves goalposts, and the
canon states plainly that fusion-harness enforces this at the prompt level, not a tool-level
guarantee this stack has yet. Resolves the ticket's three Scope/Open gaps: N stays at the existing
clause's N=2 (no new parameter), the pattern lands in the SKILL.md body (short enough, and it's
canon a builder reads inline at Design time, not reference depth), and the body's "same check
failing twice" line (formerly line 80) now cites the diagnostician as its firing point. Fresh-context
critic pass owed per plugin-authoring.md's semantic-edit invariant.

v2.28.52 · 2026-08-25 · closes #937: adds a delegation-plan schema to `fleet-rules` Part B, Design step 5 — a citable JSON task-graph shape for multi-seat fan-out with real cross-task dependencies, adapted from disler/fusion-harness's collaboration mode. New `references/delegation-plan-schema.md` (F6 split, keeps SKILL.md under the body budget) carries the `id`/`assignee`/`description`/`depends_on[]`/`outputs[]`/`mode: read|write` schema, the acyclic-dependency and maximize-parallelism rules, the read/write serialization rule (read overlaps anything, write serializes against the shared tree per Design step 5's own host-owns-git precondition), and the one-coordinator propose→merge→delegate→execute shape ("take the best ideas from every proposal; the plan is yours, not a vote") — including the read-only-by-tool-allowlist mapping onto this plugin's own sealed-dispatch contract (a proposal dispatch states `tools: Read, Grep, Glob`, not a prompt instruction). Design step 5 and the References table both cite it; this formalizes what Design step 5 + `parallel-work-rules` already said in prose, not new doctrine. Fresh-context `harness:wording-checker` FLOOR pass owed per plugin-authoring.md's semantic-edit invariant.
v2.28.51 · 2026-08-25 · fixes #932: `fleet-bootstrap`'s Phase 1 marshal bind and `fleet-connect`'s own join handshake registered seat identity but never checked git branch/worktree alignment, so a seat could join on the wrong branch with nothing flagging it until a downstream merge conflict or wrong-base PR surfaced the drift. Adds a top-level `expected_branch` field to `fleet.json` (absent reads `"main"`, per `fleet-rules`'s "shared primary checkout stays on `main`" doctrine; a campaign cold-start inside its own worktree seeds its branch name explicitly) and a "Branch reconcile on every bind" check, documented once in `fleet-manifest-schema.md` and cited from both callers: `fleet-bootstrap` Phase 1 captures the marshal's own branch/worktree via `git rev-parse` and writes `branch`/`worktree_path`/`branch_mismatch` onto its `live_state.joined` row; `fleet-connect` step 3 runs the identical self-check against the connecting working session before its handshake, reporting the outcome (it has no `fleet.json` write path, so nothing is persisted there). A mismatch is flagged, never a hard stop — same posture as the existing model-tier reality check (issue #919), left for `fleet-rules` Section 7's routing discipline to act on. No `version` bump on `fleet.json` itself — both new fields are additive with a safe absent-default, an explicit rejected-alternative recorded in the schema doc. F6 split: `fleet-bootstrap` Phase 5's `reviewer` subprocess-spawn mechanics moved to new `references/reviewer-subprocess-spawn.md` to make room under the 500-line body budget after this addition; SKILL.md keeps a condensed summary + citation. Fresh-context `harness:wiring-checker` FLOOR pass per plugin-authoring.md's semantic-edit invariant (procedural behavior change across two skills): PASS, both gates clear, no rework needed.
v2.28.50 · 2026-08-24 · fixes #930: `dispatch-ticket`'s Phase 5 stage 2 PR-body instruction ("Open exactly one PR against `main` carrying `Closes #<id>`") silently failed to trigger GitHub's auto-close on any repo whose commit/PR convention writes issue references as `gh#NNNN` — GitHub's parser requires a bare `#NNNN` immediately adjacent to `Closes`/`Fixes`/`Resolves`, and `gh#NNNN` (or `Closes` used as a prose verb) never matches, even though a human reading the PR body sees the instruction as satisfied. Confirmed live against 3 real merged PRs in `adiahealth/gen-ui-kit` (#1931, #1942, #1938), each needing a manual `gh issue close`. Adds a mechanical Closes-keyword gate right before the version-collision re-checks conclude and the PR opens: regex `\b(Closes|Fixes|Resolves)\s+#\d+\b` against the composed body for every id this dispatch closes, failing loud and blocking PR-open on a miss rather than shipping a non-closing reference silently. Full rationale + repro in new `references/closes-keyword-gate.md` (F6 split, keeps SKILL.md under the 500-line body budget). Fresh-context `harness:skill-checker` FLOOR pass per plugin-authoring.md's semantic-edit invariant: one placement defect found (gate landed after, not before, the "Open exactly one PR..." action paragraph) and fixed.
v2.28.49 · 2026-08-24 · fixes #927: `dispatch_envelope.py`'s scratch clone (`git clone --depth 1`, which implies `--single-branch`) left a narrow `remote.origin.fetch` refspec, so a dispatched seat's bare `gh pr create` failed with "you must first push" even after the branch was pushed (workaround rediscovered per dispatch: `--head <branch> --base main`). `_do_clone` now widens the refspec to `+refs/heads/*:refs/remotes/origin/*` right after the clone and on reuse of a prior run's clone; selftest gains a refspec leg proving it. Pure script change under selftest gates, no checker seat owed.
v2.28.48 · 2026-08-24 · fixes #921: retires `team-scaffolding`'s stale `docs` not installed and role is `product` failure branch — Phase 5's own table already moved `product`'s handoff to a same-plugin command (`/bind-product`, teamwork, issue #433) with no cross-plugin `docs` dependency left to degrade against, so the branch named an uninstalled plugin that no longer gates anything. Fresh-context `harness:wording-checker` pass owed per plugin-authoring.md's semantic-edit invariant.
v2.28.47 · 2026-08-24 · fixes #919: fleet seat tiers were reconciled in `fleet.json` but never actually wired into a dispatch — `fleet-bootstrap`'s product-seat dispatch named a stale, unresolvable `docs:product-leader-agent` (post-#433 move, silently fell through to session-model-inherit), its `planner` spawn and `reviewer` `claude -p` subprocess spawn carried no `model`/`effort` at all, and `dispatch-ticket`'s task-kind `general-purpose` Agent dispatch had the same unpinned gap. Fixed all four dispatch sites to state `model`/`effort` explicitly, read from `fleet.json`'s `seats.<role>.tier` (or `agent-writing-rules`' ad-hoc-dispatch doctrine for dispatch-ticket's non-seat task dispatch), corrected the stale product-leader reference. `fleet-manifest-schema.md`'s tier-reconcile section gains a "Reality check" subsection diffing the live BINDING session's own resolved model against its seat's tier (not just the manifest against the canonical ladder) — wired into both `fleet-bootstrap` Phase 1 and `team-scaffolding` Phase 4. `fleet-rules`' sealed-dispatch contract and `references/best-practices.md` state the discipline explicitly for any fleet-seat dispatch; the A7 Workflow-scripts orchestration rubric gains a new criterion (A7-R7) for per-stage `model`/`effort` on `agent()` calls. Fresh-context `harness:wording-checker` pass owed per plugin-authoring.md's semantic-edit invariant.
v2.28.46 · 2026-08-23 · fixes #915 (Kim's ruling 2026-08-23): opt-in scope-pointer file `<scope root>/.claude/ops/fleet-scope.json` (`{"scope_root": "<repo-root-relative path>"}`) redirects fleet scope resolution to the real scope root when the scope-local `ops/fleet.json` is absent — closes the #911 rule's monorepo false negative (a skills-only app `.claude/` read as a fleet scope boundary, declaring "no live marshal" while the repo-root manifest held one) without reintroducing #909 shadowing: local `fleet.json` always wins, one hop only (no pointer chains), pointer validity checked (in-boundary, target has `.claude/`), and with neither file present current no-walk-up behavior stands. Canonical rule added to `fleet-manifest-schema.md` §Location and resolution; `fleet-connect` step 1 + failure branch and `fleet-bootstrap` Phase 0 consume it by citation. Each participating app dir seeds its pointer deliberately; nothing seeds automatically.

v2.28.45 · 2026-08-23 · fixes #911: replaces the #906/#909 "nearest existing `fleet.json`" ladder in both `fleet-bootstrap` and `fleet-connect` with a scope-root rule — the fleet SCOPE ROOT is the nearest ancestor directory (cwd upward) that contains a `.claude` directory AT ALL, never the nearest one that merely happens to already hold a `fleet.json`; the target path is always `<scope root>/.claude/ops/fleet.json`, and an ancestor's `fleet.json` sitting above the resolved scope root is out of scope — never consulted, never written, however populated (was: an app with its own `.claude/`, e.g. `frontend/apps/signup`, silently inherited the repo root's existing `fleet.json` instead of getting its own, live-repro'd by Kim in adiav2/frontend/apps/signup, teamwork 2.28.44). `fleet-bootstrap` Phase 0 and Phase 1 seat naming now derive `{scope}` from the resolved scope root's own directory basename (`signup-marshal`, not `adiav2-marshal`); every `{repo}` token in the skill's own body renamed to `{scope}` for consistency. `fleet-connect` step 1 resolves the identical scope-root rule so it finds that scope's own live marshal. `fleet-manifest-schema.md`'s "Location and resolution" section and its "Schema key vs. printed session name" field rewritten to the `.claude`-directory rule. Evals: `fleet-bootstrap` t07, `fleet-connect` t07 — an app with its own `.claude/` but no `fleet.json` seeded yet, and an ancestor `fleet.json` present but out of scope. `team-scaffolding`'s own `{repo}-marshal` naming line is NOT trivially co-located with this fix — it hardcodes a literal `.claude/ops/fleet.json` path and the repo-root basename with no cwd-walk/scope-root resolution of its own today, a materially larger change than this ticket's blast radius — named as a follow-up rather than folded in here. Fresh-context `harness:skill-checker` FLOOR pass per plugin-authoring.md's semantic-edit invariant: findings applied.

v2.28.44 · 2026-08-23 · fixes #909: `fleet-bootstrap` Phase 0's virgin SEED path now runs the same cwd-first ladder `fleet-connect` step 1 already reads with (PR #907, teamwork 2.28.42): the nearest existing `.claude/ops/fleet.json` walking cwd-upward to the repo root wins for READ; nothing found at any rung → seed at the INVOKING cwd's own `.claude/ops/` when cwd is not the repo root (the app-scoped bootstrap re-homing ruling, Kim 2026-08-23), or the repo root's `.claude/ops/` when cwd IS the repo root — never a hardcoded repo-root default (was: `fleet-bootstrap` reading a monorepo app's cwd but always seeding `.claude/ops/fleet.json` at the repo root, live-repro'd in adiav2/frontend/apps/signup). `fleet-roster.md`'s append now sits alongside whichever `fleet.json` Phase 0 resolved rather than a separately re-derived path. Phase 6's report names the resolved root and which branch (existing-read vs. fresh-seed, app-scoped vs. repo-root). `fleet-manifest-schema.md`'s Location-and-resolution section extends its cwd-first ladder note to cover the seed side explicitly. Evals: t06 nested-app-directory bootstrap case. Adjacent same-plugin callers carrying the same repo-root assumption — `mobilize-chores`' `plan.md` path, `dispatch_envelope.py` — named out of scope per the ticket, not touched. Fresh-context `harness:skill-checker` FLOOR pass per plugin-authoring.md's semantic-edit invariant: findings applied.
v2.28.43 · 2026-08-23 · fixes #901: `fleet-rules` follow-up on the #896 line-budget trim debt. Section 3's no-op-silence rule now cites the needs-input arm's own batching channel (`held-items.md`/idr-0011, already documented lower in the same section) — closes the dropped cross-reference. Contested routing eval t22 ("my subagent never gets selected — is the wiring between capabilities wrong") re-judged against the current descriptions (drifted again in #902/#904): `wiring-checker`'s description now explicitly owns "why won't this route"/"my subagent never gets selected" phrasing, so t22 flips trigger→no-trigger and the frontmatter description gains a reciprocal "NOT a live wiring failure (wiring-checker)" fence, closing the fence both ways. Description re-budgeted back to 700 chars (was pushed to 858 by the new fence; trimmed elsewhere to offset, per skill-checker finding). Fresh-context `harness:skill-checker` FLOOR pass per plugin-authoring.md's semantic-edit invariant: 1 MAJOR (description-budget regression) + 1 MINOR (missing eval comment) both fixed.
v2.28.42 · 2026-08-23 · fixes #906: `fleet-connect` step 1 gains a fleet.json resolution ladder — nearest `.claude/ops/fleet.json` walking cwd-upward to the repo root, first hit wins; a nearer file, once resolved, shadows a farther one entirely, and the resolved path is named in the step-5 report (live repro adiav2/adia-pay-0e: a stale repo-root copy with a null-`agent_name` row shadowed the app-scoped record's live resolved marshal, producing a false "recorded-but-unresolvable" stop). `fleet-manifest-schema.md` gains the same Location-and-resolution ladder note — app-scoped records are a ruled reality per the app-scoped bootstrap re-homing ruling (Kim 2026-08-23), not drift. Evals: t06 nested-fleet.json case. Fresh-context `harness:skill-checker` FLOOR pass per plugin-authoring.md's semantic-edit invariant.
v2.28.41 · 2026-08-23 · closes #903: new command skill `fleet-connect` (ticket named it `connect-marshal`; renamed — `marshal` is a reserved RoleLex token, disjoint from ObjectVocab by ADR-0015 D3, so no nominal skill name can ever contain it; `connect` added to VerbLex in the workspace-root `naming.manifest.json`, giving the reverse-wrapper object-verb production `fleet` + `connect`, G12-clean) — the Terminal-A side handshake wiring a human working session to an already-live `{repo}-marshal`: reads `fleet.json`'s latest `agent`-role row, stops naming `/fleet-bootstrap` if none is live, confirms liveness via `ListAgents` (the one sanctioned use), `SendMessage`s a handshake requesting a status ack + address roster, then adopts the session's fleet-shaped-ask-forwarding posture for the rest of the run (the session-side twin of `fleet-rules` Section 7's marshal-side routing sentence). Failure branches cover no live marshal, a live-but-unresolved-address legacy row, and a stale joined-but-unlisted seat (report + suggest takeover, never reap). Folds in #902's own leftover gap: `team-scaffolding` Phase 2's manual-join path now resolves and writes the session's real `ListAgents` address as `agent_name` too (mirroring `fleet-bootstrap` Phase 1 step 5), instead of writing `null` unconditionally — `fleet-manifest-schema.md`'s `agent_name` field doc updated from "not yet universal" to closed. Command-only (`disable-model-invocation: true`), so excluded from `check-routing`'s blind-judge fan-out by design (Phase 3's own exclusion rule); static `eval_check.py` gate clean. Fresh-context `harness:skill-checker` FLOOR pass on both `fleet-connect` and the edited `team-scaffolding`: both PASS, findings applied (liveness now keyed on `action` not `agent_name`-null-inference; a distinct "recorded but unresolved" failure branch added; the schema doc's stale paragraph closed).
...
(not yet wired). `fleet-rules`' own frontmatter description gains a routing sentence (Kim's live
ruling): a non-marshal session with a live `{repo}-marshal` forwards a fleet-shaped ask via
`SendMessage` rather than applying the skill itself, mirrored into Section 7's body; `evals.json`
gains t26 covering it. Scoped blind-judge re-run of fleet-rules' full 46-case suite against the
teamwork menu: zero regressions from the description trim (all three cut quoted phrases still
routed correctly); 3 pre-existing dead cases unrelated to this change (t17/t18 already documented
noise, t22 newly contested and left unresolved rather than chased). PATCH: prompt-body + routing
description wording only, no schema/field change shipped.
...
v2.28.34 · 2026-08-22 · closes #878: `cross_repo_coordination` moves from flat `repos[]`/`marshal_roles[]` to `participants[]` objects `{repo, app?, role}` (Kim's ruling, option a) — signup is adiav2's app-scoped seat, not a 5th repo; adds the two-holder-per-seat liveness rule (collision now scoped to `(role, app)`); old shape retired not aliased, this repo's `fleet.json` + `fleet-bootstrap` Phase 0 migrated in the same wave.
v2.28.33 · 2026-08-22 · fleet-bootstrap Phase 6 closes with an address roster (Kim's ruling): every seat as `@{repo}-<role>` with its live addressability read from the run's own outcomes — four-way, classified from fleet.json's latest row mode+action: addressable (live named dispatch or live terminal) / not live—returned dispatch (product's synchronous call) / not messageable—subprocess (the claude -p reviewer, log path as pointer) / not live—bind it; `@` is a display sigil, SendMessage targets the bare name — so a human sees who can be messaged by name right now and never a name that silently drops messages.
v2.28.32 · 2026-08-22 · session-ladder retier (Kim's ruling, aligned with the same-day agent retier): canonical fleet seats now agent sonnet+high (also fixes the #313 fork price leak), reviewer sonnet+high, planner fable+medium (unchanged), product sonnet+xhigh; NEW tier-reconcile-on-every-bind step wired into fleet-bootstrap Phase 0 + team-scaffolding Phase 1 (schema doc §Tier reconcile, canonical) — a stale-ladder fleet.json is now flagged with a fix-or-keep question instead of silently trusted (root cause: seats block was copied at seed time then treated read-only forever).
...
v2.28.12 · 2026-08-19 · closes #758: `teamwork/scripts/dispatch_envelope.py` — the dispatcher (mobilize-chores step 5, /build-feature, a marshal) runs it once before dispatching and hands the sealed prompt a pre-computed JSON envelope (version slot, decided branch, a pre-made shallow scratch clone with that branch checked out, pin_check, an open-PR/claimed-no-PR collision look); dispatch-ticket Phase 3 gains the "envelope present" verify-not-re-derive branch — lever 2 of Kim's turnaround review, fixed setup was measured at 15-25% of dispatch time (#750/#751).
...
instructions with no agent name to resolve: `$ARGUMENTS` seals whole as the charter (one inline
clarifying question on an unresolved deictic, never a guess), dispatches ONE unnamed synchronous
`general-purpose` `Agent`-tool call (`model: sonnet` pinned per the ad-hoc-dispatch doctrine
unless the charter earns more), relays the result verbatim-in-substance. Grammar-clean per
ADR-0020 D3 (`sub-` reserved head + `task` already in `object_vocab`, `validate.py --scope
grammar` 0 errors) — no manifest edit owed. Reciprocal fences closed in both siblings: `sub-agent`
and `fork-agent` each gain one NOT-clause plus one negative eval case naming `sub-task`.
Fresh-context `harness:skill-checker` pass (FLOOR depth) on the new skill and both reciprocal
edits per `plugin-authoring.md`'s semantic-edit invariant.
v2.28.5 · 2026-08-19 · closes #733 (one of three pre-existing routing residuals from #720's full-suite re-judges, resumed post-#732-merge): `fleet-rules` t15 ("review my frontmatter") reassigned trigger→no-trigger — #720's own re-judge found it unanimous 3-of-3 none/dead, a different shape than t17/t18's contested-then-confirmed noise (which resolved back to trigger). Bare "frontmatter" with no system/wiring qualifier reads as ONE artifact's own frontmatter, which this skill's existing fence already cedes away ("NOT one agent — its preloads, frontmatter, or return block"); same structural class as n07/n16 (true owner agent-writing-rules, cross-plugin, off a teamwork-scoped menu), manifesting as dead rather than leak since the fence correctly declines rather than forcing a wrong pick. No description change — the fence is doing its job; eval-only fix. Rebased onto `origin/main` post-#732-merge (`--onto` past the now-redundant `bug/720` commits, whose own content is already the v2.28.4 line below); this branch's original 2.28.5 claim (taken before #732 merged, to avoid racing #732's own then-unmerged 2.28.4) is confirmed still the next-free slot off main's post-merge v2.28.4 — unchanged.
...
`fleet-rules` Section 4) and reports one consolidated `green: #A #B / pending: #C / failed: #D`
line per invocation, replacing per-PR two-feed-event polling. Classification is explicit on a
mixed PR: a failed check-run always wins over a pending one (a failure won't self-heal by
waiting); an unrecognized future conclusion value fails closed rather than reading as safe.
`selftest` (stdlib-only, zero network) proves `classify_pr`/`build_report`'s green/pending/failed
buckets incl. the mixed failed+pending and fail-closed-unrecognized-conclusion negative controls,
and `parse_args`' #188-class controls.
...
and its experimental flag — an unconditional hold between the branch push and the PR open, held
until a durable accept-marker comment naming the pushed branch's HEAD SHA lands from the marshal
(`fleet-rules` §7); no live marshal reachable fails closed (`write-gate-blocked`, a reported
outcome, never a failure — the pushed branch and the Phase 3 claim both stay as-is for a later
run to resume). Composes strictly on top of ADR-0012's quick-build auto-merge (stage 2b), never
bypassing it: 2a's accept marker answers "accepted to land," QB5's critic-green conjunct answers
"is it good," and 2b's eight-conjunct evaluation never begins before an open PR exists. New
`.claude/docs/lld/lld-0022-fleet-native-write-gate.md` resolves the four marshal-lean design
forks (hold mechanics, acceptance-protocol shape, unconditional-vs-grant-only, `in-flight` label
interaction) with independent reasoning. New payload-layer fixture `references/
write-gate-dry-run.md` (two greppable dry-run traces, gh#686's own agent-verifiability
criterion) plus `references/plan-approval-write-gate.md` (2a's full mechanics). Two more F6
splits — `references/gate-run-time-budget.md` and `references/quick-build-auto-merge-predicate.md`
— close the SKILL.md body back under its own line budget once 2a landed inline (pre-existing
content, unrelated to this ticket's own scope, relocated only to make room). Fresh-context
`docs:doc-checker` (fix-then-ship: 1 major fixture-count contradiction + minors, all fixed) and
`harness:skill-checker` (fix-then-ship: 1 blocker dangling reference + 1 major grep-anchor miss,
both fixed) passes. Disclosed irony: this PR ships under the OLD contract — the gate cannot
retroactively hold the branch that introduces it.
v2.27.2 · 2026-08-18 · closes #692 (teamwork slice, folded with docs/harness siblings in one PR): `recurrence-audit`'s first incident-class seeding round (authorkit home) adds two `LEDGER-CLASS:` tags to this plugin's own existing doctrine citations — `worktree-pin-race | ids: #490, #609` on `dispatch-ticket/references/isolation-ladder.md`'s "Why this ladder exists" section, and `stranded-named-dispatch | ids: #154, #157` on `dispatch-ticket/SKILL.md`'s Phase 4 checker-naming bullet (the `chk667` incident already cited there). Additive citations only — no procedure/behavior change, no frontmatter/description touch, no `evals.json` update owed; `skill_lint.py` clean on both touched files. `release_gate.py teamwork` clean.
v2.27.1 · 2026-08-18 · closes #669 (post-#433 rename-retirement sweep, teamwork slice; authorkit + harness slices same PR): every surviving live `build-lead` handle retired to `build-leader` — the `Map` routing rows for `dispatch-ticket` and `mobilize-chores`, `mobilize-chores`' own frontmatter description plus 13 body/reference sites (a literal token swap; the dispatch verb, contract, and gating are unchanged), `build-feature`'s two seed sites, `fleet-rules`' worktree-precondition worked example (SKILL.md + `references/best-practices.md`), and `parallel-work-rules`' write-guard-identity citation. `mobilize-chores` carries no `evals/evals.json` (command-only, `disable-model-invocation: true`, no suite by design per its siblings' own disclosed convention) so the description edit has no suite to update; `/check-routing` re-run on the plugin's six model-invocable suites came back clean (dispatch-ticket 11/11, grill-the-ask 18/18, fleet-rules 44/45 with one pre-existing accepted leak unrelated to this edit, loop-rules 25/25, parallel-work-rules 23/23 with one single-judge contested miss on an unrelated fan-out case) — no new steal/leak attributable to this change. Pure mechanical find/replace of a retired handle name across prose, no surrounding instructional wording rewritten; no fresh-context checker dispatched per `plugin-authoring.md`'s semantic-vs-mechanical calibration. Historical records (CHANGELOG, ADRs 0010/0012/0013/0015/0016, LLDs, spec amendment notes, ops reports, README version-ledger lines, `.refactor-attic/`, dated eval-audit records) deliberately left untouched per mutability policy.
