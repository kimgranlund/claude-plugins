...
| `agents/team-lead` | Subagent | dispatch-only | The apex seat: chain-of-command, dispatch order, the review gate between phases, the discovered-reality escalation loop, rollups to the host |
...
v2.16.0 · 2026-08-16 · fleet bootstrap two-level design (#410, #404 addendum 3): `team-scaffolding` gains planner/reviewer standing-order self-checks (warn/notice, never block) and seeds `.claude/ops/fleet.json` on join; new `fleet-bootstrap` command hard-gates a full cold start on human ratification of the intent layer, then spawns an explicit reviewer/planner spawn-list as background seats. `fleet` + `bootstrap` registered in naming.manifest.json. `skill-checker`/`wording-checker` PASS after fixes. Details in #410.
