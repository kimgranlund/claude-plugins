...
v2.13.2 · 2026-08-16 · the no-nested-wait rule encoded structurally (issue #310, four measured `build-lead` stall incidents #257/#282/#269/#280): `dispatch-ticket`'s procedure text gains a standing rule — a seat already running as a nested dispatch (`build-lead`, `/lead-build`) does its Phase 3 isolate work and Phase 4 `small` build inline, never via a further nested `context: fork` skill or NAMED `Agent`-tool dispatch it then waits on for a callback that structurally routes to the ROOT session instead (the fork-from-agent finding); Phase 4's "one sealed fork/agent" clause narrowed to top-level hosts only, Phase 2's task dispatch pinned UNNAMED. `build-lead`'s own agent body states the same rule explicitly (cold-start-language precedent: every affected agent restates a standing constraint in its own body, never assumed from platform behavior). Matching harness `agent-writing-rules` Failure-catalog row (own bump)
