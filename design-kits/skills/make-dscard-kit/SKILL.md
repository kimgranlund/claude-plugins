...
description: >
  ...
  (make-design-kit); NOT for grading an export you didn't author
  (design-kit-checker).
...
SKILL for a brand, per `design-md-rules`, this plugin's format ground truth),
...
`design-md-rules`'s corpus (same plugin) — consult it rather than restating it.
...
2. **Capture the Root Brand Architecture** — the six slots from `design-md-rules`'s
...
| `design-md-rules` (same plugin) | First-party format ground truth: DESIGN.md anatomy + openness doctrine, the six-slot Root Brand Architecture, @dsCard tag syntax — steps 1–2 and 5 lean on its corpus |
