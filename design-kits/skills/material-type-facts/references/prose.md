...
- Blockquote / pull-quote / standfirst: `.md-sys-typescale-lead-{level}` — the dedicated **lead** voice
  (its own weight/tracking, own leading); the color/border come from material-color-facts. (For
...
Links keep the surrounding body voice/level — only the COLOR changes (material-color-facts:
...
- Code block: `.md-sys-typescale-body-mono-md` with `-line` for comfortable multi-line leading. The
  surface/color come from material-color-facts; the type here is only the mono-role voice +
  level.
