#!/usr/bin/env node
/**
 * ui-probe.mjs — the check-whole-ui "measured, not computed" probe.
 *
 ...
 *
 *   <screen>.focus.json — check-focus card. Tabbable candidates (interactive selectors +
 ...
 *
 *   <screen>.budget.json — check-speed card. LCP + CLS from buffered PerformanceObservers during
 ...
 *
 *   <screen>.i18n.json — check-translations card. has_lang / has_dir true iff the document carries the
 ...
 * cards through the REAL checkers (check-colors/check-focus, resolved as sibling skills) and
...
/** Assemble a check-focus card from a raw focus sweep (+ per-scheme ring contrasts). */
...
/** Assemble a check-speed budget card. inp_ms/tbt_ms are OMITTED (unmeasurable without
...
/** Assemble an check-translations card. hardcoded_strings is a static concern — [] with a note. */
...
async function selftest(noBrowser) {
  ...
  try {
    ...
    const checkers = [
      ...
      { py: path.join(skillsDir, 'check-focus', 'scripts', 'focus-check.py'),
