...
token set is owned by `material-motion-facts` (design plugin); one cross-reference
...
consult `material-motion-facts` (design plugin) where installed; historical anchor
