...
# motion-rules — the motion world model
...
- **This skill answers; it does not generate.** No keyframes, no component code, no token files —
  cite the value, hand the making off: build the animated component → [[make-component]];
  verify perceived latency, skeleton-vs-spinner, CWV budgets → [[check-speed]]; drive the change
