...
5. Where the change affects load or interaction cost, run this plugin's `check-speed` dimensions
   against it (or a performance trace via whatever tooling the project has); where it affects
   focus, keyboard, or contrast, run `check-focus`'s / `color`'s equivalents. This skill drives
...
- **Scope declaration:** the verdict header names the scope driven — this change's surfaces (the
  default), a single interaction, or "route to [[check-whole-ui]]" when the change's blast radius is
...
- **Findings format:** `file:line — [RULE_ID] finding → fix`; composed dimensions keep their
  owning verifier's IDs (a focus defect found here cites check-focus's rule names).

## Output contract

```
Scope: <this change's surfaces | single interaction | routed to check-whole-ui>
