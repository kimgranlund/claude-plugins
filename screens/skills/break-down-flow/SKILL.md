...
# break-down-flow — read a journey on two crossing axes
...
the task ends and what must be TRUE there). Where [[break-down-layout]] owns *within*-screen, this
...
card; a flow you designed goes to the `flow-checker` agent — the fresh-context critic; generator ≠
critic).

**DESIGN mode's taste gate** (canon: [[break-down-layout]]'s `references/taste-elicitation.md` —
...
timeout-resume persistence). In a [[check-whole-ui]] sweep the inventory's declared flows are the card
...
- **Between-screen only.** One screen's regions and verbs hand DOWN to [[break-down-layout]]; a
  single async step's latency feel to [[check-speed]]; a destructive action's friction calculus to
  [[check-safety]]. This skill owns the states, the verbs between them, and the exit truths.
...
| the `flow-checker` agent | GRADE's fresh-context critic for a flow you designed (generator ≠ critic) |
| [[break-down-layout]] | within-screen space + behavior (the sibling altitude, same two-axis polarity) |
| [[check-whole-ui]] | the set-scoped sweep that composes this per declared flow |
| [[ui-pattern-facts]] | within-screen state grammar (the pentad) + page templates a stage renders as |
| [[check-speed]] · [[check-safety]] | a step's latency feel · a destructive step's friction — hand off, don't re-derive |
