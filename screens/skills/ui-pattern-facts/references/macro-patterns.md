...
`break-down-layout`) — this file carries the **page templates** that live inside a shell. Each entry: anatomy → when it fits → the failure it invites.
...
break-down-layout); the page-level rule worth
