...
component routes to `make-component`; their interaction invariants route to the verifier family.
...
- **Contract:** the overflow seam is the module's load-bearing part — see
  `make-component/references/composition-patterns.md` for the mechanism.
...
- **Toast:** transient confirmation + undo vehicle (see check-safety); never for errors that need
