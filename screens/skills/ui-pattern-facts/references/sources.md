...
`break-down-layout`); interaction invariants in the verifier family; component realization in
`make-component`; the spacing arithmetic behind `container-patterns.md`'s nested-boundary note in
`size-and-shape-rules`. Claims here that drift from those owners should be fixed HERE — they own
their layers.
