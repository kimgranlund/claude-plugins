...
description: >-
  ...
  (llm:chat-harness-instructions-and-guardrails); NOT for Resident authoring/orchestration patterns
...
`llm:chat-harness-instructions-and-guardrails` — an Ephemeral-agent-scoped skill — as if they were
...
`llm:chat-harness-instructions-and-guardrails` and its sibling packs already document in depth for
...
| "Which instruction layer wins, how do I gate a risky action behind confirmation" | Ephemeral | `llm:chat-harness-instructions-and-guardrails` |
| "How does my chatbot remember things across turns/sessions" | Ephemeral | `llm:chat-harness-knowledge-and-memory` |
| "How do my chat-agent's skills/tools get selected at runtime" | Ephemeral | `llm:chat-harness-skills-and-routing` |
...
`llm:chat-harness-instructions-and-guardrails`'s own `references/sources.md` for a worked example
