---
name: chat-harness-skills-and-routing
description: >-
  ...
  (chat-harness-instructions-and-guardrails); NOT for multi-agent composition
  (chat-harness-orchestration-and-workflows); NOT for a repo's skill-authoring structural standard
  (skill-authoring-standards).
disable-model-invocation: false
user-invocable: false
---

# chat-harness-skills-and-routing — discoverable capabilities and how requests find them
...
- **Layer standing instructions and enforced guardrails across a whole harness** (CLAUDE.md
  structure, hooks, what belongs where) → [[chat-harness-instructions-and-guardrails]] (the sibling
  pack in this plugin family).
- **Compose multiple skills, tools, or subagents into a multi-step workflow** (orchestration,
  hand-offs, review gates between seats) → [[chat-harness-orchestration-and-workflows]] (the
...
- **Deferring a large TOOL catalog's own schema-loading** (the analogous "load on demand" shape,
  a different mechanism than a skill's own model-invoked routing) → [[chat-harness-tools-resources-and-services]]
...
- **Persisting a fact or preference across sessions** (memory, not routing a live request to a
  capability) → [[chat-harness-knowledge-and-memory]] (the sibling pack in this plugin family).
- **Measuring routing accuracy AFTER the fact, over repeated real runs** (as opposed to the
  routing mechanism itself) → [[chat-harness-observability]] (the sibling pack in this plugin
