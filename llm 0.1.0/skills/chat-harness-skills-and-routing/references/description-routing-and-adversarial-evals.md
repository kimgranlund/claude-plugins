...
**Worked instance:** this very plugin's sibling pack, `llm-provider-gateway`, states its own
negative-selection rule verbatim in `scripts/routing-corpus.json`'s `_note` field: "Negatives
drawn adversarially from the dangerous near-neighbors: `llm-jsonl-streaming` (the sibling pack —
...
**Worked instance:** `llm-provider-gateway/scripts/routing-corpus.json` carries `{skill, _note,
...
([[chat-harness-orchestration-and-workflows]]).
