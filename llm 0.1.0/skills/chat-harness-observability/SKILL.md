---
name: chat-harness-observability
description: >-
  ...
  agents that DO the work (chat-harness-orchestration-and-workflows); NOT the routing MECHANISM
  (chat-harness-skills-and-routing); NOT implementing or building any of this — a logging
  pipeline, eval-suite runner, or notification webhook to write is the project's own build seat.
disable-model-invocation: false
user-invocable: false
---

# chat-harness-observability — making a harness's own behavior legible
...
- **Compose or orchestrate the agents that DO the work being observed** (a build team, a
  multi-agent workflow) → [[chat-harness-orchestration-and-workflows]] (the sibling pack in this
...
- **The routing MECHANISM itself** — how a skill actually gets selected at discovery time, as
  opposed to how its accuracy is measured after the fact → [[chat-harness-skills-and-routing]] (the
  sibling pack in this plugin).
- **The rule a hook ENFORCES** (blocking or transforming a tool call before it runs) →
  [[chat-harness-instructions-and-guardrails]] (the sibling pack in this plugin) — this pack only
