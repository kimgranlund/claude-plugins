---
name: chat-harness-knowledge-and-memory
description: >-
  ...
  routing a request (chat-harness-skills-and-routing); NOT for a streaming wire format
  (llm-jsonl-streaming).
disable-model-invocation: false
user-invocable: false
---

# chat-harness-knowledge-and-memory — citable knowledge, durable memory
...
- **Routing a live request to the right capability** (should this be a skill vs. hardcoded, why a
  ...
  fact across sessions → [[chat-harness-skills-and-routing]] (the sibling pack in this plugin
  family).
- **The wire format of a model's OWN live streaming output** (SSE framing, JSONL structured
  ...
  [[llm-jsonl-streaming]] (the sibling pack in this plugin).
- **A vendor-swap or trust-boundary concern for calling an LLM** → [[llm-provider-gateway]] (the
