...
- MCP is an open, versioned specification (modelcontextprotocol.io) independent of any single
  ...
  discipline `llm-provider-gateway` applies to its own Vite-specific platform facts.

## Routed siblings — cited, never duplicated

- **[[llm-provider-gateway]]** (this plugin) — the fully worked instance of a18 (external-service
...
- **[[chat-harness-skills-and-routing]]** — a skill library's own model-invoked routing/discovery
...
- **[[chat-harness-knowledge-and-memory]]** — persisting facts/preferences/state across turns or
  sessions; a distinct extension surface from a tool, a resource, or a service call and not
  covered by this pack at all.
- **[[llm-jsonl-streaming]]** (this plugin) — the wire format a tool's or service's result streams
...
[[llm-provider-gateway]]), a skill library's own routing (route to
[[chat-harness-skills-and-routing]]), or cross-turn memory (route to
[[chat-harness-knowledge-and-memory]]) — when this pack and a routed sibling could both plausibly
