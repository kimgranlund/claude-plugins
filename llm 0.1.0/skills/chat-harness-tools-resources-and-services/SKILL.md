---
name: chat-harness-tools-resources-and-services
description: >-
  ...
  LLM-provider specifics route to llm-provider-gateway. Grounded in this harness's live
  ToolSearch mechanics + the MCP spec. Answers from a cited corpus, no build. NOT the
  LLM-provider pattern (llm-provider-gateway); NOT skill routing/memory
  (chat-harness-skills-and-routing / chat-harness-knowledge-and-memory); NOT a wire format
  (llm-jsonl-streaming); NOT implementing any of this.
disable-model-invocation: false
user-invocable: false
---

# chat-harness-tools-resources-and-services — reaching outside the context window
...
4. For a18 (external services) specifically: state the general principle in one or two sentences,
   ...
   `llm-provider-gateway` already owns in full.
...
`llm-provider-gateway` already teaches instead of routing to it.
...
- **The swappable LLM-provider adapter, its registry/trust-boundary validation, its dev-proxy, or
  its stateless turn model** → [[llm-provider-gateway]] — the fully worked instance of a18's
...
- **A skill's own model-invoked routing/discovery** (how a large skill LIBRARY, as opposed to a
  tool catalog, decides what loads into context) → [[chat-harness-skills-and-routing]] — a
...
- **Persisting facts, preferences, or state across turns/sessions** (memory) →
  [[chat-harness-knowledge-and-memory]] — memory is neither a callable tool, a static resource,
...
- **The wire format a tool's or resource's result is streamed over** (SSE framing,
  validate-then-stream) → [[llm-jsonl-streaming]] (the sibling pack in this plugin) — a transport
