...
| Cross-platform set — two or more of the above from one canonical core | hub: `${CLAUDE_PLUGIN_ROOT}/skills/make-design-system/references/rubric.md` (H1–H7) + `references/shared-doctrines.md` | each member's row above, plus carrier equality across exports (same sRGB triple ±1/255 per channel) |
...
4. **Standing-rules sweep** (every platform) — per
   `${CLAUDE_PLUGIN_ROOT}/skills/make-design-system/references/shared-doctrines.md` §5–§6, the statement of
...
- **Grade only; the maker fixes.** Token-level fixes route to token-builder; export repairs route
  to the owning make-design-system sibling; one preview component's internals hand to
