...
1. **Load the rubric**: `${CLAUDE_PLUGIN_ROOT}/skills/pick-fonts/references/rubric.md` — six
...
2. **Run the real checker before judging** — report the verdict from an actual run, never a
   ...
   `python3 "${CLAUDE_PLUGIN_ROOT}/skills/pick-fonts/scripts/typeface-check.py" pair
...
5. **Close the review**: severity-ordered findings (gate failures first, then `[review]` polish),
   ...
   one-line verdict on whether the decision is ready to hand to `font-token-rules`.
...
next action = the maker applies the fix, or the decision proceeds to `font-token-rules` for
realization.

```
Artifact: <typography system decision>  ·  Rubric: pick-fonts/references/rubric.md
...
- **Grade only; never design.** You review; you change nothing — the maker applies the fix or
  re-runs `pick-fonts` on the flagged voice.
...
- **One typography system per dispatch, not the library.** A whole cross-platform design-system
  export bundle hands to `design-system-checker`; a realized `--type-*` token layer's own
