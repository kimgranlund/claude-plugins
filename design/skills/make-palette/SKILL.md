...
description: >-
  ...
  realizing token layers in a repo (token-builder); NOT ticket-capture fields for a
  token/palette-seed (token-feature-intake-rules).
disable-model-invocation: false
user-invocable: true
---

# make-palette — OKLCH ramps & semantic mappings, verified before emit

Generator peer of [[check-colors]]: this skill designs the palette; the verifier proves it. It
...
contract: **design → verify** — nothing finalizes until [[check-colors]] returns a passing
ColorProof.
...
this skill — route to [[check-colors]].
...
7. **Verify handoff.** Hand the candidate — every scheme — to [[check-colors]]: it gates the
...
| Evenness | per-label-unit ΔL (ΔL ÷ Δlabel): adjacent gap-to-gap ratio ≤ 1.5 — [[check-colors]]'s form, checked on every `ramp_build.py` run |
...
| Neutral chroma | ≤ 0.02 on every step — a design margin, deliberately under [[check-colors]]'s 0.06 gate for declared-tinted neutrals; exactly 0 if `neutral: true-grey` |
...
necessary, not sufficient: it proves the geometry, [[check-colors]] proves the palette.
...
a [[check-colors]] run · HSL/HSV ramp generation (perceptually misleading) · shade-by-RGB-scalar
...
| [[check-colors]] | the mandatory verify handoff — contrast gate, (theme × scheme × contrast) sweep, ColorProof |
| [[color-space-facts]] | the perceptual theory these constraints assume (OKLCH, gamut math) |
| [[color-contrast-facts]] | the perceptual theory these constraints assume (APCA/WCAG, CVD) |
| [[color-theory-facts]] | art direction upstream of the anchors — harmony, palette mood/vibe, what a color communicates |
| `token-builder` agent | realizes the verified palette as a token layer in a repo |

**Update:** when [[check-colors]]'s invariants or `ramps/*.json` move, re-derive the numbers
