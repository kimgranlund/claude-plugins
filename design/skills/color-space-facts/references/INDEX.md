...
pack (the 5 historical files split to `physical-color-facts` and `color-perception-facts`); no
...
(contrast standards & CVD), `physical-color-facts` (pigment, print, naming) — plus the earlier
...
naming pickers (color.pizza API, 147colors, ISCC-NBS picker) moved to `physical-color-facts`
§naming; the huevaluechroma.com/handprint.com entries moved with the vision-science and
pigment-data content they front (`color-perception-facts` / `physical-color-facts`); pure
