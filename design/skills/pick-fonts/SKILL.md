...
description: >-
  ...
  font-token-rules realizes it. NOT for color ramps (make-palette); NOT for realizing a decided
  system as tokens (font-token-rules); NOT for type-history knowledge (lettering-facts); NOT for
  Material's typescale (material-type-facts); NOT ticket-capture fields for a typography-seed
  (token-feature-intake-rules).
disable-model-invocation: false
user-invocable: true
---

# pick-fonts — the per-voice font-and-rationale decision

Generator peer of [[font-token-rules]]: this skill DECIDES which typeface fills each voice; the
...
artifact. Its knowledge peer is [[lettering-facts]]: the world-model (why a face reads
...
this text" is not this skill — route to [[font-token-rules]] · "why does this font read neutral"
or "what's the history of X" is not this skill — route to [[lettering-facts]] · "verify this
...
1. **Interpret the territory into a specific point, not a region.** A named reference —
   ...
   (`make-design-system/references/context-potency.md`'s presupposition technique and
...
2. **Per-voice creative decisions.** Typography-tokens' five font-family roles (`display · heading
   ...
   deepens past `font-token-rules/references/font-selection.md`'s fallback heuristic lives in
...
4. **Craft-correctness verification.** Run [`scripts/typeface-check.py`](scripts/typeface-check.py)
   ...
   actual sizes are known from `font-token-rules`' bound scale — the size-jump ratio (≥3× hierarchy
   ...
   cite `lettering-facts/references/accessibility/low-vision.md` and
   `lettering-facts/references/accessibility/dyslexia.md` for why those voices carry the
   floor. If the brief names multi-script needs, confirm every chosen face actually ships the
   needed scripts (`lettering-facts/references/scripts/`) before finalizing.
5. **Handoff.** The output is a concrete per-voice font-and-rationale decision document — five
   ...
   verdict on every pairing — routed to [[font-token-rules]] for REALIZATION as bound `--font-*`/
   `--type-*` custom properties. This skill never emits CSS or token names; font-token-rules
...
| Parameter | Value |
|---|---|
| Metric compatibility tolerance | x-height AND cap-height (when both are known) within ±10% of each other at the same nominal size (ratio ≥ 0.90), else a stated `font-size-adjust` compensation — `lettering-facts/references/techniques/pairing.md` |
...
from font-token-rules' pre-migration font-selection.md, 2 rows from
`lettering-facts/references/voice/neutral-by-design.md`'s Roboto/Arial figures — extend the
...
(the job font-token-rules' `type-check.mjs` does) — it matches the corpus's Python convention for
...
`typeface-check.py` over every same-baseline pairing · doing font-token-rules' job (binding CSS
...
| `make-design-system/references/context-potency.md` | the presupposition / generic-output-clinic doctrine step 1 applies (read-only citation, not this skill's to edit) |
...
every voice's rationale traces to it, and font-token-rules has received the handoff. **NOT done**
