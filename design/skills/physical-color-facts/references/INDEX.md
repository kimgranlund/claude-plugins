...
Curated transcripts, notes, and source material for the `physical-color-facts` pack — pigment
...
sibling packs: `color-space-facts`, `color-perception-facts`, `color-contrast-facts`,
`physical-color-facts`); prior history lives in `color-science-project-files`'s changelog.
...
## Cited from color-perception-facts (straddle files — live there, never duplicated)

Paint-mixing chapters of the David Briggs scrape carry the *pigment-physics* half of subtractive
mixing; the full scrape (all 14 chapters) lives in `color-perception-facts` as one coherent
...
| Chapter (in `color-perception-facts`) | Why this pack cites it |
| --- | --- |
