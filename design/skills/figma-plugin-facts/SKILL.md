...
description: >
  ...
  threw"), styles bound to variables ("setBoundVariableForPaint"), sandbox/VM failures
  ("parses in Node, fails in Figma"), token JSON shapes ("UI3 import", "aliasData"), and
  pluginData/clientStorage. ANSWERS, does not generate. NOT the nonoun color-tokens
  plugin's binder/apply cascade (maintaining-figma-plugins); NOT the DESIGN.md format
  (design-md-rules); NOT --md-sys-* semantics (material-*-facts); NOT the Figma MCP
  tools; NOT the REST API or scene-node work; NOT a Figma Make guidelines/ folder
  (make-figma-make-kit).
