...
# css_build.py's `build(FIXTURE_TOKENS)`, captured 2026-08-18, re-captured 2026-08-19 (#649, lld-0013
# v2 Resolution 10 — the type-doctrine default-binding block is now part of the verbatim output) —
# every line, comments included, nothing trimmed), plus the synthetic body/font rule `make-artifact`'s
# own page assembly phase adds on top of a css_build :root block. Bundled scripts cannot import
# across a plugin boundary (.claude/rules/plugin-authoring.md's hard-boundary rule), so this is a
# literal, dated, cited COPY, not a live import — if css_build.py's emission shape ever changes,
# BOTH this fixture and css_build.py's own selftest (which asserts the same property names from
# the emitter's side) update in the same change, per #662's Acceptance criterion 2 (amended by
# #684, 2026-08-19: zero FAILs plus exactly the one ruled doctrine-font-stack WARN on the
# synthetic body rule's brand token — see the selftest assertion below; re-verified 2026-08-19
# after #649's type-doctrine block insertion — the new default rules resolve ON-doctrine by
# construction, so they add zero findings and the fixture's expected-finding count is unchanged).
# A prior draft of this fixture TRIMMED the generated comments/spacing/radii/mermaid blocks and so
# silently missed a real regression that only the full output exposes (checker finding, #662):
# css_build's own generated mermaid comment used to cite "#662" literally in-line, which
# `#[0-9a-fA-F]{3,8}` reads as a 3-hex-digit color literal outside :root — fixed two ways, at the
# source (the citation was dropped from the emitted comment, `css_build.py`'s MERMAID_REHEME_BLOCK)
# and at the checker (comments are now stripped before the literal scan, `CSS_COMMENT_RE` above) —
# this fixture, now genuinely verbatim, is what would have caught it.
