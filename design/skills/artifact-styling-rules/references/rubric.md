...
| R2 | Token binding | [gate] | Every visual property traces to a role, never a literal | 1: a bare hex/oklch/rgb literal used directly in a non-`:root` rule · 3: every visual property reads a `var(--<short-role>/--text-*/--space-*/--r-*)` (the unprefixed artifact-page short role names, never `--c-<role>` — superseded #662), roles traced to the source system · 5: + a tier-2/`_unbound` need routes to a named gap, never an ad-hoc invented color |
