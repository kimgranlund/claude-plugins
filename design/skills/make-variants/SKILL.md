...
name: make-variants
description: >-
  Publish an Artifact of N variants along declared axes, grounded in the project's design
  system when one exists, each card voted up/down with a note, serializing a live variant-feedback/v1 JSON block pasted back to resume at the same URL. Use when asked to explore variants of a component, show
  variants of X, N takes on a layout, iterate on a design, or run a variant
...
1. **Ground.** Resolve the project's design substrate down the standard ladder, most
   interpretable first: a DESIGN.md/design-system export → a token layer (`tokens.json`,
   `:root` custom properties, a theme file) → framework theme config → existing component
   styles read off siblings. First rung that answers, wins — resolved per question (color can
   ground at one rung, motion at another). Compile the subset the declared axes touch (the
   relevant role ladders + base surface/text pairs, plus a facsimile of the target component's
   markup where one exists) into a grounding snapshot recorded as `{sources, resolved_version,
   hash}` — `references/feedback-schema.md` carries the field. Grounding is DEFAULT-ON: when
   any rung exists it constrains every variant; unconstrained exploration happens only when
   the user asks for it or no rung exists — either way the artifact header SAYS which
   (silent ungrounded output styled to look on-brand is this step's failure mode).
2. **Declare axes.** From the request (or by asking once if the brief names no axes), fix 2-4
...
3. **Derive stable ids.** Each variant's id joins its chosen axis values in the axes' declared
   order (`compact-sharp-quiet`); grounded axes draw their values from the snapshot's scales,
   never invented ones — `references/feedback-schema.md` is this id scheme's contract
   and the full serialization shape; read it before building the page.
4. **Build ONE artifact.** The grounding snapshot's tokens inline ONCE at page level as CSS
   custom properties (the artifact CSP blocks every external source — values are copied in,
   never linked); cards override only the axis values they vary. Every variant card renders in its own namespaced style scope (a
...
5. **Serialize the feedback block.** The page keeps a live `variant-feedback/v1` JSON block
...
6. **Republish to the same path.** The artifact's file path is fixed at round 1; every later
...
7. **Resume mode.** A pasted blob whose first top-level key is literally
   ...
   held-but-uncited) and republish round `round + 1` to the same path from step 6. When the blob
   carries `grounding`, re-hash the pinned sources first — a hash match reuses the snapshot so anchors stay comparable;
   a mismatch is surfaced as a question (re-ground or hold the pin), never absorbed silently.
8. **Terminate.** A "pick" (a named winning id) or an all-up round is the stopping predicate —
...
- Grounding sources changed between rounds (hash mismatch on resume) → ask: re-ground on the
  new substrate or hold the pinned snapshot; never silently restyle an anchored card.
...
themes, the header states the grounding source (or that none exists), the JSON block live-updates and copies, and a pasted `variant-feedback/v1` blob
...
| `artifact-capabilities` (soft, global) | the republish-to-same-URL mechanism step 6 depends on |
