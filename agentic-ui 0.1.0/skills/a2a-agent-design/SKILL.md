...
description: >-
  ...
  proving two built agents are isolated (a2a-isolation-verify); NOT for the payload an agent RENDERS
  (a2ui-* siblings); NOT for corpus records (a2a-training-corpus).
disable-model-invocation: false
user-invocable: true
---

# a2a-agent-design — build an agent that speaks A2A
...
- Version pin stated explicitly (see a2a-protocol's versioning axis — the method names are
  pin-specific and wire-breaking across 0.3.0/1.0.x).
- If the agent will face untrusted peers: hand off to `a2a-isolation-verify` for the provenance/
