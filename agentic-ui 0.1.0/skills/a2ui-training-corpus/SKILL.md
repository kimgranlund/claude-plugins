...
# a2ui-training-corpus — the corpus subsystem world model
...
3. Check the boundary before answering: if the ask turned into the wire shape a payload conforms to,
   it was `a2ui-protocol`'s; catalog rows/coverage, `a2ui-catalog-design`'s; the live demo's use of
   `retrieve()`, `a2ui-conversational-agent`'s. Route it to the owner rather than answering past the seam.
...
- **Sibling knowledge packs** (answer their asks, don't answer past the seam): `a2ui-protocol` (the wire
  shape a record's `a2uiOutput` conforms to), `a2ui-catalog-design` (catalog rows + coverage the tier-1
  check validates against), `a2ui-conversational-agent` (the live demo that CALLS `retrieve()`).
