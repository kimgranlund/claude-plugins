...
Catalog authoring / coverage policy / type→widget resolution → the **`a2ui-catalog-design`** pack
(live; over `a2ui-catalog.spec.md` + the a2ui-builder seat) · the live agent's session/turn/provider
model → the **`a2ui-conversational-agent`** pack (live; over `a2ui-live-agent.spec.md`) · corpus
record format / admission / retrieval → the **`a2ui-training-corpus`** pack (live; over
`a2ui-training-corpus.spec.md`) · transport interop (AG-UI/A2A/MCP) →
