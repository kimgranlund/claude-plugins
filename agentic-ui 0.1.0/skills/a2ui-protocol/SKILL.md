...
# a2ui-protocol — the wire model + renderer world model
...
- **Sibling knowledge packs — route a question in their territory to them:**
  - Catalog authoring / coverage policy / type→widget resolution → the **`a2ui-catalog-design`**
    pack (live).
  - The live demo's session / turn / provider / transport model → the **`a2ui-conversational-agent`**
    pack (live).
  - Corpus record format / admission / retrieval → the **`a2ui-training-corpus`** pack (live).
