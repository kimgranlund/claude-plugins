---
name: material-design-token-semantics
description: >-
...
# material-design-token-semantics — the token glossary
...
3. **Route output work at the boundary**: the binding grammar, utility classes, pairing law, and full
   recipes live in the matching consumption skill — `material-design-color-tokens` /
   `material-design-typography-tokens` / `material-design-geometry-tokens`; this pack never teaches
...
`knowledge-forge` (axis decomposition, grounded content, index discipline where the corpus grows past a
