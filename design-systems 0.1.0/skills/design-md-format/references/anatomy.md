...
- Companion files are optional, not required: a `tokens.json` machine carrier and a README receipt may ride alongside (Ultimate Tokens ships both), but **the DESIGN.md alone must be sufficient**.
