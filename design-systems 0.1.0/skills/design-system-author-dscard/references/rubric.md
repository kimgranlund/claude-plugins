...
|---|---|---|---|---|
| B1 | Mechanical gates | [gate] | `bundle_gates.py` exit 0: contrast both schemes, scheme parity, carrier equality ±1/255, preview self-containment, reference resolution, section grammar, on-partner coverage, relative leading/tracking (any `lineHeight`/`letterSpacing` in px, in any carrier — unitless/em/% only), pill/chip padding consistency across previews (G9 — one value bundle-wide; the value itself being below the spacing scale is a sanctioned exception, `gates.md`, not a violation) | 1: any FAIL line · 3: exit 0 with unexplained WARNs · 5: exit 0, every WARN/DIVERGENCE explained in the receipt |
