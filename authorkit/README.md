...
| `skills/estate-audit` | Reference skill | model-only | The audit-family index (issue #293/#272) — no procedure of its own; names the six instruments (naming/bloat/attention/pattern/doctrine/orchestration) and the `instrument` parameter `agents/estate-audit-agent` takes. The backing skill for that agent's primary ADR-0011 naming production |
| `skills/repo-audit` | Command skill | both (`/repo-audit`) | The full-battery counterpart to `estate-audit` — fans out all six audit instruments plus, where installed, harness's cross-plugin axes, into one verdict-first roll-up. Structurally read-only |
| `agents/estate-audit-agent` | Subagent | dispatch-only | Batch estate-audit sweeps across N estates/plugins/corpuses in an isolated context, parameterized by one `instrument` (naming/bloat/attention/pattern/doctrine/orchestration), one aggregated report. Replaces `naming-audit-agent`/`bloat-audit-agent`/`attention-audit-agent` (issue #293, executing #272's checker-seat consolidation ruling) |
| `skills/orchestration-audit` | Command skill | both | Sweeps an estate for orchestration-archetype drift against `teamwork:fleet-rules`' eight per-archetype rubrics (A1..A8, issue #666) — runs ONLY the mechanizable slice (six criteria: X-R3 review-coverage existence, A3-R2 durable-channel reconciliation, A4-R1 roster rows, A5-R3 resume-path presence, A6-R2 grant-literal presence, A7-R4 workflows/*.js syntax lint), reports every judgment/not-built criterion "queued, not built" per its rubric-named owning checker. Read-only |
| `skills/orchestration-audit/scripts/audit.py` | Script | invoked by orchestration-audit | Splits a `workflows/*.js` file at the Workflow-tool loader's own `export const meta = {…}` boundary (comment- and string-literal-aware brace balance) and `node --check`s each half under the grammar it actually runs under (`.mjs` for meta, `.cjs` — never `.js`, to avoid Node's module-auto-detection false-passing a real defect — for the async-wrapped body); reconciles `.claude/ops/fleet.json`'s `live_state.joined` against `fleet-roster.md` rows. `selftest` mode proves real `node --check` positive/negative controls incl. a reconstruction of the historical #529 defect |
...
| `skills/spend-audit` | Command skill | both | Instruments idr-0010's estate economy claim — a per-firing, per-archetype token-spend ledger at `.claude/ops/spend-ledger.csv` (attention-trend.csv-shaped, append-only). Appends one validated row per sweep/build firing (tokens, provenance, outcome, verdict, archetype), re-validates an existing ledger standalone, runs a Collector/backfill procedure plus a per-class worth-firing Audit render, and emits a measured per-archetype cost-multiplier table vs the A1 solo baseline. Read-only over doctrine/gates; reports and appends only |
| `skills/host-audit` | Command skill | both | Dev-host performance audit: probe script (host_probe.mjs) measures the machine (CPU/RAM/disk/process pressure), remedies reference maps findings to fixes with warned/destructive tiers |
| `skills/spend-audit/scripts/validate.py` | Script | invoked by spend-audit | The schema owner: `HEADER` (incl. gh#673's `archetype` column), the closed enums (incl. `ARCHETYPES`/`ARCHETYPE_NAMES`), `OUTCOME_CLASSES`, `DATE_RE`, `validate_row`/`validate_file`. Deterministic, stdlib-only. `selftest` mode proves clean/foreign-header/per-field/cross-field/tokens-shape/ref-shape/archetype-shape/duplicate-key-WARN/no-blended-column/missing-file counters bite |
| `skills/spend-audit/scripts/trend.py` | Script | invoked by spend-audit | Appends exactly one validated row to `spend-ledger.csv` — imports `validate.py` as a sibling module so the schema exists in one file. Validates BEFORE writing: a malformed row or a foreign-header file is refused outright, nothing written. `--archetype` is REQUIRED (gh#673), same tier as `--outcome`/`--verdict`. `selftest` mode proves append semantics, the write-refusal negative control (byte-identical file), the schema-mismatch path, the archetype-required usage error, and `--dry-run` |
| `skills/spend-audit/scripts/archetype_gradient.py` | Script | invoked by spend-audit | New (gh#673). Reads the ledger and emits a measured per-archetype token-cost multiplier table vs the A1 baseline, per outcome class (`pr-shipped`/`record-minted`, defined against the existing `outcome` enum). A cell computes only with ≥1 `measured` row on both sides, else honest `UNMEASURED` — never guessed. Wall-clock is not instrumented (a stated non-goal); gh#265's own 1.92x/3.6x anchor is cited textually, never computed from ledger rows. `selftest` mode proves no-ledger-yet, all-UNMEASURED, a real computed ratio, per-class isolation, the measured-only reverse control, and the anchor-is-external reverse control |
...
disallowedTools (convention), with kind/performs/autonomous_write/context/author/created/
last_updated/review_after marked authorkit-internal-only dogfood, not estate-wide (citing
ADR-0025). Both fix stale ADR-0011 §6.1/§8 restatement. Reference-doc fix only, no grammar/schema
change.
...
`naming.manifest.json` governed it one or more levels up — recommending a plugin-local
manifest seed that would have forked the estate lexicon. `main()` now falls back once to
`find_repo_root`'s existing single `git rev-parse --show-toplevel` call (no directory walk —
that was tried and rejected for a DIFFERENT case, hook-mode target derivation, per
`find_repo_root`'s own docstring) and probes the git toplevel for a manifest before refusing;
`--manifest` still overrides every probe. Repro from the ticket (`--target brand-design` from
the workspace root) now resolves governed. Selftest gains 3 new fixtures (governed via
toplevel fallback + `--manifest` override regression, and a negative control proving no
manifest anywhere still exits 2 naming all three probes tried) — all real git repos, same
fixture discipline as the existing hook-mode worktree fixture. No sibling script duplicates
this resolution logic (checked bloat-audit, spend-audit, pattern-audit, recurrence-audit).
v0.26.1 · 2026-08-20 · the screens->frontend plugin rename (Kim's ruling 2026-08-20: one plugin absorbing the front-end knowledge series): live `screens:` handles and plugin-prose mentions repointed to `frontend:`/`frontend`; ledger history untouched. Also: `renames.json` plugins map gains screens->frontend; ~37 new_plugin fields repointed (fix-old-names' live lookup).
v0.26.0 · 2026-08-20 · host-audit gains F10 + a P4/P5-shaped F5 enrichment (source: gen-ui-kit's 2026-08-20 performance report, relayed by its resident session): `host_probe.mjs`'s worktree census now carries per-home `pkgManagers` (npm/pnpm mix — the mixed-bootstrap census is the precondition for the pnpm-vs-npm-ci dist-drift class, which stays a REPO gate's job, named informationally only) and `nodeModulesDiskMB` (aggregate duplicated-store cost, null when du exceeds the probe budget), selftest extended with per-home schema + numeric checks; `references/remedies.md` F5 gains the shared lockfile-hash-keyed pnpm virtual store as a changes-system-behavior structural remedy carrying its two named verification prerequisites, and new **F10 — gate-lane oversubscription** (trigger `devProcs.testRunners.count >= cores` with `perCore1m >= 1.5`, computed from existing probe fields): the cause-side twin of F3 naming the uncapped repo check chain, repo-side lane cap as the durable fix, fenced against `flaky-gates` (this class reports oversubscription-by-design, never judges a specific red run); Non-macOS portable list gains F10. SKILL.md body F1-F9 -> F1-F10 + probe-field line. Fresh-context `harness:skill-checker` pass per plugin-authoring.md's semantic-edit invariant.
v0.25.4 · 2026-08-20 · closes #798 (two instrument-calibration findings, agent-ui estate 6-instrument sweep): `bloat-audit/scripts/measure.py`'s `dense-description` flag no longer over-fires on legitimate `NOT for X (owner)` routing-disambiguation clauses — a `NOT_FENCE_RE` pattern discounts every parenthetical-owner NOT-clause from the description length before comparing to the 700-char threshold (14/19 flagged files in the sweep were entirely this shape); `selftest` gains a negative control (a NOT-fence-heavy description over the raw threshold must NOT flag) and an inversion fixture (a genuinely padded description with no NOT-fence content still must). `pattern-audit/SKILL.md`'s step 2 gains a probe-craft note: a bare keyword probe over an estate with a same-named sanctioned API or plain-English noun drowns in noise (cited incident: an `ENUM_USAGE=\benum\b` probe came back 91% noise, 80/88 hits being the sanctioned `prop.enum(...)` call or "enum" as an ordinary word) — guard with lookaround (not preceded by `.`, not followed by `(`) before running, not after judging matches by hand. Fresh-context `harness:skill-checker` pass on the pattern-audit body edit per plugin-authoring.md's semantic-edit invariant.
v0.25.3 · 2026-08-19 · closes #794 (marketplace.json displayName/description drift): `plugin.json` gains the `displayName: "Authorkit"` field its 7 sibling plugins already declared — the one gap the drift audit found in this plugin's own manifest (workspace root's `marketplace.json` already carried "Authorkit" independently, masking the gap). No naming/authoring-standard change in this plugin beyond the manifest field itself; the sibling drift fixes (agent-protocols displayName, docs/llm/design/screens description sync) land in the same PR against the workspace root and those plugins' own dirs, not this one.
v0.25.2 · 2026-08-19 · closes #775 (`bloat-audit`'s `large-failure-section` flag, 2/2 false positives on a 3-estate batch run): `measure.py`'s bare char-count threshold now gates through a restatement test before flagging — a list-shaped Failure/Failure-catalog section (2+ top-level bullet/numbered entries) is only flagged when a MAJORITY of its entries substantially restate the procedure text stated above them in the same document (unigram shingle-overlap Jaccard, `RESTATEMENT_JACCARD=0.15`, over `RESTATEMENT_MAJORITY=0.5`); a non-list-shaped (prose) section still flags on char count alone, unchanged. Confirmed against both live false positives (design/make-variants, screens/check-ui-change: both now 0 flags, chars still measured at 901/885) and against harness's genuinely bloated Failure-branches sections (still flagged, unchanged). `measure.py selftest` gains two fixtures (issue #775): a dense-but-legitimate enumerated failure list modeled on the two live false positives (negative control, must not flag) and a genuinely restating failure section (inversion fixture, must still flag). Pure code/config under `measure.py`'s own selftest + ruff — no SKILL.md prose described the flag's mechanics in terms this changes, so no checker dispatch owed.
v0.25.1 · 2026-08-19 · gate repair for 75b6cff (host-audit landed direct-to-main missing its release riders — G10/G12b red on main): bundled naming.manifest.json resynced from root (host token), README map row added, this ledger section completed.
v0.25.0 · 2026-08-19 · new skill host-audit — dev-host performance audit: host_probe.mjs probe (CPU/RAM/disk/process pressure) + references/remedies.md with warned/destructive remedy tiers; landed at 75b6cff (ledger line written retroactively by the 0.25.1 repair).
v0.24.4 · 2026-08-19 · closes #763 (spend-audit gains a PR-label split axis — the instrument #759's live-lane measurement depends on): new bundled script `spend-audit/scripts/cohort_report.py` — `--label <name>` (repeatable) splits a `gh pr list`-fetched merged-PR window into `labeled`/`unlabeled` cohorts (any named label -> labeled), each reporting open->merge wall-clock, additions/deletions, checker-verdict-in-body rate, revert-mention rate, and a 48h follow-up-fix rate (file-overlap proxy, checked against the full window). No `--label` -> unchanged single `all` cohort. Pure function (`compute_cohorts`) split from the one `gh` I/O call for a network-free selftest: two-PR labeled/unlabeled split, absent-label single-cohort, empty-cohort `n/a` (never `0`), and 48h-window boundary cases. SKILL.md gains a Procedure step 6 paragraph + `allowed-tools` grant + a description trigger clause; evals.json gains two trigger cases (t09/t10). Fresh-context `skill-checker` pass on the semantic SKILL.md edit; `ruff` clean; `release_gate.py authorkit`: CLEAN, 0 fail / 9 warn (G10 for this entry's own ledger-line length, plus this entry's own `full-flow` hyphenated English compound joining the pre-existing G8 benign-compound class; both non-blocking, same tier as the plugin's existing warns).
v0.24.3 · 2026-08-19 · closes #718 (stolen-trigger repair from the #676 routing sweep, three case-pairs): `bloat-audit` was losing its own t04 (restated content across files) 2-of-3 to `pattern-audit` — `bloat-audit`'s description gained the explicit cross-file-restated-content trigger clause plus a reciprocal NOT-for fence against `pattern-audit`, and `pattern-audit` gained the mirrored n09 fence quoting `bloat-audit`'s own t04 verbatim. `doctrine-audit` was losing its own t06 (stale tool name/term in an agent or skill body) 2-of-3 to `fix-old-names` — `doctrine-audit`'s description now names the vocab-term edge's stale-tool-name/term trigger explicitly and fences out `fix-old-names`' rename-wave-provenance handles; `fix-old-names` gained the mirrored n12 fence quoting `doctrine-audit`'s own t06 verbatim. `manifest-authoring` was losing its own t04 (retire an exemption whose rename already landed) 3-0 to `exemption-retire` — retargeted rather than flipped, since `manifest-authoring`'s own Procedure step 4 already performs this manifest-only removal (no plan/apply left to do, distinct from `exemption-retire`'s verify->plan->execute chain for a rename not yet done): description sharpened to say so, and `exemption-retire` gained the mirrored n05 fence quoting `manifest-authoring`'s own t04 verbatim. No case content changed on any of the three "losing" suites — only the winning suite's own description and the loser's reciprocal fence. `pattern-audit`'s NOT-for list normalized to one form (`NOT for X` throughout) in the same change. Full-suite blind re-judge (19-entry authorkit menu, all six touched suites, 87 cases, unnamed synchronous routing-judge dispatch) re-run post-wording-fix: zero steal/leak/dead across all 87 — the three targeted pairs confirmed fixed (pattern-audit/n09, fix-old-names/n12, exemption-retire/n05 each now route to their true owner, quoting the winner's own case verbatim). One unrelated near-miss disclosed, not a regression: fix-old-names/n03 picks `rename-execute` instead of the out-of-menu `harness:big-change-git-rules`, an artifact of judging against the authorkit-only 19-entry menu rather than the full installed-plugin estate. `release_gate.py authorkit`: CLEAN, 0 fail / 8 warn (pre-existing G8/G10, unrelated).
v0.24.2 · 2026-08-19 · closes #693 (authorkit slice; harness slice same PR): `attention-audit`'s Step 6 trend capture now names the concrete routing-report source — `<root>/.claude/ops/routing-report.json`, the stable, git-tracked, merge-by-plugin file harness `check-routing`'s new Phase 6 "Persist" step writes — passed to `trend.py --routing-report` when present; absent (no recent check-routing run, or harness not installed) still records the dead/stolen/leaked columns as the literal `absent`, unchanged. attention-audit remains the sole attention-trend row-writer (it reads the report file, never calls check-routing); `trend.py` itself needed no change — its `--routing-report` flag and per-plugin JSON shape already matched. Historical attention-trend rows stay `absent` (honest, per the folded #693 decision); only new rows going forward can carry real numbers. Body-only SKILL.md edit (no description/evals change); fresh-context `skill-checker` pass.
v0.24.1 · 2026-08-18 · closes #669 (post-#433 rename-retirement sweep, authorkit slice; teamwork + harness slices same PR): three FUNCTIONAL stale handles fixed — `overhaul-execute`'s frontmatter description and its live `teamwork:build-lead` Agent-dispatch instruction (SKILL.md:6/:139/:204), plus `attention-audit/scripts/usage.py`'s preload map (`preloads = {"dispatch-ticket": ["build-lead"]}`, targeting a nonexistent agent) — all repointed to `build-leader`. `authorkit/README.md`'s own Map row for `overhaul-execute` updated to match. Both `overhaul-execute` and `overhaul-planning` evals.json owner-comments (n07) updated for the same handle. `overhaul-execute`'s description changed (not just its body), so its `evals/evals.json` note/comment text was updated in the same change per `plugin-authoring.md`'s routing-surface rule; `/check-routing` re-run on both suites came back clean (overhaul-execute 17/17, overhaul-planning 19/19, zero new steal/leak). Pure mechanical find/replace of a retired handle name, no surrounding instructional wording rewritten; no fresh-context checker dispatched per the semantic-vs-mechanical calibration in `.claude/rules/plugin-authoring.md`. `renames.json` and historical ledger lines deliberately left untouched (append-only, historical-by-policy).
...
`UNMEASURED` reserved for best-effort retroactive backfill only — never a live firing's own
default); `trend.py`'s `--archetype` joins the required-flag set. New script
`archetype_gradient.py` emits a MEASURED per-archetype token-cost multiplier table against the
A1 solo baseline, normalized per a newly-defined "per equivalent outcome" concept (`pr-shipped`
:= `outcome == pr-merged`, `record-minted` := `outcome == acted` — stated against the existing
closed `outcome` enum, no new column); every multiplier discloses its denominator, and an
under-measured cell honestly reports `UNMEASURED`, never a guess. Wall-clock stays
uninstrumented by design (a stated non-goal); gh#265's own 1.92x tokens / 3.6x wall-clock
coordinator-hop measurement is cited as an external anchor, never computed from ledger rows.
Output lands in both forms the ticket's Acceptance names: the script's own `--json` render, and
a committed, stable-path snapshot doc, `skills/spend-audit/references/
archetype-cost-snapshot.md`, refreshed at every authorkit release boundary — the citable home
`#672`'s ADR and `#666`'s per-archetype rubric pack point at. The existing single ledger row
(`#624`'s own seed) is migrated to the new schema, `archetype: UNMEASURED` (its own dispatch
shape crossed a fork/coordinator/named-seat chain too ambiguous to attribute honestly — never
guessed). Minor bump: a required-field schema addition + a new script, backward-incompatible
for a hand-typed row (never for anything going through `trend.py`, which already enforces the
new required flag).
