...
GRANT_LITERALS = ["auto-merge: authorized", "auto"]
...
def split_workflow_file(text: str):
    ...
    `export const meta = { ... }` object literal, found via brace-balance counting (string-
    literal aware, so a `{`/`}` inside a quoted value never miscounts). Returns
    (meta_src, body_src), or None if no `export const meta = {` marker is found (the file
    isn't this loader's shape — reported as a finding, never silently skipped)."""
...
def node_check(src: str, ext: str, wrap_async: bool):
    ...
    `await` in the body half is not a false positive. Returns (ok, stderr)."""
    if wrap_async:
...
def check_workflow_syntax(path: Path):
    ...
    if not meta_ok:
        return {
            ...
            "file": str(path),
            "status": "fail",
            "detail": f"meta block syntax error: {meta_err.splitlines()[-1] if meta_err else 'unknown'}",
        }
    body_ok, body_err = node_check(body_src, ".js", wrap_async=True)
    if not body_ok:
        return {
            "criterion": "A7-R4",
            "file": str(path),
            "status": "fail",
            "detail": f"body syntax error: {body_err.splitlines()[-1] if body_err else 'unknown'}",
        }
...
def run_g2(root: Path):
    ...
    files = [f for f in files if "node_modules" not in f.parts and "dist" not in f.parts]
    if not files:
...
def check_grant_literal(text: str):
    ...
    used it correctly stays judgment (A6-R2's own table entry)."""
    missing = [lit for lit in GRANT_LITERALS if lit not in text]
    if missing:
...
RESUME_PATTERN = re.compile(r"/file-(bug|feature|task)\s+#?[A-Za-z0-9_-]*")
...
def render_report(all_findings: dict) -> str:
    lines = []
    any_fail = False
...
def run_selftest():
    ...
    # check_grant_literal
    good_text = "the literal line `auto-merge: authorized` ... never infer from `auto`"
