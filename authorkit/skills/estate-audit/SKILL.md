...
description: >
  ...
  and pattern-audit are four read-only diagnostic instruments sharing one
  batch-sweep mechanic, consolidated behind one parameterized agent
  (estate-audit-agent, issue #293/#272). Use when the user asks which audit
  instrument fits an ask, or how to name the batch agent's `instrument`
  parameter. NOT for running any one sweep directly — reach the owning
  instrument skill (naming-audit/bloat-audit/attention-audit/pattern-audit)
  for that; this skill has no procedure of its own.
author: kim
created: 2026-08-15
