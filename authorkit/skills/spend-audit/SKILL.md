...
kind: skill
description: >
  Instrument idr-0010's estate economy claim — a per-firing token-spend ledger at
  `.claude/ops/spend-ledger.csv` (attention-trend.csv-shaped, append-only). Appends one
  validated row per sweep/build firing (tokens, provenance, outcome, verdict) and
  re-validates an existing ledger. Use for "price a sweep/build firing",
  "append a spend-ledger row", "is this sweep worth firing / worth its tokens",
  "validate the spend ledger", "backfill missing firing rows", "which recurring
  automation costs more than it yields". NOT menu rent / description collisions
...
1. **Append** (one firing): `python3 <this skill>/scripts/trend.py --out
   ...
   exactly the close-out convention above. Validated before it writes: a malformed row
...
separate and every `undetermined`/`absent` state reported as exactly that — never
conflated with a computed judgment.
