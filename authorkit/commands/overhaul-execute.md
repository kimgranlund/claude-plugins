...
The DRIVES half of the `overhaul-planning` pair: that skill generates the plan; this command
runs the campaign — discover, measure, plan, execute approved waves — composing the existing
instruments and reimplementing none. Host-run only: the three gates below all need a live
user, and this command is never dispatched unattended. Standalone command, no companion skill
— `execute` has no legal skill-grammar production today (`naming-conventions`' `ProcessLex`
doesn't carry it, and it can't; the `rename-execute`/`exemption-retire` precedent in this same
estate is the same shape for the same reason). A run where `AskUserQuestion` is unavailable,
errors, or gets auto-answered under momentum counts as no live user: it stops at its first open
gate and reports SKIPPED, leaving the decision to a human.
...
- **rename** → `rename-planning` (Skill tool) for the plan, then read
  ...
  — it is command-only, the same species as this command, invoked by reading and following its
  body rather than through the Skill tool; its own touched-file confirm still runs, verifying
  the enumerated blast radius the Gate-A decision already approved.
...
- No live user at any gate → stop, report that gate SKIPPED and the run blocked on a human —
  this command never self-approves.
