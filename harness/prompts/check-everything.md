---
description: "Audit the whole harness surface — skills, agents, hooks, entry files, plugins — in one sweep: deterministic lint pass, then fan-out fresh-context reviews against the matching standards, then a triage table routing every finding to a fix, a decision, or permanent infrastructure. Run /check-everything [root, default .]. Read-heavy; writes only reports. The recurring outer loop."
