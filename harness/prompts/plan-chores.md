---
description: "Run the standing chore-planner seat (agents/chore-planner.md) on demand — standalone, it plans from durable .claude/ops state plus live gh evidence and rewrites the prioritized action queue at .claude/ops/plan.md; every entry names action, owner, evidence, size. States the seat's operating contract as a fixed banner before the first-ever plan. Run /plan-chores [blank, or a focus instruction]."
argument-hint: "[blank to (re)plan from current state | a focus instruction, e.g. 'branches and PRs first']"
---

# chore-planner

Dispatches the standing `chore-planner` agent (`agents/chore-planner.md`) on demand — the same
skill↔agent same-name pairing ruled for `/sort-issues` and shared by `/sweep-chores`:
...
mechanics live in `/sort-issues`'s body.
...
2. **Dispatch.** Call the `chore-planner` agent (Agent tool, `subagent_type: "harness:chore-planner"`),
...
```
chore-planner — the prioritization seat for this repo's ops backlog.
...
- The ask is really a fresh sweep ("run everything, then tell me what's first") → name
  `/sweep-chores` as the right door and do not dispatch; chaining the fan-out from here would
