---
description: "Mint or grow a knowledge pack's reference corpus via question-led research waves: charter an axis, write the question set, gather dated sources, distill ask-shaped files, then register and validate. Run /make-pack [skill-dir or \"new pack: domain\"], one axis per wave. NOT the SKILL.md surface (make-skill); NOT a split/merge decision (plan-skill-split/plan-skill-merge); NOT the rules themselves (pack-writing-rules)."
