---
description: "Run a structured alignment campaign on a repo whose agent harness and context records have drifted — duplicated instruction trees, bloated entry files, dead automation, orphaned files, unwritten conventions, and a stale living design-doc corpus (ADRs, PRDs, SPECs, LLDs, RFCs). Use when the user says \"align/recalibrate this repo\", \"run a harness consolidation / hygiene campaign\", \"our CLAUDE.md is bloated and stale\", \"consolidate the .claude setup\", \"audit and clean up the agent harness\", \"our ADRs/specs have drifted from reality\", \"this repo's context has drifted\". A deliberate, multi-phase, committing campaign — never auto-fires; invoke explicitly via /clean-repo (the trigger phrases above are what a session should recognize and SUGGEST this command for). NOT for the estate's own plugin workspace (check-everything); NOT for layout-only migration of docs to the canonical docs/ map (tidy-docs, docs, where installed — this campaign's corpus lenses go deeper: liveness, supersession, status-vs-reality)."
