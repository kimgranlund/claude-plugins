...
Checks (each cites the ADR-0022 line it realizes):
  1. Working-tree cleanliness — every git-ignored-and-present path under repo-root, classified
     against the repo's OWN top-level `.gitignore` (declared-regenerable, e.g. `dist/`), OR a
     NESTED in-repo `.gitignore` (a tool-written cache dir's own file, e.g.
     `.ruff_cache/.gitignore` — still regenerable, resolved live per-path via `git check-ignore
     -v`), vs a NAMED exception (e.g. `settings.local.json`, exception 4) vs UNCATEGORIZED (a
     defect candidate — load-bearing local state declared regenerable nowhere and enrolled
     nowhere).
...
def classify_ignored_path(rel_path: str, gitignore_patterns: list, nested_source: bool = False):
    """Pure: bucket ONE git-ignored-and-present relative path.

    - Matches a pattern this repo's OWN top-level `.gitignore` declares (substring match on the
      pattern's own text, trailing `/`/`*` stripped) -> "regenerable" (self-declared, in-repo).
    - `nested_source=True` -> the ignore rule's own source file is a NESTED `.gitignore` living
      INSIDE the repo tree (e.g. a tool-written `.ruff_cache/.gitignore`, `.pytest_cache/`'s own
      convention) rather than the repo's top-level file or an external global excludesFile —
      still "regenerable": the rule ships with, or is regenerated by, the tool inside the repo's
      own tree, never something a fresh clone would need external state to reproduce. Distinct
      from `.gitignore`-pattern matching because the RULE lives at a path this function is never
      handed a parsed pattern list for.
    ...
    - Anything else -> "uncategorized" — a defect candidate: load-bearing local state neither
      declared regenerable (top-level or nested, in-repo) nor enrolled in the ADR's exception list.
    """
...
def _nested_ignore_source(repo_root: str, rel_path: str) -> bool:
    """Live: `git check-ignore -v` names the exact `<source-file>:<line>:<pattern>` that ignores
    `rel_path`. True when that source file sits INSIDE the repo tree at a path other than the
    top-level `.gitignore` (a nested/tool-written `.gitignore`, e.g. `.ruff_cache/.gitignore`) —
    still in-repo and regenerable. False for the top-level `.gitignore` itself (handled by
    pattern-matching instead) or a source outside the repo tree (Check 3's own territory) or no
    resolvable source at all."""
    r = subprocess.run(["git", "-C", repo_root, "check-ignore", "-v", rel_path],
                        capture_output=True, text=True)
    if r.returncode != 0 or not r.stdout.strip():
        return False
    source = r.stdout.split(":", 1)[0].strip()
    if source in (".gitignore", "", None):
        return False
    source_path = (Path(repo_root) / source).resolve()
    try:
        source_path.relative_to(Path(repo_root).resolve())
    except ValueError:
        return False  # outside the repo tree entirely — Check 3's global-excludes territory
    return True
...
def run(repo_root: str, claude_home: str, config_home: str):
    ...
    # Check 1 — ignored-and-present paths. `_nested_ignore_source` resolves whether the actual
    # ignore RULE for a path lives inside the repo tree at a path other than the top-level
    # `.gitignore` (e.g. a tool-written `.ruff_cache/.gitignore`) — still in-repo, still
    # regenerable, distinct from an external global excludesFile (Check 3).
    ...
    ignored_findings = [
        {"path": p, "class": classify_ignored_path(
            p, gitignore_patterns, nested_source=_nested_ignore_source(repo_root, p))}
...
def selftest():
    ...
    # nested-source (a tool-written .ruff_cache/.gitignore etc.) is regenerable even though it
    # matches no top-level .gitignore pattern — the live day-one finding this closes
    assert classify_ignored_path(".ruff_cache/foo", [], nested_source=True) == "regenerable"
    # reverse control: nested_source=False with no matching pattern still reads uncategorized
    assert classify_ignored_path(".ruff_cache/foo", [], nested_source=False) == "uncategorized"
