...
- **token-builder** (the `design` plugin's seat, where installed) — *Files changed* = the token files (`tokens.css` / `dimensions.css`). *Evidence* = the WCAG-AA + forced-colors result across *both* schemes, and that no role ladder collapsed to one step.
