...
# plan-plugin-split — partition the surface, don't just group it

The distribution-layer sibling of `plan-skill-split`: where packs partition by *question type*,
...
artifact. Execution: `/make-plugin` Phases 3–6 scaffold and release each resulting plugin; member
moves follow the validated manifest. Surface: `$ARGUMENTS` (default `.`).

## Phase 1 — Survey: the graph is the evidence

`python3 "${CLAUDE_PLUGIN_ROOT}/skills/plan-plugin-split/scripts/surface_map.py" map <root>` —
...
## Phase 2.5 — Escalate: partition as refactor opportunity (`thinking-depth-rules`)

The four tests answer "how should today's artifacts group" — a 1st/2nd-order question. Before
proposing, invoke `thinking-depth-rules` and climb, paying rent at each step:
...
- **5th order** — interrogate the inputs themselves: is the observed structure a fossil of its
  ...
  line in the **refactor-opportunities ledger** (owner: plan-skill-split, plan-skill-merge,
  check-skill, make-hook, or make-pack), co-deliverable with the partition manifest.
...
(`/make-skill`, `/make-pack`, `/make-hook`, `/make-agent`).
...
```jsonc
{
  ...
  "plugins": [
    { "name": "harness", "charter": "authoring the harness's artifacts",
      "members": ["make-skill", "skill-writing-rules", "skill-checker", "skill_lint.py"] }
  ],
  "seams": [ { "from": "harness", "to": "data-plane", "kind": "mention", "count": 2, "justification": "handoff after verdict" } ],
  "gap_candidates": [ { "name": "x-refactor", "evidence": "3 dangling handoffs from x-decompose", "owner": "/make-skill" } ],
  "refactor_opportunities": [ { "finding": "thin sibling pair with shared vocabulary", "owner": "plan-skill-merge" } ],
...
- **Not plan-skill-split.** One corpus → child packs is content partitioning; this is distribution
  partitioning of heterogeneous artifacts. A pack that is itself too big routes there first.
- **Not make-plugin.** Greenfield family design runs on projections; this runs on an inventory.
