...
def _load_routing_eval():
    ...
    location upward (works regardless of which plugin hosts it — harness keeps it at
...
def selftest():
    ...
    # same-line regex in an earlier version of this parser had — harness's own agents, including
    # its pre-existing skill-checker, use this YAML shape, not the inline `[a, b]` form)
    def block_style(d):
