...
A plugin is one versioned unit carrying skills, agents, hooks, and MCP config — and it loads **atomically**: one malformed file fails the whole plugin, so the release gate is not a nicety but the difference between shipping and shipping an error screenshot. This is the standard `/ship-plugin` gates against and `/check-everything` scores against; `skill_lint.py` (P-rules) and `release_gate.py` enforce the checkable slice.
...
Structure, cache-key, reload, and trust semantics verified against code.claude.com/docs/en/plugins-reference and this plugin's own three build incidents, 2026-07; all mechanics [drift-prone]. Skill-side rules: `skill-writing-rules`. Hook-side: `hook-writing-rules`. Agent-side: `agent-writing-rules`.
