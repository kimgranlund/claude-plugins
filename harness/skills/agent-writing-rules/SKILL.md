...
8. **Delegate, don't dictate — the dispatch prompt states the goal, not the mechanics.** A dispatch prompt carries the goal, the constraints that are genuinely NOT discoverable from the repo or the seat's own preloaded contracts (measured numbers, decisions already ratified, a deliberate no on a named approach), and the target to verify the result against. It does not restate repo mechanics the receiving seat's own preloads already cover — label taxonomies, sizing schemes, lint rules, naming grammar — because that seat will look them up (or already has them loaded) regardless of what the dispatch says, and a wrong restatement actively misleads. Specification is paid on every dispatch whether or not it helps; verification bills only on failure — so the token spent describing a discoverable fact buys nothing, while the token spent on a genuinely non-discoverable constraint is the only kind that pays for itself. Enumerated step-by-step instructions are the exception, not the default: earn them only where a specific past incident already showed this seat (or its class) getting that step wrong unprompted. Worked example: this session's #259–#264 intake batch — the dispatching brief specified `kind:*` label names and an S/M size ladder, both WRONG for this repo (bare `task`/`feature` labels, a binary `size:small`/`size:big` scheme), while the dispatched intake seat correctly overrode them from its own preloaded contracts (`doc-writing-rules`, a live `gh label list`); the mechanics the dispatch spelled out cost tokens and bought nothing but errors to correct, while the non-discoverable evidence in that same dispatch (measured `skillUsage` numbers, a deliberate no on a de-loop) was exactly what the seat could not have recovered any other way.
...
| Dispatch prompt dictates repo mechanics instead of delegating | The seat's own preloaded contracts override wrong mechanics when they win — and mislead the seat when they don't; either way the specification cost is paid on every dispatch and buys nothing but errors to correct (issue #267, #259–#264 worked example) | State the goal + non-discoverable constraints + verify-against target; enumerate steps only where a named incident proved the seat gets it wrong (cold-start language item 8) |
