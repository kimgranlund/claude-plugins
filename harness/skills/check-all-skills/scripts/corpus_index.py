...
# violation). The CANON is harness_checks.RESERVED_SKILLS (harness's scripts/) — parsed at
...
def _harness_reserved():
    ...
    lands on the real tree. harness keeps harness_checks.py at <plugin-root>/scripts/, a sibling
...
def main(argv):
    ...
    CRITIC = _re.compile(r"reviewer|generator ≠ critic|generator≠critic|\[\[check-whole-ui\]\]|\[\[check-colors\]\]|round-trip|fresh-context", _re.I)
    ...
    for d, s in sorted(skills.items()):
        ...
        if tail not in VERBS and not is_transform and "make-pack" not in body_:
            findings.append(f"{d}: knowledge pack names no factory route ([[make-pack]])")
    for f_ in findings:
        print(f"  [advisory] {f_}")
    if not findings:
