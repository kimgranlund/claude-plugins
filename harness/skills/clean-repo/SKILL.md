...
description: >
  ...
  campaign — never auto-fires; invoke explicitly via /clean-repo (the trigger phrases
  above are what a session should recognize and SUGGEST this command for). NOT for the
  estate's own plugin workspace (check-everything); NOT for layout-only migration of docs to
