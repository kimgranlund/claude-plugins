...
description: >-
  ...
  knowledge", "did this citation go stale", "already a reference covering this", "propose a plan
  for capturing this decision before we write anything down". NOT authoring
...
save-lessons turns a signal — a repeated correction, a ratified decision, a high-impact fact
...
forever. This skill orchestrates existing machinery (`make-pack` authors, `release_gate.py`
gates, `/check-routing` measures routing) — its own delta is the signal, the confirmation payload, the
...
Route the confirmed plan to `make-pack` (the corpus: `references/*.md`, `scripts/routing-corpus.json`,
`evals/evals.json`) and `make-skill` (the `SKILL.md` entry surface) — or apply their conventions
...
1. **Structural gate:** `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/release_gate.py" <plugin-root>` —
   ...
   linter for it.
2. **Independent review:** dispatch `skill-checker` (a new skill) or `plugin-checker` (an existing
...
3. **Routing gate:** if any description or routing-corpus changed, name `/check-routing <plugin-root>` as
