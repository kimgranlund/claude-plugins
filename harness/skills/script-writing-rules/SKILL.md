...
| Deny-list in an eligibility/permission check | Fails OPEN for anything unenumerated — a new script, file kind, or contract surface added later slips through untouched [dated 2026-08-14/15, two convergent occurrences: `dispatch-ticket`'s QB4 fix (a denylist naming specific scripts matched harness's own gate scripts); `naming-audit`'s `validate.py` fix (author_registry shape)] | Allow-list scoped BY CONSTRUCTION (e.g. "skill-level bundled scripts only," never "every script except these named ones") — nothing added later needs the check updated |
