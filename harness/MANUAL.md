...
**`/estate-maintenance [root] [--window-days N] [--out dir]`** — the periodic self-improvement retrospective: mines auto-memory, the two trend CSVs, ADR/IDR decision records, and `gh` issue history for negative patterns (a repeated user nudge, a re-filed near-duplicate ticket, metric drift, context-surface ceremony) and proposes context-level fixes behind exactly one confirm — read-only until then, never `context: fork`. Also model-invocable: "what keeps going wrong", "why do I keep correcting you", "we keep re-filing the same ticket", "context keeps growing".
> `/estate-maintenance` — one static audit instrument (authorkit's attention/bloat/pattern/doctrine-audit, repo-audit) is a read-only measurement; plugin lint health is `/check-everything`'s job; work state is `/check-state`'s job; one fact's harvest is `save-lessons`'s job — this is the temporal cross-source join none of them perform
...
- **`repo-sync`** — "pull/rebase/merge so we can see current dev" · "sync main and catch me up on merges" · "is it safe to merge this PR" · "the app looks stale after that pull" — the plain git-hygiene half of touching a shared checkout: fast-forward-only main sync (never merge/rebase the shared main), rebase-onto-fresh-main for a feature branch, PR-merge verification by state read-back (never trust an exit code), and post-pull dev-freshness (clear the build tool's dependency cache, flag a restart, never kill a dev server this session doesn't own). Distinct from `clean-git`/`repo-cleaner` — that's a standing hygiene SEAT with its own gated scripts; this is what any session does on a plain pull, no dispatch required.
...
- **Agents `issue-sorter`, `repo-cleaner`, and `decision-watcher`** — the estate's standing operational seats, a different character from the review agents above: fired on a schedule (CronCreate) or dispatched on demand, not dispatched by a review workflow. `issue-sorter` implements the watch/triage/trust SPEC to intake and route work items, gated by a durable friendlies allow-list; `repo-cleaner` inventories and cleans repo hygiene, executing ONLY through this plugin's own gated scripts; `decision-watcher` periodically diffs the ADR corpus by checkpoint, judges each changed Decision against `save-lessons`'s bar, and queues candidates for one batched confirm — never authoring, only naming the `/make-pack`/`/make-skill` command a human runs next. None of the three ever edits source or merges/deletes/authors a knowledge-pack file outside its own narrow, script-backed allowance. Each is a thin shell (issue #205): its full per-firing procedure lives in a preloaded skill of its own (`watch-tickets`, `clean-git`, `watch-adrs` respectively), never restated in the agent body.
- **`ops-write-sandbox-rules`** — "why can't an ops seat write its own state file" · "what's a fenced target-pathed payload block" · "what does narrated-but-absent mean" — the one canonical statement (issue #125, consolidated issue #205) of why the four compute-only ops seats above carry no `Write` tool and how the dispatching session applies their computed state instead (`/sweep-chores`'s own procedure, via `scripts/chore_sweep_apply.mjs` — issue #266 retired the `chore-lead` coordinator agent that used to hold this role); preloaded by all four ops-family agents rather than restated per file.
- **`blocked-by-rules`** — preloaded by `chore-planner` (`disable-model-invocation: false`, own eval suite). How `chore-planner` reads a ticket's `Blocked-by: #NN` line and orders `.claude/ops/plan.md` so a blocked entry queues behind its blocker, or names the dependency when the blocker isn't itself queued (issue #193). The line's own canonical format is documented once, in teamwork's `mobilize-chores` skill — this skill cites it rather than re-defining it.
...
- **`sweep-chores` and agent `chore-planner`** — the family's coordination pair. Issue #266 retired the `chore-lead` coordinator agent that used to run this sweep: its whole job was a deterministic dispatch graph (fan out, collect, hand off), exactly the shape the platform's native Workflow tool exists for, not one that pays a model-in-the-loop coordinator (#265 measured the old chain at 1.92x output tokens / 3.6x wall-clock vs. solo for equivalent outcome quality on a quiet estate). `sweep-chores` now runs that choreography directly — a checked-in Workflow script (`workflows/chore-sweep.js`) when the Workflow tool is available, an identical Agent-dispatch fallback otherwise — fans out the three standing seats above in parallel (or a scoped subset), applies every returned payload via `scripts/chore_sweep_apply.mjs` (the former coordinator's own step 3, mechanized), then hands the bundle to `chore-planner`, which turns it into ONE prioritized action queue at `.claude/ops/plan.md` — every entry naming its action, owner, evidence, and size. Neither ever authors a queue or mutates anything beyond `.claude/ops/`; the planner executes nothing it queues and writes exactly that one plan file. `chore-planner` also runs standalone, planning from durable `.claude/ops` state plus live `gh` evidence when no fresh sweep exists. `sweep-chores` cites `agent-writing-rules`' Failure catalog for the bare-name-dispatch (gh#154) and named-teammate-misaddress (gh#157) mechanisms in its fallback branch instead of re-deriving them inline (issue #205).
- **`/sweep-chores [scope]` and command `/plan-chores [focus]`** — run on demand. `/sweep-chores` carries its own choreography directly now (issue #266) rather than dispatching a coordinator agent, and is also reachable cross-plugin via the Skill tool (`Skill(harness:sweep-chores)`) for `teamwork:mobilize-chores`; `/plan-chores` stays a thin dispatcher for the `chore-planner` agent. Each states its own operating contract as a fixed banner before the repo's first ops queue exists (checked before the sweep — the sweep itself is what creates `plan.md`), then relays the report unmodified. `/sweep-chores "repo hygiene only"` scopes the fan-out; `/plan-chores "branches and PRs first"` shifts emphasis without changing the entry contract.
