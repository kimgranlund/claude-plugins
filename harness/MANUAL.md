...
- **`ops-write-sandbox-rules`** — "why can't an ops seat write its own state file" · "what's a fenced target-pathed payload block" · "what does narrated-but-absent mean" — the one canonical statement (issue #125, consolidated issue #205) of why the four compute-only ops seats above carry no `Write` tool and how `chore-lead` applies their computed state instead; preloaded by all five ops-family agents rather than restated per file.
- **`blocked-by-rules`** — preloaded-only (`disable-model-invocation: true`, never model-routed, no eval suite — `chore-planner`'s own preload is the only caller). How `chore-planner` reads a ticket's `Blocked-by: #NN` line and orders `.claude/ops/plan.md` so a blocked entry queues behind its blocker, or names the dependency when the blocker isn't itself queued (issue #193). The line's own canonical format is documented once, in teamwork's `mobilize-chores` skill — this skill cites it rather than re-defining it.
