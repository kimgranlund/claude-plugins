...
| `skills/runtime-state` | Procedural skill | both | A LOCATOR over existing session `.jsonl` transcripts, never an export (issue #605): resolves the CURRENT session's own transcript path plus its recent subagent transcripts (`agent-*.jsonl` under the project/worktree-scoped `~/.claude/projects/<slug>` dir), or a PAST session's given its repo path/slug; `scripts/transcript_locate.py` derives the slug (every `/` and `.` in the resolved cwd path becomes `-`, verified against this machine's own real directory names), reads the live `CLAUDE_CODE_SESSION_ID` env var for "current," falls back to the newest top-level transcript otherwise; prints or clipboard-copies the path — zero new data written, no bundle, no hook |
...
| `skills/repo-sync` | Procedural skill | model-only | Moved in from an adiav2 project-local skill (2026-08-19) and genericized: safe fast-forward-only main sync, feature-branch rebase, PR-merge verification (state read-back, never trust an exit code), and post-pull dev-freshness (build-tool cache clear + restart flag) for a shared checkout. Distinct from `clean-git` (repo-cleaner's own inventory/classify/execute hygiene seat) — this is the git-surface-sync half any session doing a plain pull/rebase/merge needs, no seat dispatch required |
...
| `scripts/docs_check.py` | Script | CLI + selftest + gate G10 | R1–R7: README/MANUAL cover every skill, ledger version = manifest version, CLAUDE.md counts, script mentions, one-line-per-version ledger cap, `.claude/docs` spine id-collision (R7, closes #633) — docs freshness as a ship invariant, deliberately not a per-write hook |
...
v3.18.4 · 2026-08-23 · fixes #899: `harness_emit.py`'s Hermes `__init__.py` template emitted `os.path.join(...)` plain-str skill paths; Hermes's own `register_skill` (`hermes_cli/plugins.py:3355`) calls `path.exists()` on that argument, so the first registration raised `AttributeError` and zero skills registered on any plugin. Fixed to `from pathlib import Path` / `_HERE = Path(__file__).resolve().parent` / Path-joined skill paths, and every `register_skill()` call now also passes `description=` sourced from the skill's own frontmatter (truncated at 500 chars) to feed Hermes's progressive-disclosure metadata. `HARNESS-NOTES.md`'s Hermes section gains a note that plugin skills never appear in `<available_skills>` (`hermes_cli/plugins.py:3334-3337`) and resolve only via an explicit `<plugin>:<skill>` load — the verify step is "load `<plugin>:<skill>`", never "list your skills". `harness_emit.py selftest` gains a stub-`ctx` leg that imports the generated `__init__.py` for real and asserts `register_skill()` receives a `Path` with `.exists() == True` and a non-empty `description=`, plus a negative control proving the old `str` emission raised `AttributeError` on the same gate. All nine plugins' `__init__.py` + `HARNESS-NOTES.md` regenerated. PATCH: fixes a broken emission path, no new capability.
v3.18.3 · 2026-08-23 · closes #895 (LLD-0025 W4, T-2): `harness_emit.py`'s Pi MCP cell corrected from drop-with-note to PASSTHROUGH (T-2 amendment, #887: Pi core has no MCP field, but the third-party `pi-mcp-adapter` reads a project-local `.mcp.json` in the same `mcpServers` shape and `${VAR}` interpolation style Claude Code uses) — verified a no-op at the file level: `.mcp.json` already sits at the plugin root where a `pi install` places it, so nothing is written or copied for Pi; the change is documentation-only. `HARNESS-NOTES.md`'s Pi section now states the passthrough plainly, names the third-party-adapter caveat (install `pi-mcp-adapter` separately, not part of Pi core), and flags its own `[needs-substitution]` tokens independently of Codex/Hermes's rows (brand-design's `.mcp.json` regenerated as the one live example — `brand-corpus.args[0]`/`env.BRAND_CORPUS_DIR` flagged). `harness_emit.py selftest` gains a Pi MCP-passthrough leg asserting the note text and the substitution flag; `release_gate.py`'s existing G15 Pi leg already covers `package.json` freshness and needed no change. PATCH: corrects a documentation cell to match a closed research question — no new capability, no schema change.
v3.18.2 · 2026-08-23 · closes #891 (LLD-0025 W3, T-6): `harness_emit.py` gains the Pi backend — `package.json` (`name` scoped `@nonoun/<plugin>`, `version`/`description` mirrored verbatim, `keywords: ["pi-package", ...plugin keywords]` — the `pi-package` keyword IS Pi's registry contract, no separate marketplace file; `"pi": {"skills": ["./skills"]}`, plus `"prompts": ["./prompts"]` when the plugin ships a command-only skill; a pre-existing `package.json` is merge-only, touching just the `pi` key + `keywords`) and `prompts/<skill-name>.md` — one per `disable-model-invocation: true` skill, flat and non-recursive, frontmatter `description`/`argument-hint` from the skill's own frontmatter, body the SKILL.md body verbatim with `$ARGUMENTS` kept as-is (R-5 amendment, #888: this is how the human-only guard survives natively on Pi — a prompt template expands only from a human keystroke, so the model has no channel to self-invoke it, unlike Codex/Hermes where a command-only skill degrades to a plain model-routable one). `DEFAULT_HARNESSES`/`G15_HARNESSES` widen to `codex,hermes,pi`. `HARNESS-NOTES.md` renders a real Pi section (install form + command-only-skills-as-prompts note + the MCP drop-with-note pointing at the third-party `pi-mcp-adapter`, T-2/#887). `harness_emit.py selftest` gains Pi legs (write/verify roundtrip, `package.json` content predicates, prompt-file content/frontmatter, merge-only behavior, hand-edit bite, orphan detection on `prompts/*.md`); `release_gate.py selftest` gains a matching G15 Pi bite leg. `/ship-plugin`'s Phase 1 preflight and this workspace's CLAUDE.md/dist-output.md records repaired in the same change. `--probe`'s Pi leg established from `pi --help`/`pi install --help` (no `pi doctor`/`pi check` subcommand exists) — `pi install <root> -l` into a throwaway scratch cwd then `pi list`. PATCH: activates a stubbed backend the interface already declared (W2's `NotImplementedError` named this ticket) — not a new capability.
v3.18.1 · 2026-08-23 · closes #890 (LLD-0025 W2): `harness_emit.py` gains the Hermes backend — `plugin.yaml` (name/version/description, `manifest_version: 1`), the REQUIRED `__init__.py` `register(ctx)` stub (fixed template, one `ctx.register_skill()` line per skill, nothing else registered — R-5's Hermes verdict on #888 is terminal: command-only skills degrade to plain skills, the guard is not expressible on Hermes), and `hermes-mcp.yaml` (a config.yaml `mcp_servers:` fragment from `.mcp.json`, `\${CLAUDE_PLUGIN_ROOT}`/`\${user_config.*}` values flagged `[needs-substitution]`). `DEFAULT_HARNESSES`/`G15_HARNESSES` widen to `codex,hermes`. `HARNESS-NOTES.md` renders a real Hermes section (install form + config.yaml merge instruction — Hermes has no marketplace file) instead of the not-built-yet stub; Pi keeps the stub. `harness_emit.py selftest` gains Hermes legs (write/verify roundtrip, `__init__.py` template assertions, MCP fragment content, hand-edit bite, orphan detection on `hermes-mcp.yaml`); `release_gate.py selftest` gains a matching G15 Hermes bite leg. `/ship-plugin`'s Phase 1 preflight and this workspace's CLAUDE.md/dist-output.md records repaired in the same change. PATCH: activates a stubbed backend the interface already declared (W1's `NotImplementedError` named this ticket) — not a new capability.
v3.18.0 · 2026-08-23 · closes #886 (LLD-0025 W1): new script `harness_emit.py` derives a Codex overlay (`.codex-plugin/plugin.json`, per-skill `skills/<s>/agents/openai.yaml`, `HARNESS-NOTES.md`) from each plugin's own `.claude-plugin/plugin.json`/SKILL.md frontmatter, plus the estate-level `.agents/plugins/marketplace.json`; Hermes/Pi backends stubbed to raise `NotImplementedError` naming their own wave tickets (T-5, T-6). New gate `G15` (verify-only; the writer runs in `/ship-plugin`'s own preflight) fails on a stale/unexpected/marketplace-mismatched overlay — `.github/workflows/gate.yml`'s comment range moves G1–G14 to G1–G15. `release_gate.py selftest` gains a bite-proof G15 leg. MINOR: a new bundled script + gate check is new capability, not a patch.
v3.17.7 · 2026-08-22 · agent model retier (Kim's estate-wide ruling, live session): chore-planner → sonnet+xhigh; agent/hook/plugin/skill/wording-checker → sonnet+high; agent-writing-rules §Model tiering ladder rewritten to the new canon.
v3.17.6 · 2026-08-21 · `hook-writing-rules` gains a cross-reference in Mechanics: project-scope `settings.local.json`'s `PreToolUse` hooks and `permissions.deny` share plugin hooks' never-hot-reloads-mid-session behavior, but with no `/reload-plugins` equivalent — only a new process picks up the config (teamwork issues #852/#853, live-verified independently by two sessions). Failure catalog gains the matching diagnostic row; Provenance dated. Fresh-context skill-checker PASS with one minor wording fix applied (avoided overstating the parallel as mechanism-identity) and two nits folded in (catalog placement, Provenance freshness). Live-lane, single file.

v3.17.5 · 2026-08-20 · closes #814: `release_gate.py`'s G8 gains a mechanism-level fix instead of two more allowlist entries — a token glued to a literal `.md` anywhere in a SKILL.md (`<tok>.md`) is a FILE citation (this pack's own `references/<tok>.md`, or a verbatim external-repo filename kept for grounding fidelity) and never a phantom skill name, so it's exempted from the stale-name sweep; the existing prose-compound allowlist is untouched (redundant-but-harmless overlap, not cleaned up here). Fixes the two live false positives (`select-menu-name-bug` in frontend's state-model-rules, `url-state-sync` in persistence-facts) plus one pre-existing one for free (`plugin-authoring.md` in estate-maintenance). G8 still does not strip code spans generally — a bare backticked stale name with no `.md` glued to it still fires, proven by a new selftest negative control (a sources.md-shaped table row citing a genuine phantom name with no `.md` suffix). `release_gate.py selftest` PASS.
...
v3.17.1 · 2026-08-19 · `release_gate.py`'s G8 sibling-name allow set gains 11 entries triaged during the brand-forge -> brand-design migration (Phase 4): agent-seat/legacy-command prose (`brand-council`, `sub-council`, `orchestration-rubric`), named concepts (`brand-stack`, `expression-system`), plain prose (`cloud-skill`, `main-agent`, `per-stage`, `real-brand`), and the disclosed cross-marketplace mention of `design-skills:brand-decomposer` (`design-skills`, `nonoun-skills`) — none were rename drift, each verified against the actual prose at its site. `release_gate.py selftest` still PASS.
v3.17.0 · 2026-08-19 · new skill `repo-sync` — moved in from an adiav2 project-local skill (Kim's own ask) and genericized: the adiav2/`@adia-ai`-specific phrasing (checkout name, hoisted UI-kit package, the `adia-admin-integration` dev-server-ops citation, Auth0-port specifics) replaced with repo-agnostic equivalents (a generic "shared checkout," "hoisted UI-kit/dependency version," a soft pointer to "the host repo's own dev-server-ops reference, where one exists"); Never-list's `/finish-work` citation genericized to "the host repo's own PR skill." `skill_lint.py` clean; `evals/evals.json` authored fresh (6 trigger + 4 no-trigger, the no-trigger set fencing off finish-work-shaped PR creation, clean-git's own repo-cleaner-seat procedure, a host repo's dev-server pathology reference, and a standing fleet-wide watcher). No script of its own (pure prose procedure, same species as `big-change-git-rules`/`clean-git`'s own SKILL.md bodies).
v3.16.8 · 2026-08-19 · closes #774 (seat-suffixed report filenames for multi-seat firings, agent-ui 2026-08-19T21:56:06Z path-collision incident: issue-sorter/repo-cleaner both defaulted to the bare `.claude/ops/reports/<UTC-timestamp>.md`): `watch-tickets`' Failure branches now carry the canonical prescription — a firing whose resolved scope names more than one seat suffixes this seat's own name (`<UTC-timestamp>-<seat>.md`), a single-seat firing keeps the bare path; `clean-git` and `watch-adrs` cite it instead of restating their own near-duplicate copies. `clean-git`'s step 5 dedup read (the abbreviated-report "most recent file" lookup) now scopes to this seat's own naming so a sibling seat's suffixed report in the same directory can't be misread as this seat's own prior firing. `sweep-chores`, `chore-planner`, and the three thin agent wrappers (issue-sorter/repo-cleaner/decision-watcher) were grepped and confirmed to carry no direct path prescription of their own — no edit owed there. Fresh-context `harness:skill-checker` pass on the three-file slice per plugin-authoring.md's semantic-edit invariant.
v3.16.7 · 2026-08-19 · watch-adrs/adr_queue.py: coverage-grep pathspec anchored to '*skills/*/references/*.md' — the unanchored form missed plugin-nested <plugin>/skills/ layouts (4th adr-0023 false positive); selftest gains the nested-layout fixture with a negative control proving the old pathspec fails it.
v3.16.6 · 2026-08-19 · chore-sweep.js: firing key comes from args.timestamp (usage error when absent) — the workflow runtime now shims Date.now()/new Date() to throw (breaks resume), which killed the 22:20Z sweep firing at launch; callers (sweep-chores step 6) already pass the timestamp.
v3.16.5 · 2026-08-19 · closes #752 (decision-watcher: recurring adr-0023 harvest false positive, 3rd recurrence same week): `watch-adrs`' step 2 (harvest judgment) and step 3 (stale-citation check) now resolve their `skills/*/references/*.md` coverage grep against `origin/main`'s own tree (`git fetch origin main` then `git grep ... origin/main --`), never the invoking checkout's working files — a worktree-isolated session branched before a covering reference merges upstream (here `teamwork/skills/fleet-rules/references/substrate-choice.md`, PR #707) could not see it and re-queued the same false positive every firing. `adr_queue.py` gains `citation_on_ref(term, ref, pathspec, cwd)`, the shared ref-resolving primitive both steps cite, plus a `selftest` fixture built against a real throwaway git repo: a term covered on `main` but absent from a stale sibling branch's own working tree reads as covered (the regression itself), a term covered nowhere still reads as uncovered (negative control), and an unresolvable ref degrades to a plain no-hit rather than raising. Fresh-context `harness:skill-checker` pass on the watch-adrs SKILL.md edit per plugin-authoring.md's semantic-edit invariant.
v3.16.4 · 2026-08-19 · closes #738, closes #739 (combined build, agent-ui 2026-08-18T15:20Z stale-regurgitation incident): `chore_sweep_apply.mjs` gains a `--firing` staleness guard — a payload predating its target's last commit is refused by name (exit 1, other blocks still apply), a target with no git history yet fails open and discloses that in stdout; `chore-sweep.js`'s Plan-phase dispatch now mints a per-firing key, states it in the planner prompt and seat bundle, and flags (never throws) a returned plan whose Dispatch stamp doesn't match it. Fresh-context `harness:wording-checker` pass on the chore-sweep.js prompt wording per plugin-authoring.md's semantic-edit invariant.
v3.16.3 · 2026-08-19 · closes #733 (one of three pre-existing routing residuals from #720's full-suite re-judges, resumed post-#732-merge): `check-state`'s `--fleet` drift-check clause gains trigger-voice phrasing matching t12's own vocabulary — "has a fix landed in another repo yet, or is it still on the old cached copy" — disambiguating its cross-repo drift REPORT from `plugin-install-facts`' t14 (a person's own single-install update-lag troubleshooting), which was stealing it 3-of-3. Reciprocal NOT-fence + negative case (n10) added to `plugin-install-facts`. Rebased onto `origin/main` post-#732-merge (`--onto` past the now-redundant `bug/720` commits, whose own content is already the v3.16.2 line below); re-slotted 3.16.2 (this branch's original claim, taken before #732 merged) -> 3.16.3, the next-free slot off main's post-merge v3.16.2.
v3.16.2 · 2026-08-19 · closes #720 (routing fence + t07 ruling, sweep #676): `prompt-wording-rules`' description gains "NOT improving a BRIEF's ask/content, even when it's called a 'prompt brief' (find-intent)" (n05 leaked 3-0, matching the eval's own "Improve this prompt brief" vocabulary against the existing generic WHAT-fence). `check-state`'s t07 eval ("What changed since I last checked in on this repo?") expectation flipped no-trigger→trigger, superseding the 2026-07-30 cede to what-shipped (PR #121) — "since I last checked in" is checkpoint-relative, matching check-state's own "Delta since last run" bullet, not what-shipped's calendar-window axis (its own boundary line cedes "current work-state...a point-in-time snapshot" to check-state and claims only "a window of activity" for itself); no reciprocal case existed in what-shipped's own suite. Re-slotted twice post-merge: the original claim (3.16.1) collided in VALUE with #734's own 3.16.1 (research-leader/fact-finder disjointness note, closes #721, merged after this branch's first rebase) — takes 3.16.2 rather than re-shipping that number. Fresh-context `harness:wording-checker` pass on the prompt-wording-rules edit per plugin-authoring.md's semantic-edit invariant.
...
v3.15.3 · 2026-08-18 · closes #669 (post-#433 rename-retirement sweep, harness slice; teamwork + authorkit slices same PR): `agent-writing-rules` split-fixed one line the sweep's fresh census surfaced as mixed historical/live — the three-piece pattern's "Live instance" pointer now names the current file `teamwork/agents/build-leader.md` (was `build-lead.md`, stale since #433's rename), and the failure-catalog row's trailing live-fact clause now reads "`teamwork:build-leader`'s agent body ... states this explicitly" — while the SAME row's dated incident citation ("four measured `teamwork:build-lead` incidents: #257, #282, #269, #280") is left untouched, since those incidents genuinely happened to the agent under its then-current name. Scope note: harness was not named in issue #669's teamwork/authorkit slot list, but the fresh census surfaced this one genuine live-fact hit here that was not an excluded historical incident citation, so harness is a third touched plugin this PR, named explicitly. Pure mechanical/split-site find-replace, no surrounding instructional wording rewritten; no fresh-context checker dispatched per `plugin-authoring.md`'s semantic-vs-mechanical calibration — this skill's own description was not touched, so no evals/`/check-routing` obligation applies. `harness/skills/big-change-git-rules/references/who-ships-what.md`'s two `build-lead` mentions were deliberately left untouched (dated `[incident, 2026-08-14]`/`[verified, ...]` log entries describing what was true when observed, not live pointers).
v3.15.2 · 2026-08-18 · closes #671 (Track A, harness slice; teamwork slice same PR): `agent-writing-rules` gains two adoptions from the folded #671 answer — a "Fan-out contracts" section (bounded rejection: a contract-violating worker output is re-dispatched ONCE then flagged UNMEASURED, never hand-patched in the lead's own context; synthesis-budget doctrine: merge-cheap row-shaped output contracts, 5–10-worker batch sizing, chunked merges past ~10 workers), scored by `fleet-rules`' A2-R6/A2-R7 rubric criteria; and a new "Workflow guidance (interim home)" section carrying the `acceptEdits`+inherited-allowlist permission-model note for `Workflow`-tool `agent()` dispatches, scored by A7-R6 — ruled interim since `#666`'s G2 fold-in shipped a lint tier for `workflows/*.js`, not a dedicated workflow-guidance skill. Harvested substance (never authority) from an external agent-classes doc. Body-only edit, no description/evals change. Fresh-context `skill-checker` pass.
...
v3.9.4 · 2026-08-17 · watch-tickets' Scope section repoints its REQ-001..013 citation at the skill's own inline Procedure section instead of `.claude/docs/spec/spec-ticketing-watch-triage.md` — that path is a workspace-side record never shipped with this plugin, so it dangled in every other install target (closes #582, gen-ui-kit portability finding, operator ruling 2026-08-17). No behavior change; the operative contract was already carried inline in the Procedure steps.
v3.9.3 · 2026-08-17 · github-facts pr-lifecycle gains two storm-verified facts: a closed PR force-pushed becomes unreopenable-422 (supersede with a fresh PR), and REST outlives GraphQL in 503 storms with storm-time mergeability answers being stale (agent-ui PR #1123→#1128 instance).
v3.9.2 · 2026-08-17 · Ops rulings round 2: `watch-adrs` records the supersession contract — the formal `supersedes:` marker (or the one named active-voice body clause) is what the classifier honors, prose-only claims are deliberately out-of-contract (adr-0048 missed by design, adr-0071→adr-0043 via the marker classified cleanly); `watch-tickets` gains a ruling-shaped classification routed to the `needs-ruling` label, referenced (not restated) by `chore-planner`'s §3; `clean-git` names `.claude/ops/state-checkpoint.json` as repo-cleaner's checkpoint home where no prior claim exists.
v3.9.1 · 2026-08-17 · Ops rulings: `sweep-chores` gains a dispatch-time concurrency guard (`scripts/sweep_guard.mjs`, selftested) declining a second firing while an earlier one's marker is fresh (30min staleness window; the 18:45Z-vs-18:55Z duplicate firing, decision-watcher pre-empted); `campaign_close.py`'s C5 reused-head check proven to already cover the gen-ui-kit #1479→#1568 branch-succession class via a new selftest fixture, no code change needed.
...
unrelated to this merge). (Rebumped past 3.8.38: PR #564's G12b companion shipped that slot.)

v3.8.38 · 2026-08-17 · `release_gate.py` gains G12b, a bundled-manifest parity check (closes #562): a plugin's own bundled `naming.manifest.json` (today: authorkit's self-dogfood copy) is re-validated BARE (`--scope grammar` explicit) so a vocab entry the plugin's own names need but the bundled copy never mirrored fails the gate (the drift PR #560 fixed by hand, twice). Selftest gains a BUNDLED_TRIGGER negative-control leg.
...
v3.8.34 · 2026-08-17 · `harness/workflows/chore-sweep.js` fixed to conform to the Workflow tool's contract (closes #529): dropped the wrapper function and trailing `export default` causing the live `SyntaxError: Unexpected keyword 'export'`; proven by static contract-conformance only, live Workflow-path proof rides the next sweep firing (`sweep-chores` paragraph updated to match); eslint excludes `**/workflows/*.js` (loader-owned contract, not this repo's JS convention).
...
default (report-before-idle, report-supersedes-nudge, idle pings as no-ops); no
description/frontmatter change, no evals impact. Numbered 3.8.29, not 3.8.27, per this repo's own
version-slot rule: two other open PRs already claimed harness's next two slots (#493 → 3.8.27,
#496 → 3.8.28), verified via `version_claim_check.py` immediately before this PR opened.
