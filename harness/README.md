...
v3.8.31 · 2026-08-17 · ops seat-definition fix batch (closes gen-ui-kit#1484): `ops-write-sandbox-rules` states the payload-fence rule binds regardless of hedge language and across cross-file citations; `clean-git`/`repo-cleaner` resolve the HOST repo's own ticket-claim ruling dynamically (this plugin's ADR-0005 is a named default, never a fixed id assumed to travel — gen-ui-kit's ADR-0042 rules the identical convention) and cite host-repo plan/backlog items by content, never a bare section number; `sweep-chores`' fanned-out dispatches and its own relay report state scope, named exclusions, and a UTC timestamp explicitly — timestamps are the standing sequence key, never an incrementing sweep count.
