...
v3.8.2 · 2026-08-16 · `agent-writing-rules` gains a Failure-catalog row for the nested-dispatch-and-wait stall (issue #310, teamwork's companion fix): a `context: fork` skill or a further NAMED `Agent`-tool dispatch invoked from inside an already-dispatched agent completes to the ROOT session, never back to the dispatching agent (the fork-from-agent finding, 2026-08-10, intake-lead A4) — cites the four measured `teamwork:build-lead` incidents (#257/#282/#269/#280) and both fixed teamwork artifacts (`dispatch-ticket`'s procedure text, `build-lead`'s own agent body)
