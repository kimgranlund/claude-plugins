...
v3.7.3 · 2026-08-16 · `checking-rules` gains the generator≠critic invariant's UNIT calibration (issue #272): semantic edits earn a fresh-context critic dispatch at any diff size, mechanical edits (ledger-line trims, version renumbers) ride floor-tier verification — evaluated per build/PR slice, not per touched file; `agent-writing-rules` gains a checker-seat consolidation merge test (identical parameterizable mechanics, never shape alone), citing #293 as the one confirmed candidate and `agent-checker`/`hook-checker`/`plugin-checker` as the negative proof case; CLAUDE.md's standing invariant line points at the new checking-rules section for the unit
