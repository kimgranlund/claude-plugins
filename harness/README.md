...
v3.8.18 · 2026-08-16 · `chore_sweep_apply.mjs` entry guard: the `import.meta.url === \`file://${argv[1]}\`` string-compare was silently false under an install path containing a space (Cowork's `~/Library/Application Support/...` — the URL side percent-encodes, argv does not), so `main()` never ran and the process exited 0 with no output (seen live in an agent-ui `/sweep-chores` fallback). Now compares `realpathSync(fileURLToPath(import.meta.url))` to `realpathSync(resolve(argv[1]))` (realpath: macOS `/var`→`/private/var`); selftest gains a spaced-path negative control. Audit: no other harness script has the pattern; screens' `ui-probe.mjs` is space-safe but symlink-fragile (noted only).
