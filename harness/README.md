...
| `skills/ops-write-sandbox-rules` | Declarative skill | model-only | The `.claude/ops/` write-sandbox contract (issue #125, canonicalized issue #205): why the four compute-only ops seats carry no `Write` tool, the fenced target-pathed payload shape, and how `chore-lead` applies it — one statement, preloaded by all five ops-family agents instead of restated 90-110 words per file |
| `agents/chore-lead` | Agent | spawned (on-demand) | Standing sweep coordinator for the ops-* family: fans out decision-watcher + issue-sorter + repo-cleaner in parallel (or a scoped subset), hands the returned handoffs to chore-planner, relays the queue plus per-seat status (returned · UNMEASURED · refused); Read+Task allowlist — coordination is its entire surface, a zero-return sweep never dispatches the planner. Preloads `write-handoff` + `ops-write-sandbox-rules` + `agent-writing-rules` (cites its Failure catalog for bare-name dispatch/gh#154 and named-teammate misaddress/gh#157 instead of re-deriving them) |
...
v3.4.0 · 2026-08-13 · new preloaded-only `blocked-by-rules` skill (issue #193, disable-model-invocation: true, no eval suite) gives `chore-planner` the read/order steps for teamwork's `Blocked-by: #NN` ticket convention — agent stays thin-shell, cites the skill rather than absorbing the logic
