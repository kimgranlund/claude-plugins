...
v3.8.18 · 2026-08-16 · `chore_sweep_apply.mjs` entry guard: `import.meta.url === \`file://${argv[1]}\`` was silently false under an install path with a space (Cowork's `~/Library/Application Support/...` — URL side percent-encodes, argv does not): `main()` never ran, exit 0, no output (seen live in an agent-ui `/sweep-chores` fallback). Now compares `realpathSync(fileURLToPath(import.meta.url))` to `realpathSync(resolve(argv[1]))` (macOS `/var`→`/private/var`); selftest gains a spaced-path negative control. No other harness script carries the pattern; screens' `ui-probe.mjs` is space-safe, symlink-fragile (noted only).
