...
|---|---|---|
| Mechanically checkable rule | You can write the pass/fail function ("no raw hex in `src/ui/**`") | **Hook** (`/hook-forge`); prose deleted, pointer optional |
| Task procedure | Steps, workflows, "when doing X, first…" | **Skill** (`/skill-forge`); loads only when used |
...
- **Salience is a budget.** A wall of bolded MUSTs normalizes to noise; three plain sentences in a dedicated block outlast thirty scattered warnings. Structured emphasis over typographic shouting (`linguistic-techniques` §8).
...
|---|---|---|
| Growing dotfile | Monotonic additions, no eviction | `/entry-file-audit` on a cadence; residency test per line |
