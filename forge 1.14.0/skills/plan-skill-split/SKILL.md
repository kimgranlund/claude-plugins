...
# plan-skill-split — decide the family, don't assume it
...
growing each child pack is `/make-pack`'s (research waves per axis; `/make-skill` covers each
child's SKILL.md surface), and the move/rewire/prove pass is a
refactor executed by `/reshape-skill` from the validated manifest. The method below is the one the source corpus ran
...
3. **Vocabulary separability** — do candidate children have orthogonal trigger tokens, or do they
   ...
   `/check-routing`'s *stolen* failure shape, measured)? `color-contrast-facts` survived at only
...
4. **Cost ledger** — only price this once 1–3 clear. Each new child costs: a description (≤1024
   ...
   this is the blast-radius map `/reshape-skill` will need). Net the total against an
...
3. **If 1–3 clear**, propose N candidate packs: one line each (identity, file range, axis list, one
   ...
   posture deliberately (skill-writing-rules' dials — both, explicitly) — never leave it at
...
6. **Hand off, never execute.** The manifest + repair map go to `/make-pack` (mint/grow each
   child's references/INDEX, wave by wave) and `/make-skill` (each child's surface) and `/reshape-skill` (move, rewire every referrer, prove the sweep — from this manifest). For high-stakes splits, the `skill-checker` agent scores the
...
- **This skill decides and designs; it never moves a file.** Authoring a child's actual
  references/INDEX is `/make-pack`'s; executing the move, fence
  transfers, and referrer rewire is a refactor executed by `/reshape-skill` from the validated manifest.
- **Not a general decomposer.** A UI layout, technical architecture, or goal breakdown is
  `break-down-problem`'s domain table — this skill is specific to knowledge-pack corpora being
  tested for a knowledge-family split.
- **Not its own inverse.** Consolidating several existing skills into one is `plan-skill-merge`.

## Routing

| Peer | For |
|---|---|
| `plan-skill-merge` | the inverse operation — merging several packs into one |
| `break-down-problem` | a non-knowledge-pack decomposition (layout, architecture, goals) |
| `check-everything` | the corpus-wide sweep that surfaces strained packs in the first place |
| `make-pack` | minting or growing each resulting child's corpus |
| `reshape-skill` | executing this skill's manifest — moves, rewires, sweep proof |
| `make-skill` | authoring each resulting child's SKILL.md surface |
| `check-skill` | scoring a child skill once authored |
