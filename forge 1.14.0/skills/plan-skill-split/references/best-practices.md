...
*(Canonical source docs cited below are archival — they live in the source corpus, outside this
plugin; the method lessons here are self-contained.)*

Both cases below are dated, verified deliverables from the source corpus's own history — not
...
Canonical source (archival): `~/.claude/docs/color-science-extraction-plan.md`.
...
- **Sizing** — not straining: 28 files / 4 axes is inside the healthy range, the same scale as
  `ui-pattern-facts`, the source corpus's own model of a single healthy pack. No INDEX bloat, no
  load-discipline pressure, no count drift.
