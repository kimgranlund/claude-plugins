...
verifier/binding, and pack/orchestrator batches (36 claims reconciled across four batches). Owner: [[check-all-skills]]. The per-skill floor
remains `skill-writing-rules`; this document is the corpus-level ceiling: what a skill
...
## Scoring rules (the procedure lives in [[check-all-skills]])

How a campaign runs — context assembly, batching, dispatch, the ledger, fix waves — is owned by
[[check-all-skills]]'s deep-review mode; one canon per fact: procedure there, dimensions here. A review
...
## N · Naming rigour (gates — canon: `skill-writing-rules`'s own naming section)
...
- **A4 Mechanization — species-shaped.** Everything mechanizable routed to a **selftest-locked**
  ...
  (check-all-skills) plus the harness D9 in-bundle pointer gate (relative markdown links in every
  ...
  **inversion fixtures** (prove the wrong leg/state is detectable, not merely that a good input
  ...
  the mechanization test itself — is canon in `script-writing-rules` (2026-07-14); this
...
- **S3 Vocabulary coherence.** One language, used by name: `[gate]`/`[review]` (rubric tiers) ·
  ...
  **eval** (a repeatable metric over a labeled corpus — one of the four registered instruments;
  canon: the instrument registry in `skill-writing-rules`'s own naming section). Axis names (A/B, outside-in/inside-out, "two planes") are OVERLOADED across the
...
The campaign ledger is sharded per batch — `check-all-skills/campaign/batch-N/<skill>.findings.jsonl` —
