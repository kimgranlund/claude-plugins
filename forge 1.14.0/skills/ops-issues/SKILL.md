...
name: ops-issues
description: >-
  Run the standing intake/triage seat (agents/ops-issues.md) on demand against this repo — a plain
  ...
  surfaces one pending. Run /ops-issues [blank, or an instruction].
...
Dispatches the standing `ops-issues` agent (`agents/ops-issues.md`) for an on-demand run against
...
(`agents/ops-issues.md:69-70`) — owns running the first-run interview `spec-ticketing-watch-
...
1. **Dispatch.** Call the `ops-issues` agent (Agent tool, `subagent_type: "forge:ops-issues"`),
...
```
ops-issues — standing intake/triage seat for this repo's features, bugs, tasks, issues, and PRs.
