...
# agents-audit — the team as one system
...
whole). It audits, it never fixes: `agent-authoring-standards`/`agent-forge` repairs one agent;
`skill-refactor` executes renames or the concept-folder moves the audit's findings imply.
...
- **Own vs delegate.** N and S are owned here; L defers to `linguistic-techniques` and F to
  `agent-authoring-standards` — cite the owning standard, never re-derive it.

1. **Index (Phase 0):** `python3 "${CLAUDE_PLUGIN_ROOT}/skills/agents-audit/scripts/agent_corpus_index.py" <agents_dir> [skills_root ...]` —
...
2. **Per-agent fan-out (Phase 1):** dispatch `agent-reviewer` (F — against the agent-authoring-standards rubric
   + harness) and `linguistics-reviewer` (L — does the body instantiate or merely describe?).
...
6. **Fix wave, severity-ordered:** N/M gates → S2 boundaries → A3 drift → other A/L → polish; a
   ...
   fixes (`agent-authoring-standards`/`agent-forge` and the finding's owner) — this skill never edits what it judged.
...
8. **Verdict the portfolio:** KEEP / MERGE / SPLIT / RETIRE / RE-CHARTER per agent; missing seats
   ...
   `skill-refactor` executes the restructures.
...
| `${CLAUDE_PLUGIN_ROOT}/scripts/harness_checks.py` | M1 gate — same script `skill-auditor`'s siblings use, `agent` mode |
...
| `agent-authoring-standards` / `agent-forge` | The per-agent standard — and the tool to FIX what either mode finds |
