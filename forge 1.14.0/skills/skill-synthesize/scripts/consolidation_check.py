#!/usr/bin/env python3
"""Validate a skill-synthesize manifest: reconciled consolidation, description budget, delta stated.
...
The counterpart to skill-decompose/scripts/manifest_check.py — same checker-as-gate convention.
"""
import json
import sys
...
def check(manifest):
    ...
    if len(axes) < AXIS_MIN or len(axes) > AXIS_MAX:
        warns.append(f"target_pack has {len(axes)} axes, outside the healthy "
                    ...
                    "skill-decompose before shipping (D4): the merge may just be a relabeled monolith")
...
def report(fails, warns, label):
    print(f"skill-synthesize consolidation check — {label}")
    for w in warns:
        print(f"  [WARN] {w}")
    for f in fails:
        print(f"  [FAIL] {f}")
