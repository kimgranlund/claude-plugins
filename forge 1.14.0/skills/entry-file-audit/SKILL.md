...
description: >-
  ...
  /entry-file-audit [path, default ./CLAUDE.md]. Human-timed; edits files on approval only.
...
entry-file-audit slims an entry file without losing a single behavior: every evicted line gets a landing artifact before the prose is cut. Target: `$ARGUMENTS` (default `./CLAUDE.md`, plus `.claude/rules/` if present).

Invoke `entry-file-standards` now — its routing table is the classifier; it is not restated here.
...
```
entry-file-audit · <path> · N lines → keep K · hooks H · skills S · rules R · cut C
...
- `→HOOK` — draft via `/hook-forge` (or extend an existing script); the prose is cut only after the hook's selftest passes and it is registered.
- `→SKILL` — route to `/skill-forge` for a new skill, or land the lines in the owning skill's body; cut after the skill lints clean. A one-line pointer may replace the block when discoverability warrants it.
