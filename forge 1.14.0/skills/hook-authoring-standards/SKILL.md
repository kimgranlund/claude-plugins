...
Judgment (skill): "does this body carry a genuine behavior delta?"       → skill-review R1
```
...
A hook's output lands adjacent to the model's next action — the highest-recency, highest-potency position in the whole system (`linguistic-techniques` §9). Most teams spend it on `exit 1`. Spend it as a **repair affordance**: the hook's name (handle), `file:line` evidence, each fix stated affirmatively, and a named branch for disagreement.
...
Event set, output semantics, layering, and the wrapper trap verified against code.claude.com/docs (hooks, plugins-reference) and this plugin's own build incidents, 2026-07. All mechanics [drift-prone]: re-verify on a version bump. Message language: `linguistic-techniques` (§5, §9, §10). Routing: `skill-authoring-standards` carries the skill side of the boundary.
