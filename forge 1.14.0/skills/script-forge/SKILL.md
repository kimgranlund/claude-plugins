...
description: >-
  ...
  script-authoring-standards, validate (selftest green, negative control bites, wired into its
  ...
  for the standard or the bare is-this-mechanizable question (script-authoring-standards owns the
  test; this workflow runs it on the way to writing); NOT for event-fired enforcement — blocking
  an action on a lifecycle event is a hook (hook-forge); NOT for a whole new skill around the
  script (skill-forge); NOT for capturing prose knowledge (knowledge-harvest).
...
script-forge turns a hand-run check into a bundled script that proves itself — the deterministic
tier's maker, pairing with `script-authoring-standards` the way hook-forge pairs with its
standard. Invoke `script-authoring-standards` now; the anatomy, placement, and selftest contracts
...
2. **A prose checklist re-followed by hand** — the same steps executed manually a second or third
   ...
   knowledge-harvest uses, applied to procedures).
...
`script-authoring-standards`, filled for this candidate — never re-derived.
...
the user's ask WAS the explicit instruction ("mechanize this as a script", "/script-forge <x>") —
...
`script-authoring-standards` references/selftest-patterns.md). Wire the caller in the same
