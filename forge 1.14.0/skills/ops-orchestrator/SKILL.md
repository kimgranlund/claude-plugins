...
description: >-
  Run one ops-family sweep on demand against this repo — dispatches the standing ops-orchestrator
  agent (agents/ops-orchestrator.md), which fans out ops-adr + ops-issues + ops-repo in parallel
  and returns ops-planner's single prioritized queue. States the agent's operating contract as a
  fixed banner before the first ops queue exists here. Run /ops-orchestrator [blank, or a scope
  instruction].
...
Dispatches the standing `ops-orchestrator` agent (`agents/ops-orchestrator.md`) for an on-demand
sweep — the same skill↔agent same-name pairing ruled for `/ops-issues`, whose body records the
...
2. **Dispatch.** Call the `ops-orchestrator` agent (Agent tool, `subagent_type:
   "forge:ops-orchestrator"`), carrying `$ARGUMENTS` verbatim as the dispatch context — blank for
...
```
ops-orchestrator — one-pass sweep coordinator for this repo's ops-* seats (intake · ADR review · repo hygiene).

What it does: fans out the three standing seats in parallel, then ops-planner turns their reports
...
- `$ARGUMENTS` carries one seat's own job with no sweep intent (e.g. "file this bug" — intake, or
  "delete that stale branch" — hygiene) → name the direct door (`/ops-issues`, `ops-repo`,
  `ops-adr`) and do not dispatch; a sweep that exists to wrap one seat's single task is fan-out
