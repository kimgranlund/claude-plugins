...
# skills-audit — the corpus as one system
...
`skill-authoring-standards`/`skill-forge` repairs one skill; `skill-refactor` executes the
restructures.
...
- **Own vs delegate.** N and P are owned here; L defers to `linguistic-techniques` and F to
  `skill-authoring-standards` — cite the owning standard, never re-derive it.

1. **Index (Phase 0):** `python3 "${CLAUDE_PLUGIN_ROOT}/skills/skills-audit/scripts/corpus_index.py" [skills_root]` — read its output before
   dispatching anything.
2. **Per-skill fan-out (Phase 1):** dispatch `linguistics-reviewer` (L — does the body instantiate
...
3. **Dispatch a fresh-context critic per skill** (`skill-auditor` at DEEP depth; generator ≠
...
| `skill-authoring-standards` / `skill-forge` | The per-skill standard — and the tool to FIX what either mode finds |
