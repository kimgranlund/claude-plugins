#!/usr/bin/env python3
"""Validate a skill-decompose manifest: reconciled file mapping, description budget, axis sizing.
    ...
    for i, pack in enumerate(packs):
        ...
        if len(axes) < AXIS_MIN or len(axes) > AXIS_MAX:
            warns.append(f"pack {name!r} has {len(axes)} axes, outside the healthy "
                        f"{AXIS_MIN}-{AXIS_MAX} range (knowledge-author doctrine) — verify this is deliberate")
...
def report(fails, warns, label):
    print(f"skill-decompose manifest check — {label}")
    for w in warns:
        print(f"  [WARN] {w}")
    for f in fails:
        print(f"  [FAIL] {f}")
