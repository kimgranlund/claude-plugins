...
All research for this pack ran 2026-07-17 via six parallel `fact-finder` dispatches, one per
...
## Grounding-marker legend (this pack's convention, adapted from `big-change-git-rules`)
...
before being trusted on any `[drift-prone]`-marked claim — per `pack-writing-rules`'
