...
- **skill-forge** + **skill-authoring-standards** own the SKILL.md's *structure and standard* today; future forge families (agents, hooks, entry files) will own theirs. Each artifact author owns structure; this skill owns the *language inside it* — run it as the language pass of their Evaluate/Rewrite (skill-forge Phase 4 does exactly this).
