...
- **Recommended next action** — exactly **one** best next step **and its owner**. "system-planner designs the error-vocab ADR" routes; "various follow-ups remain" does not. If the work is done and verified, say so plainly and name the owner of the commit.

## Per-seat notes

- **system-planner** — *Files changed* = the docs authored (PRD / SPEC / LLD / ADR). *Tests/checks run* = `harness_checks.py <type>`, `coverage_check.py`, `trace_check.py`. *Recommended next action* often = "ratify ADR-NNNN, then dispatch the build".
- **system-builder** — *Files changed* = code / scaffold / tests. *Tests/checks run* = `npm run check && npm test` (+ any browser gate). Map each DoD proof point to its test name in *Evidence*. Surface a discovered constraint as an *Open question* / *Risk* rather than editing the contract.
- **orchestration-coordinator** — a **rollup**: the same eight fields aggregated across the team (*Status* = the worst seat's status: one blocked seat makes the rollup `partial` or `blocked`). *Recommended next action* = the dispatch-or-ratify decision the host acts on.
...
- **reviewer seats** (doc-reviewer, skill-reviewer, agent-reviewer, linguistics-reviewer, component-/layout-/orchestration-reviewer) — *Files changed* = `(none)`: a critic grades, it does not build. *Evidence* = the gap-map's citations (file:line or quoted text per scored dimension, gate verdicts first). *Recommended next action* = "maker applies the fix" — the maker is the recipient-owner.
- **token-builder** — *Files changed* = the token files (`tokens.css` / `dimensions.css`). *Evidence* = the WCAG-AA + forced-colors result across *both* schemes, and that no role ladder collapsed to one step.
