...
paradigm governs NEW names — new estates, new plugins, newly minted members. Shipped names are
APIs; changing one is a deprecation campaign (ADR + branch + PR), not an effect of this skill.
...
- **Reserved-word escape**: the name points at the artifact, never the platform —
  ...
  skill-authoring-standards' F8 row).
- **Term-of-art exception**: a term users type verbatim outranks the shape grammar — `llms-txt`,
  `a2a`, `ui` stay as-typed (rule owned by skill-authoring-standards, 2026-07-15 amendment).
...
Bad (counter-example — do not imitate):
  skill: ops-issues        agent: ops-issues
...
motivated each rule and the rows still marked open. It is review output demonstrating the
paradigm at scale — not a ratified rename plan.
