...
name: eval-judge
description: |
  Blind routing judge for /eval-run. Dispatched only by that command, one judge per eval suite,
  ...
  <example>
  Context: /eval-run Phase 3 fan-out.
  user: "/eval-run ."
  assistant: "Dispatching one eval-judge per suite with the menu and that suite's shuffled prompts."
  <commentary>
