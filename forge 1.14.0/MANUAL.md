...
- **Agents `eval-judge` and `pack-researcher`** — dispatch-only, declared because their tool allowlists ARE the contract: the judge has no tools (structural blindness for /eval-run), the experiment-runner can't Edit (gather≠distill for /pack-forge). All other parallel work self-spawns ad-hoc, per the fork-vs-agent gate.
