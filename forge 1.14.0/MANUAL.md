...
- **Agents `ops-issues` and `ops-repo`** — the estate's standing operational seats, a different character from the review agents above: fired on a schedule (CronCreate) or dispatched on demand, not dispatched by a review workflow. `ops-issues` implements the watch/triage/trust SPEC to intake and route work items, gated by a durable friendlies allow-list; `ops-repo` inventories and cleans repo hygiene, executing ONLY through this plugin's own gated scripts. Neither ever edits source or merges/deletes outside its own narrow, script-backed allowance.
