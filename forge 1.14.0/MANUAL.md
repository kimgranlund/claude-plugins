...
- **Agents `agent-reviewer`, `hook-reviewer`, `plugin-reviewer`, `linguistics-reviewer`** — the same fresh-context pattern as `skill-auditor`, one per remaining artifact type: each preloads its matching `*-authoring-standards` (or `linguistic-techniques`) plus `handoff-compose`, gates on `skill_lint.py`'s or `release_gate.py`'s real rule codes, and returns a severity-ordered gap-map. Dispatch after authoring or editing the matching artifact type; you rarely call them directly.
...
- **Scripts** (`scripts/`, all with `selftest` modes): `skill_lint.py` (F/W/A/H/C/P/E/K rules), `release_gate.py` (G1–G7), `eval_check.py` (suite schema + coverage), `corpus_check.py` (pack INDEX reconciliation), `docs_check.py` (README/MANUAL freshness, gate G10). Per-skill: `manifest_check.py`, `consolidation_check.py`.
- **Eval suites** (`skills/*/evals/evals.json`) — trigger-routing regression tests for all eleven model-invocable skills; the example prompts in this manual are drawn from them. Execute via `/eval-run`.
