...
v1.14.0 · 2026-08-17 · ADR-0020 wave 5 (closes #523, docs slice — riding alongside teamwork's rather than a separate PR: the retiring `LEAD_HEAD_GRANDFATHER` constant grandfathers this skill too, so splitting the PR would leave a real gap between merges where estate-wide naming-audit fails): `skills/lead-intake/` → `skills/bind-intake/` (`name: bind-intake`, already skill-as-command shaped per #525 — no wrapper conversion needed, just the rename). Citers repointed: `agents/intake-leader.md`'s own NOT-fence, this README's live table row (row 18) and `agents/intake-leader`'s twin-reference row (row 28). Ledger/ADR/audit-report history of the old name left untouched.
v1.13.1 · 2026-08-17 · closes #541: `file-bug`/`file-feature`/`file-task` Phase 2 corrected — a `context: fork` background dispatch has NO `AskUserQuestion` channel (measured live, falsifying the 2026-08-09 "forking does not remove the person" claim). Uniform degrade: capture-with-gaps names the unasked question(s) in the record AND the mint close-out, plus a resume command that folds an answer in via each skill's Phase-1 fold-in (now unconditional/first). file-feature's Phase 6 index-offer fixed the same way. Three fresh-context skill-checker passes; no evals touched. Details: `docs/skills/file-*/SKILL.md`.
