...
v1.11.0 · 2026-08-16 · issue #433: `product-authoring` skill full-moved to `teamwork/skills/leading-product` (Kim's ruling); `docs/agents/product-leader-agent.md` stays put, agent-file leg blocked on a grammar-ADR. `product-lifecycle-rules`/`check-stage`/`doc-writing-rules` stay canonical here, already soft-mentioned not preloaded, no boundary fix owed. teamwork bumps to 2.17.0 in step.
