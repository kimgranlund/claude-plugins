...
v1.10.2 · 2026-08-16 · remove product-forge runtime mentions, supersedes #404 Addendum 2 (closes #411): `cold-start-adoption.md` step 4 rewritten native — `docs:doc-checker` fresh-context review replaces the `product-forge` soft mentions; flow is now orient→harvest→draft→review→ratify, no conditional plugin path; `product-leader-agent.md` updated to match. Study-pass prior-art note added, provenance-cited, never runtime. Details in issue #411.
