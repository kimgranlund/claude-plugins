...
| `scripts/doc_lint.py` | Script | CLI + selftest + gate G10 (PostToolUse hook retired 2026-08-17, #466 — remove-all-hooks directive) | T1-T5: frontmatter/type/status/sections/ID-spine; the accepted-ADR append-only rule (T4) is now enforced only at gate time, not on every write |
