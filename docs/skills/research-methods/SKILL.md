...
description: >-
  ...
  parameters", "what breaks this", "it used to work, now it doesn't", "stress-test this before
  we ship". NOT lookup (web search); NOT artifact review (*-checker agents); NOT
  authoring/building (*-forge/builder); NOT judging whether a rule or reasoning-depth itself is
  wrong absent a scorer loop (harness:thinking-depth-rules).
