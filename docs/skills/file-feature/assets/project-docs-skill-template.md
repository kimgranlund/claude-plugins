...
description: >-
  ...
  editing a document (/make-doc, docs); NOT for capturing a new feature idea (/file-feature) or bug
  (/file-bug); NOT for building from a record (/build-feature, orchestration).
...
"spec-shaped content lives at rfcs/ — not indexed; /tidy-docs (docs) can migrate it." A false
...
4. Route all making: a new idea → `/file-feature`; a bug → `/file-bug`; building a queued record →
   `/build-feature`; authoring or revising any document → `/make-doc` (all where installed — otherwise
