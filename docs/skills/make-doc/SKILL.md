...
before minting
one, same as IDR's own gates. **Same round, every type: the document's `scope:` grain (for the
grain-governed types — doc-writing-rules' table) and its `audience:` seat-class(es) (one-or-more,
required — no any-agent default)** — decide both now, not as an afterthought at draft time;
`doc_lint.py`'s T12/T13 FAIL a new mint that skips either.

Consult docs' `agent-harness-rules` for how to choose the assert layer or design the harness —
