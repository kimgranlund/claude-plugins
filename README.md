...
| [`agent-protocols`](<agent-protocols/README.md>) | Knowledge packs for the A2UI four (wire protocol, renderer, catalog design, training corpus) and the A2A four (protocol, agent design, isolation proofs, training). |
...
| [`design`](<design/README.md>) | Color science and OKLCH palettes, the house typography system, Material Design tokens, N-variant design exploration (make-variants), and the design-system export bundles (Claude Design/Figma Make/Google Stitch). |
| [`frontend`](<frontend/README.md>) | UI structure (layouts, flows, components, patterns), the design/UI knowledge layer (spacing-scale theory, CSS box-model/flow mechanics, Apple HIG semantics, motion), and non-functional verification (focus, i18n, perf, safety, live-artifact checks). |
