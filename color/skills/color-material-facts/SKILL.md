...
# color-material-facts — pigment, print, and the names for color
...
Its siblings [[color-space-facts]] (space-conversion math), [[color-perception-facts]]
(vision/appearance science), and [[color-contrast-facts]] (contrast/CVD) answer the rest
...
| Subtractive-mixing mechanism under an aesthetic effect | INDEX §Cited-from-color-perception-facts (huevaluechroma ch05/ch06 — the mixing physics is cited, never duplicated) |
...
3. Check the source-tracing rule before answering: every claim traces to a reference file; an
   ...
   [[color-space-facts]]'s; if it turned into contrast numbers, [[color-contrast-facts]]'s.
4. Route output work at the boundary: build a ramp, theme, or palette from these colors →
   [[make-palette]]; judge harmony or mood → [[color-theory-facts]]; realize token layers in a repo →
...
- **This skill answers; it does not generate.** No ramps, no palettes, no token sheets — name the
  physical fact and its evidence, hand the making to [[make-palette]].
- **Space-conversion math, gamut, gradients, and CSS color syntax belong to
  [[color-space-facts]]** — this pack answers what a pigment or print process *does*, not how
  to convert or interpolate a color space.
- **Vision and appearance science belong to [[color-perception-facts]]** — this pack cites its
  huevaluechroma subtractive-mixing chapters (ch05/ch06) rather than re-deriving the vision-side
  mechanism.
- **Contrast standards and color-vision deficiency belong to [[color-contrast-facts]]** —
  never answer an APCA/WCAG/CVD question from this pack.
- **Harmony, mood, and meaning belong to [[color-theory-facts]]** — a naming or pigment-history answer
  should not drift into aesthetic judgment about whether the result "looks good."
- Verifying a candidate palette end-to-end (ColorProof) is [[check-colors]]'s job, not this
  pack's.
