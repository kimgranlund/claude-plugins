...
mechanized instance of (`make-component`'s `references/geometry-system.md` §"Composed padding —
...
— an icon centered in a square cell via `(height − glyph) / 2` (make-component's law) stops being
...
(`make-component`'s geometry system) splits into a **comfortable** band (buttons, inputs, menu
...
that region's density, applied once per boundary — consistent with how `break-down-layout` expects
...
- The *specific* numeric ramp for a specific component family — that is a maker skill's job
  (`make-component`'s law, or a bound Material export via `material-shape-facts`).
...
- A raw CSS mechanism for the gap itself (`gap` property vs. margin vs. `:where()` selectors) —
  the box-model/flow mechanics that realize a gap in the DOM are `dom-layout-facts`'s territory.
...
(make-component's one-scale-per-library rule).* Precedence: a
