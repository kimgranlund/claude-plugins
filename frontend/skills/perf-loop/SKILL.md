...
- `audit X: passing (1) -> missing from the after report` or `score seo: 91 -> missing`: the
  audit key or category is absent from the after report (a different config or Lighthouse
  version); a regression until the run is repeated with the same config. An audit still
  present but unscored is listed under Unchanged as `passing -> not applicable in after`, in
  both modes. The gate also prints both urls and form factors and warns when they differ.
