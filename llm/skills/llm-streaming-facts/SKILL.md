...
# llm-streaming-facts — streaming structured output safely
...
| Validate-then-stream — "never emit invalid structured output", "the self-correct retry loop", "halt-and-report", "feed failures back to the model", "no early token — the leading meta-line answer" | `references/validate-then-stream-self-correct.md` |
| The transport-agnostic stream abstraction — "one contract across SSE/WebSocket/EventSource", "backpressure policy", "EventSource can't send custom headers", "keep one NDJSON splitter", "one `turn(input)` seam, swap replay/live/peer backends", "pin the proxy's request-body fields" | `references/stream-abstraction-transport-constraints.md` |
