...
[[llm-gateway-facts]] in this same plugin. That pack covers, in full and with a real worked
...
`llm-gateway-facts`'s pattern to the new domain rather than inventing a new one from scratch;
...
stateless session/turn model themselves — all fully owned by [[llm-gateway-facts]] · the wire
format a service's response streams over once a call is already underway ([[llm-streaming-facts]])
