...
written into `llm:chat-harness-guardrail-facts` as if they were
...
## fences
- NOT for authoring the actual Ephemeral/chat-agent guardrail content itself (llm:chat-harness-guardrail-facts and its siblings own that)
...
4. `chat-harness-guardrail-facts`'s own SKILL.md carries a reciprocal named-mention pointer to this new skill.
...
P5 validate:   PASS — skill_lint.py + potency_lint.py both clean; skill-auditor FLOOR review PASS
               ...
               closure landed both directions (chat-harness-guardrail-facts/SKILL.md's
...
- skill-auditor FLOOR review (2026-07-20): PASS, no blocking findings. Fix-before-shipped F1
  (reciprocal pointer from chat-harness-guardrail-facts missing) routed to this
