...
| `agents/council-strategy-agent`, `agents/council-design-agent`, `agents/council-voice-agent`, `agents/council-creative-agent` | Agent | dispatched directly (the EXTERNAL seat for a fleet/session — unlike `brand-judge`/`council-chair-agent`, never invoked by an orchestrating skill) | NEW (`#840`) — one addressable role agent per lens sub-council. Each reads `check-brand-council/references/roster.md`, fans `brand-judge` out unnamed over ONLY its own sub-council's seated active personas, and returns one phase-1-only rolled-up read (findings verbatim + the five synthesis shapes scoped to its lens); an empty/VACANT bench reports "no seats" and stops. `council-creative-agent` convenes the new `creative` sub-council, currently seeded empty (bench-seating ownership stays open). Deliberation stays `check-brand-council --deliberate`'s own job — out of scope for a role agent |
...
rewritten). **NEW agents**: `council-strategy-agent`, `council-design-agent`,
`council-voice-agent`, `council-creative-agent` — one per lens sub-council, patterned off
`brand-judge`'s own dispatch shape but dispatched DIRECTLY rather than fanned out unnamed;
`council-creative-agent` convenes a genuinely empty bench today (`creative` — a new ORDINARY
sub-council, not reserved like `advisory` — seeded with zero rows; bench-seating ownership stays
open, Kim seats it later). `roster.md` (`check-brand-council`) gains the `creative` section and the
`## Role agents` mapping (chair/strategy/design/voice/creative → their five agents);
`roster_check.py`'s selftest gains 7 new cases (empty-ordinary-sub-council, mapping positive,
dangling handle, unmapped role, section-absent, reserved-name key, undeclared key) — all green.
`make-council` now mints one role agent per sub-council alongside its critic-shell agent
(`references/roster-and-chair-wiring.md` gains the role-agent wiring checklist, patterned off
`council-strategy-agent`) and seeds the `## Role agents` mapping when standing up a new instance.
Non-goals (ruled in the seed): `check-brand-council`'s own phase-1 host-side orchestration and
sub-council enumeration are UNCHANGED — `creative` is convened today only via
