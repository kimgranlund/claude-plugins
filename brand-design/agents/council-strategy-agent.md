...
1. **Resolve the roster and check the bench.** Read
   ...
   `references/critics/` directory; resolve `strategy`'s seated, active handles only, never a
...
5. **Synthesize** — the five synthesis shapes (`council-rules`' `references/synthesis-shapes.md`),
   scoped to the strategy lens, incl. the blind spot (what a strategy-only read structurally
...
Done when every seated active strategy critic's verdict was collected (or UNMEASURED), every
