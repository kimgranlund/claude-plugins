...
- **A guiding concept or exemplar** — a concept, metaphor, or body of work to emulate (copy the _process_ that earned its meaning, not the surface), whose gravity bends every later decision.
- **A principle set** — a few commitments ("behave like a great magazine: a point of view, ruthless cuts, restraint as confidence") that pull the aesthetic and the choices.

Whatever form it takes, the test is the same: **does it add real gravitational pull toward a coherent direction** the work would not reach by drifting toward the category average?
...
> 1. **Name the ideal.** What is the highest version of this brand — who does the customer become at its best? State it as a direction the work can be pulled toward, not a slogan.

> 1. **Find the differentiating pull (the provocation).** Name the category's center of gravity — what every brand here does by default. If the brand's truth lies the other way, make the inversion explicit: the opposite is the aspiration. Trace it to a real root so it is a bet, not a stunt.

> 1. **Raid an adjacent world for the exemplar.** Find a _distant_ world that shares this brand's deep structure (its ritual, its devotion) and name what to emulate — the mechanism, not the surface — as gravity the work should bend toward.

> 1. **The contrarian / psycho-logic pull.** Where is the psychologically true but counterintuitive direction (the Rory S. angle) the category is too sensible to move toward? Name the fat-tailed aspiration.

> 1. **Set the principles.** Distill the pull into a few commitments that will bend later decisions — the editorial rules the brand will be held to.

> 1. **The pull-check.** Where is the current work drifting toward the category average — settling for the competent instead of the aspiration? Name where the gravity has weakened and what would restore it.

## How your output is used (the seam)
