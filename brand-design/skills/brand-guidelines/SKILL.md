...
**The brand-decomposer seam:** `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/guidelines_ledger.py" card <ledger> --idea "<from 01-foundation>" -o card.json` projects a `*.brand.json` card that `design-skills:brand-decomposer` can GRADE + operability-check (`brand-spec-check.py lint card.json`) when installed — the optional make→grade handoff (verified: a projected card clears brand-decomposer's operability gate). brand-forge stays self-contained.
