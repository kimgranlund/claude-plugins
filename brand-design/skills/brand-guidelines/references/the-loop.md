...
Append a typed **choice** to the ledger (validated by `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/guidelines_ledger.py"`):
...
**Location:** the ledger lives in the corpus so the assembled docs can `sources:`-cite it — **`00-sources/guidelines-elicitation.json` under the default corpus root `./brand-corpus`** (the same default `file-brand-corpus` + `make-brand-stack` use; pass a path to override). It is evidence — the choices that produced the guidelines. Append-only; a revision sets `supersedes` to the earlier entry id (never rewrite a decision — the corpus write-discipline). Validate after every write: `guidelines_ledger.py validate` (no arg → the default ledger).
...
Choices aren't independent: a loud-expressive color should inform the type + expression options offered next. Each accepted choice can write **coherence edges** into the ledger's `graph` (brand-decomposer's relationship vocabulary — `constrains · supports · contradicts · …`; edge ends are validated to be a real domain or entry id). Before framing a domain's 2×2, run **`guidelines_ledger.py coherence --domain <d>`** — it reports (mechanized) the **prior commitments** the new domain should cohere with, the **edges** touching it, and any **`contradicts`** edges to resolve. Frame the cards to honor the commitments, and **flag** (don't silently forbid) a contradiction. Coherence *informs*; it doesn't railroad — the computation is code, the resolution is judgment. (`--strict` exits non-zero on any unresolved contradiction, if you want it gated.)

## Assembly + scoring (the loop closes)

**Coverage:** `guidelines_ledger.py coverage` reports, per domain, resolved/absent + the frontier (the domains still to elicit). **Assembly:** `guidelines_ledger.py assemble [--out <corpus>] [--apply]` (defaults to `./brand-corpus`) compiles the **live** ledger (supersessions applied) → corpus docs deterministically (no inference in the compile): each domain's choices → its mapped layer doc (`mark`→`03-identity`, `color`/`type`/`expression`→`04-expression`, `voice`→`05-voice`, `governance`→`07-guidelines`), every choice rendered as a typed rule (`must/should/may`, default `should`) with its rationale · effect · exemplar · ledger-trace, each doc carrying `sources:` + `contributors:` frontmatter. It **matches the corpus's flat/folder convention, refuses a mixed corpus**, is **dry-run until `--apply`**, and is **non-destructive**: a hand-authored layer doc is never clobbered — the elicited version goes to a flagged `<name>.elicited.md` sibling (reconcile by hand, or `--force` to replace); re-assembling our *own* prior output is idempotent. Then **`corpus_provenance.py` gates the trace** (verified end-to-end: an assembled corpus passes clean) and **`brand-rubrics`** scores the result. The build loop closes into the provenance + evaluate loops.

**The brand-decomposer seam:** `guidelines_ledger.py card <ledger> [--idea "<brand idea>"] [-o card.json]` projects the live ledger into a `*.brand.json` card matching brand-decomposer's documented shape — each chosen design-move a typed `rule` (truth `proposed`; evidence pointing back at the ledger entry, so it's traced). Grade it with `design-skills:brand-decomposer` (the `nonoun-skills` marketplace) when installed: `brand-spec-check.py lint card.json`. [verified] end to end: a projected card **clears brand-decomposer's operability gate** (well-formed · traced · accessible · complete), with an honest `surfaces[]` incompleteness warn (the elicitation captures directional rules, not surface enumeration). Best-effort to brand-decomposer's shape, not a hard contract; brand-design stays self-contained.

## Relationship to brand-decomposer (parallel, not a dependency)

We MAKE; brand-decomposer GRADES. We mirror its six domains, seven axes, and evidence discipline as **vocabulary** (restated, not imported — it's a different marketplace and brand-design is self-contained), and hand it gradeable output via the optional card-projection seam. We do not re-implement its rubric, schema, or operability checker.
