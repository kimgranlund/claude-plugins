...
`bug-report`, `feature`, and `issue` each used to carry a hand-duplicated "Phase 0, decided once
...
|---|---|---|
| **A — local** | Today's default: a `doc-type: ticket` file under `docs/tickets/` (doc-forge's TICKET path) | Inline in each capture skill's own Record phase — unchanged from before this ADR |
...
reports the fallback in its close-out — the same discipline `issue`'s own git-native-backend
