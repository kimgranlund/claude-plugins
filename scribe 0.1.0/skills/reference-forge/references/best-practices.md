...
How to author the referential documents an agent reads to ground itself. These are retrieved, never obeyed. Assumes `foundations.md` (same directory). References are scored by `reference-forge/references/rubric.md`; llms.txt by `llms-txt-forge/references/rubric.md`.

**Canon home:** this file lives in `reference-forge` and is the knowledge family's shared best-practices canon; `llms-txt-forge/references/best-practices.md` symlinks in (one inode, divergence impossible). Symlinks are port-fragile, so the owner is declared here and in both consuming SKILL.md reference tables.
