...
not to describe what to build. Built via `rubric-author`; scored by **doc-reviewer** against this file.
...
dimensions that block shipping — and per rubric-author D8 that set may include a `[review]` dimension
...
ship set, sanctioned by rubric-author D8 (the gate set may name a review dimension whose failure is
