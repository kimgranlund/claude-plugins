...
|---|---|---|---|---|
| Daily board drain | Daily | Cloud `/schedule` routine → `/mobilize-chores auto` | Unattended (build-leader dispatch) | **Definition committed, not yet armed** — see "First standing schedule" below |
| Kim ruling/merge digest | Daily | Human pass over `.claude/ops/held-items.md`'s "Kim's ruling/merge queue" section | Kim | Living — channel exists now; digest cadence is a lean, not yet fired on a schedule of its own |
