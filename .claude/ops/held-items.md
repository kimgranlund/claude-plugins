...
- Status: queued — Kim to read the branch's `.claude/docs/adr/0027-narrow-t4-carve-out-for-intent-refs-backfill.md` and accept/reject/amend. Accepting it (merge + ratify) unblocks a follow-up per-ADR retrofit dispatch; #978 itself is already closed out as a valid BLOCKED-with-Findings outcome for this run regardless of ADR-0027's fate.
