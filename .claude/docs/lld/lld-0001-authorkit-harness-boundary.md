...
| Move the 3 standards skills alongside their checker agents (`skill-checker`, `agent-checker`, `wording-checker`) | Those checkers audit the *whole* harness estate, not just the authoring family; relocating harness's own self-hosting gate into a plugin disabled in this very workspace is worse than the problem it solves |
