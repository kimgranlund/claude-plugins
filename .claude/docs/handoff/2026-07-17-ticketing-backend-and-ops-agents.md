...
For the next agent/session opening this repo. Contract: harness's `write-handoff`.
...
- **Summary** — Ratified ADR-0003 (generalizes ADR-0002's binary file/git-native backend seam into
  ...
  versioned `break-down-problem` manifests, built a new grounded knowledge pack
  (`harness:github-facts`, real dated web research, forge bumped 1.31.0→1.33.0), and
  authored two new standing agents (`issue-sorter`, `repo-cleaner`) implementing the SPECs — each passed
  independent fresh-context review (doc-checker ×3, skill-checker, wording-checker,
  agent-checker ×2) with real findings caught and fixed, not rubber-stamped. Wired scheduling
  last: `issue-sorter` as an hourly cloud routine, `repo-cleaner` as a session-scoped local cron.
...
- **Tests/checks run** —
  ...
  dispatched, all findings applied, none skipped): `doc-checker` ×3, `skill-checker` +
  `wording-checker` on the knowledge pack, `agent-checker` ×2 on the two agents — pass, all
  fixed. `RemoteTrigger action:"run"` smoke test on the `issue-sorter` cloud routine → UNMEASURED,
...
- **Evidence** — ADR-0003 frontmatter: `status: accepted`,
  ...
  `agent_corpus_index.py` reverse-control caught a real bug: `issue-sorter.md`'s first draft
  ...
  fixed by dropping the preload and stating the needed TICKET shape inline (`issue-sorter.md`,
  Scope section). `repo-cleaner.md`'s first draft claimed `campaign_close.py` "discards the worktree" —
...
- **Risks** — (1) The entire change set is uncommitted, directly on `main`, at
  ...
  at this size; flagging for a maintainer call, locus: execution/process. (2) `issue-sorter`'
  hourly cloud routine's first run is unverified — needs `gh` authenticated inside an isolated
  cloud sandbox, untested whether it is; locus: execution. (3) `repo-cleaner`'s cron dies with this
  ...
  exists yet — `issue-sorter` correctly scopes to `gh`-only today, but don't assume Linear intake is
...
- **Open questions** — (1) Commit this directly to `main`, or stage as a campaign
  ...
  `issue-sorter` cloud routine actually working — check the routine link before trusting the hourly
  schedule; if `gh` isn't authenticated there it needs a different environment. (3) Re-arm
  `repo-cleaner` as a durable OS-crontab entry now that its session-scoped cron is about to disappear
  with this session close?

- **Recommended next action** — Maintainer (Kim) reviews `git status`/`git diff` for the full
  change set, decides commit-direct-vs-campaign, and checks the `issue-sorter` routine's first-run
