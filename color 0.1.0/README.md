...
`agents/token-builder`'s frontmatter preload list is `[check-colors]` only — `handoff-compose` and
`check-focus` were dropped from the hard preload because they now live in different plugins
(`forge` and `screens`, respectively) and a `skills:` preload cannot cross a plugin boundary. Both are
