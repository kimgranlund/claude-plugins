...
canon) in three stages:
  ...
  gamut walk   — an out-of-sRGB-gamut step reduces C in 0.005 steps until in gamut — C only,
                 NEVER L or H (the rule both palette-design and color-verify state); the ΔC is
...
published matrices) that color-verify/scripts/contrast-check.py uses — one math on both sides of
the design→verify contract, each selftest-locked against the same known values.

Invariants checked on EVERY build (color-verify's ramp invariants):
...
EVENNESS_MAX = 1.5      # color-verify's per-label-unit evenness gate (ΔL ÷ Δlabel, gap-to-gap)
...
# identical to color-verify/scripts/contrast-check.py).
_OKLAB_TO_LMS = (
...
# --- invariants (color-verify's ramp invariants) --------------------------------------------------
