...
description: >-
  ...
  (color-material-facts); NOT for building a ramp, theme, or semantic mapping (make-palette);
...
# color-theory-facts — what reads as intended

The judgment layer of color: harmony, meaning, and the history of the ideas designers still argue
with. Its primary sibling [[color-perception-facts]] answers *what is perceptually true*
(vision science); the wider `color-*-facts` family (spaces, accessibility, materials) covers
...
`color-*-facts` family, 2026-07-06); `references/INDEX.md` is the canonical manifest and owns
...
| Perceptual mechanism under an aesthetic effect — simultaneous contrast, warm/cool, opponent pairs | INDEX §Cited-from (straddle files live in the color-*-facts packs) |
...
2. Answer with the **claim, its cited file, and the correction the corpus carries** — this corpus
   ...
   > claim exits to [[color-contrast-facts]]'s APCA/WCAG contrast math.
3. Check the source-tracing rule before answering: every claim traces to a reference file; an
   ...
   [[color-contrast-facts]]'s; if it turned into gamut or ΔE numbers, it was
   [[color-space-facts]]'s.
4. Route output work at the boundary: build the ramp/theme/semantic mapping → [[make-palette]];
   prove a candidate palette (ColorProof, WCAG/APCA pass-fail, CVD) → [[check-colors]]; realize
...
- **This skill answers; it does not generate.** No ramps, no palettes, no token sheets — name the
  judgment and its evidence, hand the making to [[make-palette]] and the proof to
  [[check-colors]].
- **Mechanism and math belong to the `color-*-facts` family** — simultaneous contrast and
  warm/cool spectra → [[color-perception-facts]]; opponent process → [[color-contrast-facts]]
  (the straddle files this pack cites); space/conversion/gamut → [[color-space-facts]]; pigment
  → [[color-material-facts]]. Never duplicate its files; cite them.
