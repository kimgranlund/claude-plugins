...
Compose these primitives once at the token layer, then components consume only semantic tokens. See also [`apca-myndex-contrast.md`](../../../color-contrast-facts/references/techniques/apca-myndex-contrast.md) (color-contrast-facts) for picking text colors against a surface using APCA Lc thresholds rather than `contrast-color()`'s black/white heuristic.
