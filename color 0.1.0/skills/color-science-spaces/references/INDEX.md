...
Curated transcripts, notes, and source material for the `color-science-spaces` pack — computational
...
pack (the 5 historical files split to `color-science-materials` and `color-science-perception`); no
scrape sites live here either (the David Briggs huevaluechroma.com and colorandcontrast.com scrapes
moved whole to `color-science-perception`).

**Lineage (2026-07-06):** extracted from the monolithic `color-science` pack alongside three
siblings — `color-science-perception` (vision/appearance science), `color-science-accessibility`
(contrast standards & CVD), `color-science-materials` (pigment, print, naming) — plus the earlier
`color-theory` split (2026-07-02, harmony/meaning/history). Full prior history lives in
...
naming pickers (color.pizza API, 147colors, ISCC-NBS picker) moved to `color-science-materials`
§naming; the huevaluechroma.com/handprint.com entries moved with the vision-science and
pigment-data content they front (`color-science-perception` / `color-science-materials`); pure
...
`color-science-perception` (a scraped site is one cited source, never split by chapter). Its
space-conversion chapter is cross-cited here rather than duplicated:

| Chapter | Topics |
|---|---|
| *Color Spaces* (colorandcontrast.com, in `color-science-perception`) | sRGB, P3, LMS, XYZ, CIELAB, OKLAB, CAM16 — the space-conversion chapter of the colorandcontrast scrape; full site lives in `color-science-perception` |
