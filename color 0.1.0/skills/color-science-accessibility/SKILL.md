---
name: color-science-accessibility
description: >-
...
# color-science-accessibility — contrast standards & color-vision deficiency
...
siblings — `color-science-spaces` (space/gamut math), `color-science-perception` (vision
science), `color-science-materials` (pigment/naming). `references/INDEX.md` is the canonical
...
| WCAG 3 spec status | the one intentional cross-pack consult — INDEX §Cited-from-color-science-spaces (never duplicated) |
...
status" ask routes to `color-science-spaces`'s
[CSS Color 2026 Snapshot](../color-science-spaces/references/techniques/css-color-2026-snapshot.md)
...
- **Answers-vs-proves boundary with [[color-verify]]** (the seam this pack owns): theory,
  ...
  real hex values — is [[color-verify]]'s job. Route there, never improvise a verdict.
...
- **Space-conversion and gamut math belong to `color-science-spaces`** — sRGB/OKLCH/CIELAB
  conversion, gamut mapping, gradients. This pack cites its CSS Color snapshot but owns no
  conversion math itself.
- **Vision science beyond CVD belongs to `color-science-perception`** — cone mechanics, appearance
...
- **Building a ramp, theme, or semantic mapping is `palette-design`'s job**, not this pack's —
  hand it the accessibility target (e.g. "APCA 60 minimum") and let it build.
- **Harmony, mood, and meaning are `color-theory`'s domain**, not this pack's — a "which scheme is
