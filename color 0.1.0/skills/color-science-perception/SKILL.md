...
description: >-
  ...
  colorandcontrast.com. NOT for space-conversion math, gradients, or CSS (color-science-spaces); NOT
  for contrast standards or color-blindness checks (color-science-accessibility); NOT for pigment,
  print, or naming (color-science-materials); NOT for harmony, the color wheel, or meaning
  (color-theory). ANSWERS, does not generate.
disable-model-invocation: false
user-invocable: false
---

# color-science-perception — what the eye and brain do
...
pack (split into four: `color-science-spaces`, `color-science-perception`,
`color-science-accessibility`, `color-science-materials`); `references/INDEX.md` is the canonical
...
2. Answer with the **claim and its cited file** — every claim traces to a reference file; an answer
   ...
   > geometry is `color-science-spaces`'s to explain (HSL-is-geometry-not-perception); this pack
...
3. Check the boundary before answering: if the ask turned into space-conversion math, gamut, or CSS
   syntax, it was `color-science-spaces`'s; if it turned into a contrast ratio or CVD check, it was
   `color-science-accessibility`'s.
4. Route output work at the boundary: never produce a palette, ramp, or token sheet here — hand
   making to `palette-design` and end-to-end proof to `color-verify`.
...
## Straddle rule with color-theory

`color-theory` documents harmony, meaning, and the history of the wheel; three files in this pack
...
of these files, answer the mechanism and route the judgment itself to `color-theory`.

## Boundaries

- **This skill answers; it does not generate.** No ramps, no palettes, no token sheets — name the
  vision-science claim and its evidence; hand making to `palette-design` and proof to `color-verify`.
- **Space-conversion math, gamut, gradients, and CSS color syntax belong to `color-science-spaces`**
...
- **Contrast standards and color-vision-deficiency checks belong to `color-science-accessibility`**
...
- **Pigment, print, and color naming belong to `color-science-materials`** — including the
  huevaluechroma subtractive-mixing chapters (ch05, ch06), which materials cross-cites from this
  pack rather than duplicating.
- **Harmony, the color wheel, and meaning belong to `color-theory`** — see Straddle rule, above.

## Extending this pack

A missing axis, a stale reference, or "add X to this pack" is authoring work — route to
[[knowledge-author]] (axis decomposition, grounded research waves, index discipline); never bolt an
