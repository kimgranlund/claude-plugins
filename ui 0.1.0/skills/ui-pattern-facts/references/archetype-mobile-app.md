...
> The A1–A5/B1–B5 notes below are owned by break-down-layout's method (references/decomposition-method.md there); this file hosts them for one-stop reading. This file names the shell's REGIONS; the platform's own semantics for them — tab-bar vs stack guidance, sheet modality (incl. iOS's modal-vs-nonmodal split), detents, alert escalation — are `apple-mobile-facts`' territory (2026-07-15): consult it when the question is what HIG says a region is FOR, not where it sits.
