...
A · OUTSIDE-IN  ┌────────────────────────┬────────────────────────┐
    passes      │      SHIPPABLE         │  pretty but dead —      │
                │  (floor: §Scoring)     │  clean frame, aligned   │
                │                        │  atoms, but panels host │
                │                        │  no verbs / a verb has  │
                │                        │  no home                │
                ├────────────────────────┼────────────────────────┤
A · OUTSIDE-IN  │ functional but         │  broken — re-run DESIGN │
    fails       │ unreadable — every     │  from the archetype     │
                │ action works, but it   │                         │
                │ all stacks in one      │                         │
                │ column, no frame       │                         │
                └────────────────────────┴────────────────────────┘
...
- **A1 is archetype-relative:** productivity-shell and saas-dashboard require the full fixed shell
  (only inner panes scroll). **mobile-app** requires *pinned chrome* — header and/or tab-bar stay
  put while the view scrolls between them; unpinned chrome that scrolls away fails A1.
  **marketing-site** legitimately page-scrolls — A1 there asks only that the nav band behaves
  deliberately (sticky or consciously not). Grade against the matched archetype's clause.
...
- `[review]` dimensions score **1–5**; a shippable layout is **≥4 on every review with zero gate failures**.
  *This line is the canonical shippable floor — SKILL.md and the quadrant point here; state it nowhere else.*
