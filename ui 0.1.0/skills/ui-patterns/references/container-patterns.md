...
- **Failure:** a drawer with both autosave AND a stale "Save" button in the footer — two save
  models on one surface (the same mixed-save-model failure `ui-patterns`' own macro `Consult
...
- **Failure:** "card-in-a-card" — visually redundant nesting where the inner boundary adds no
  ...
  rather than collapsing — is `geometry-systems`' `composable-spacing.md`; this file names the
...
composed padding at nested boundaries is `geometry-systems`; building the container as a real
component is `component-forge`; placing it within a page's region map is `layout-decompose`.
