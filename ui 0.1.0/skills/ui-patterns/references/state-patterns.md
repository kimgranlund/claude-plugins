...
| **loading** | latency window | reserve layout (no CLS), skeleton for structure / spinner for actions — the decision table and time budgets are `perf-verify`'s |
...
optimistic+undo is allowed vs type-to-confirm is `safety-verify`'s; the latency windows and
rollback behavior are `perf-verify`'s. This file only places them: undo-toast for reversible,
...
Cross-SCREEN state machines — a journey's states, transitions, and exits — are `flow-decompose`'s
