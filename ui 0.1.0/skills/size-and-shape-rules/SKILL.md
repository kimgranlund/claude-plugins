...
description: >-
  ...
  component family's law (make-component) or bind to an already-specified platform scale
  (material-shape-facts), its two cited worked instances. NOT for page-region layout
  (break-down-layout); NOT for raw CSS box-model/flow mechanics (dom-layout-facts); NOT for a
  component's own anatomy (make-component).
