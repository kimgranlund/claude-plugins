---
date: 2026-06-03
curated: 2026-07-02 — harvested from the product-forge product-genres corpus; product-metrics section and surrogate-metric strategy dropped, UI conventions kept
...
> The genre's defining tension: the session that proves the product is working (sustained, voluntary attention) is one keystroke away from the session that proves it is exploiting the user (compulsive, regretted attention). Designing this genre well means engineering engagement that the user would endorse on reflection — and being able to tell the difference.
...
This is the genre's defining design-ethics question, and a review should score it explicitly.
...
- **Filter-bubble collapse / discovery starvation.** Over-tuning relevance re-serves the same content, starving the long tail and making the catalog feel smaller than it is.
