---
date: 2026-06-03
curated: 2026-07-02 — harvested from the product-forge product-genres corpus; the metrics section dropped (the journey-length figures folded into the persistence convention), UI conventions kept
coverage: expanded
primary_sources:
...
- **The journey persists across sessions.** Recently-viewed, saved/favorited, price-watch, and "pick up where you left off" are expected — because the purchase spans many sessions and devices: Think with Google's traveler research found people use **~7 touchpoints for inspiration and up to ~10 for research**, with cross-device journeys running longer still (Google-sponsored research; directional). A product that forgets the user between visits is fighting the genre.
