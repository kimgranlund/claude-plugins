...
This file is the canon for the verify species — check-focus, check-translations, check-speed,
check-safety, check-ui-change, this sweep, and color's check-colors (cross-plugin: cited
...
| One CHANGE (the diff's surfaces) | check-ui-change — which composes the owning verifiers per affected dimension |
| The whole product / a flow set | check-whole-ui's sweep |
...
(break-down-layout's taste-elicitation canon owns that discipline).
