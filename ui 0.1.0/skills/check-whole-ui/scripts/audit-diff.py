#!/usr/bin/env python3
"""audit-diff.py — the check-whole-ui baseline differ (the audit ledger). Self-contained (stdlib only).
...
FIX_BASE = [
    {"checker": "check-colors", "gate": "CONTRAST_FAIL_AA", "id": "dashboard",
     "screen": "dashboard", "detail": "4.2:1 body text"},
    {"checker": "check-focus", "gate": "NO_VISIBLE_FOCUS", "id": "settings#save"},
...
FIX_CUR = [
    {"checker": "check-colors", "gate": "CONTRAST_FAIL_AA", "id": "dashboard",
     "screen": "dashboard", "detail": "4.2:1 body text"},
    {"checker": "flow-check", "gate": "DEAD_END", "id": "checkout:pay"},
    {"checker": "check-focus", "gate": "POSITIVE_TABINDEX", "id": "reports#filter",
     "detail": "tabindex=3"},
]
...
def selftest():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        data = json.loads(out)
        if _keys(data["new"]) != [("check-focus", "POSITIVE_TABINDEX", "reports#filter")]:
            errs.append("NEW bucket wrong: %r" % _keys(data["new"]))
        if _keys(data["resolved"]) != [("check-focus", "NO_VISIBLE_FOCUS", "settings#save")]:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with open(bad_f, "w", encoding="utf-8") as fh:
            fh.write('{"skill": "check-whole-ui", "dim": "S1", "finding": "no tier"}\n')
