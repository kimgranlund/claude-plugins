...
# check-focus — keyboard & focus invariants, card-gated
...
| [[check-colors]] | the ramps/contrast primitives ring colors draw on |
| `token-builder` agent | consumes emitted interactive-role tokens |
| [[make-component]] / the repo's component seat | where keyboard-affordance defects route — the maker that fixes a role's missing APG keys |
