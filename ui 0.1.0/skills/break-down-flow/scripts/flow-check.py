#!/usr/bin/env python3
"""flow-check.py — the break-down-flow mechanism gate. Self-contained (stdlib only).
...
Checks (gate = a hard FAIL; advisory = a WARN that surfaces a risk for human review):
  ...
  UNKNOWN_SCREEN    (advisory) — with --inventory inventory.json (check-whole-ui's spine): a state whose
...
def _load_screens(inventory_path):
    """The inventory's screen ids (check-whole-ui's inventory-scan.py shape, or a bare list)."""
