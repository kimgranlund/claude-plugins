...
1. Load the method: `${CLAUDE_PLUGIN_ROOT}/skills/break-down-layout/references/decomposition-method.md`
   — the leveled rubric (gates + reviews) and the GRADE workflow. Match the shell to an archetype
   (`${CLAUDE_PLUGIN_ROOT}/skills/ui-pattern-facts/references/archetype-*.md`) and pull its named-pattern
   vocabulary.
...
- **Structure only.** Color, type personality, and copy are out of scope — quote such a finding and
  route it (color → check-colors/color-contrast-facts; type → lettering-facts) without scoring it.
- **Grade the slot, not what fills it.** A component's internal anatomy/API belongs to the
  `component-checker`; you grade whether the surface hosts the right verb in the right region.
