...
- **Regenerate a plugin's harness overlays** (Codex + Hermes + Pi all land as of W3, #891):
  `python3 harness/scripts/harness_emit.py <plugin> [--harness codex,hermes,pi] [--verify | --probe]` —
  writes `.codex-plugin/plugin.json`, per-skill `agents/openai.yaml`, `plugin.yaml`,
  `__init__.py`, `hermes-mcp.yaml` (where `.mcp.json` exists), `package.json`,
  `prompts/<name>.md` (one per command-only skill), and `HARNESS-NOTES.md` in-tree
  (committed, never `dist/`); `release_gate.py`'s G15 verifies freshness (`codex,hermes,pi`),
  the writer runs in `/ship-plugin`'s own preflight (LLD-0025).
